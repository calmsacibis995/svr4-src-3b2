...
char	*str;
{
	LOG2(L_TRACE, "(%5d) enter: isnum\n", Logstamp);
	if (*str == '\0') {
		LOG2(L_TRACE, "(%5d) leave: isnum\n", Logstamp);
		return(0);
	}
...
struct mach  *curr_mach;
{
	...
	LOG2(L_TRACE, "(%5d) enter: place_name\n", Logstamp);
	...
	LOG2(L_TRACE, "(%5d) leave: place_name\n", Logstamp);
}
...
char	    *token;
{
	...
	LOG2(L_TRACE, "(%5d) enter: map_rtn\n", Logstamp);
	mach = curr_mach->m_name->l_val;

	if (EQ(token, "all")) {
		LOG2(L_TRACE, "(%5d) leave: map_rtn\n", Logstamp);
...
struct nmlist *nmlist;
{
	...
	LOG2(L_TRACE, "(%5d) enter: place_map\n", Logstamp);
	...
	while (place != NULL) {
		if (place->idmap->i_rem == nmlist->idmap->i_rem) {
			...
			LOG2(L_TRACE, "(%5d) leave: place_map\n", Logstamp);
			return(SUCCESS);
		}
		if (place->idmap->i_rem > nmlist->idmap->i_rem)
			break;
...
char	    *token;
{
	...
	LOG2(L_TRACE, "(%5d) enter: excl_rtn\n", Logstamp);
	mach = curr_mach->m_name->l_val;
	...
	if (last != NULL) {
		if (!isnum(first) || !isnum(last) || 
			...
			LOG2(L_TRACE, "(%5d) leave: excl_rtn\n", Logstamp);
			return(FAILURE);
		}
	}
...
struct mach *mach_info;
{
	...
	LOG2(L_TRACE, "(%5d) enter: pr_trtab\n", Logstamp);
	...
	if (count == 0) {
		if (rfsys(RF_SETIDMAP, name, mapping, buf) < 0) {
			if (errno != ENONET) {
				...
				LOG2(L_TRACE, "(%5d) leave: pr_trtab\n", Logstamp);
				exit(1);
			}
		}
		LOG2(L_TRACE, "(%5d) leave: pr_trtab\n", Logstamp);
...
int size;
{
	...
	LOG2(L_TRACE, "(%5d) enter: m_malloc\n", Logstamp);
	if ((ptr = malloc(size)) == 0) {
		if (print_err_msg)
			fprintf (stderr, "%s: FATAL: cannot allocate memory\n", cmd_name);
		LOG2(L_TRACE, "(%5d) leave: m_malloc\n", Logstamp);
		exit(1);
	}
	LOG2(L_TRACE, "(%5d) leave: m_malloc\n", Logstamp);
	return(ptr);
...
char	*name;
{
	...
	int  found = 0;
	int  inval = 0;
	uid_t rtn = GETID_FAIL;

	LOG2(L_TRACE, "(%5d) enter: getid\n", Logstamp);
	/*
...
char *mach;
{
	...
	LOG2(L_TRACE, "(%5d) enter: read_file\n", Logstamp);
	if (mach == NULL)
		strcpy(file, "/etc");
	else {
		/*
...
char	*name;
{
	...
	LOG2(L_TRACE, "(%5d) enter: checkname\n", Logstamp);
	/*
	...
	if (ptr == NULL || ptr == name || *(ptr + 1) == '\0') {
		...
		LOG2(L_TRACE, "(%5d) leave: checkname\n", Logstamp);
		return(FAILURE);
	}
...
getdname()
{
	...
	LOG2(L_TRACE, "(%5d) enter: getdname\n", Logstamp);
	/*
	...
	if (rfsys(RF_GETDNAME, dname, MAXDNAME) < 0) {
		...
		|| (fgets(dname,MAXDNAME,fp) == NULL)) {
			LOG2(L_TRACE, "(%5d) leave: getdname\n", Logstamp);
			return(NULL);
		}
		/*
...
char	*token2;
{
	LOG2(L_TRACE, "(%5d) enter: parse_err\n", Logstamp);
	if (print_err_msg == 0) {
		LOG2(L_TRACE, "(%5d) leave: parse_err\n", Logstamp);
		return;
	}

...
struct netbuf	*addr;
{
	...
	LOG2(L_TRACE, "(%5d) enter: stoa\n", Logstamp);
	myadr = FALSE;

	if (!str) {
		LOG2(L_TRACE, "(%5d) leave: stoa\n", Logstamp);
		return NULL;
	}
...
int	quote;
{
	...
	LOG2(L_TRACE, "(%5d) enter: dostring\n", Logstamp);
	while (*s) {
		if (len >= SBUFSIZE) {
	...
	LOG2(L_TRACE, "(%5d) leave: dostring\n", Logstamp);
...
int	type;
{
	...
	LOG2(L_TRACE, "(%5d) enter: atos\n", Logstamp);
	if (addr == NULL) {
		LOG2(L_TRACE, "(%5d) leave: atos\n", Logstamp);
		return NULL;
	}
...
unsigned	l;
{
	...
	LOG2(L_TRACE, "(%5d) enter: o_atos\n", Logstamp);
	b = s;
	if (l == 0) {
		*s = '\0';
		LOG2(L_TRACE, "(%5d) leave: o_atos\n", Logstamp);
		return 0;
	}
...
int	n;
{
	LOG2(L_TRACE, "(%5d) enter: memcp\n", Logstamp);
	while (n--)
		*d++ = *s++;
	LOG2(L_TRACE, "(%5d) leave: memcp\n", Logstamp);
}
...
char	*s;
{
	...
	LOG2(L_TRACE, "(%5d) enter: prescan\n", Logstamp);
	rec = sbp = 0;
	if (!s || !*s || !scan(s)) {
		LOG2(L_TRACE, "(%5d) leave: prescan\n", Logstamp);
		return NULL;
	}
	scanbuf[sbp] = '\0';
	LOG2(L_TRACE, "(%5d) leave: prescan\n", Logstamp);

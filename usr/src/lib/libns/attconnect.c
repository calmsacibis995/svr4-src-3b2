...
#ident	"@(#)libns:attconnect.c	1.10.5.1"
#include "sys/types.h"
#include <tiuser.h>
#include <nsaddr.h>
#include "nserve.h"
...
long serv_code;
{
	...
	LOG2(L_TRACE, "(%5d) enter: att_connect\n", Logstamp);
	for (n = 1; n <= C_RETRY; ++n) {
		if ((fd = snd_connect(addr)) == -1) {
			LOG2(L_TRACE, "(%5d) leave: att_connect\n", Logstamp);
			return(-1);
		}
...
struct address *addr;
{
	...
	LOG2(L_TRACE, "(%5d) enter: snd_connect\n", Logstamp);
	...
	if (addr->protocol == NULL) {
		...
		LOG2(L_TRACE, "(%5d) leave: snd_connect\n", Logstamp);
		return(-1);
	}

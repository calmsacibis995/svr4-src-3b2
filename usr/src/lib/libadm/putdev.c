	...
	char  **pname;			/* A(ptr to temp filename's path) */
{
	...
	if (oldname = _devtabpath()) {
		...
		*(p+1) = '\0';
		 ...
		 * file.  We must reset "umask" for the operation to be
		 * sure the file's created with the appropriate permissions
		 */

		(void) sprintf(buf, TDTABNM, dirname, getpid());
		oldumask = umask((mode_t) 0133);
		if (fd = fopen(buf, "w")) *pname = buf;
		else free(buf);
		(void) umask(oldumask);
	    }
	...
	struct devtabent       *rec;		/* Record to write */
{
	...
	if (rec->comment) {
		...
		*(p-1) = '\0';					/* Terminate string */
		...
		size += (strlen(attrval->val)*2) +4;	    /* val & '="" ' or val & '=""\n' */

 ...
 *	realloc()	Shrink or enlarge malloc()ed memory
 *	devattr()	Get a device's attributes
 */
...
static	char *
oktoaddtolist(devtabentry, devices, aliases, options)
	...
	int	options;	/* Options governing search */
{
	...
	if (devices && aliases) {
	    ...
	    /* Set value to return */
	    if (found) rtnval = (options&DTAB_EXCLUDEFLAG) ? (char *) NULL : *devices;
	    else rtnval = (options&DTAB_EXCLUDEFLAG) ? devtabentry : (char *) NULL;
	...
	int			options;
{
	...
	if (options & DTAB_ANDCRITERIA) {
		...
		if (!devtabent->comment) {
			...
			if (alias = (char *) malloc((unsigned) strlen(devtabent->alias)+1))
	...
	struct srch	       *criteria;	/* Criteria governing match */
{
	...
	if (criteria && (criteria->fcn != ENDLIST)) {
		...
		if (p->fcn != IGNORE) {
			...
			else if (strcmp(p->name, DTAB_CDEVICE) == 0) failed = FALSE;
			else if (strcmp(p->name, DTAB_BDEVICE) == 0) failed = FALSE;
			else if (strcmp(p->name, DTAB_PATHNAME) == 0) failed = FALSE;
			else {
			    q = ent->attrlist;
			    while (!failed && q) {
				if (strcmp(p->name, q->attr) == 0) failed = TRUE;
				else q = q->next;
			    }
			}
		    }
	...
	struct srch	       *criteria;	/* Criteria governing match */
{
	...
	if (criteria && (criteria->fcn != ENDLIST)) {
		...
		if (p->fcn != IGNORE) {
			...
			else if (strcmp(p->name, DTAB_CDEVICE) == 0) matched = FALSE;
			else if (strcmp(p->name, DTAB_BDEVICE) == 0) matched = FALSE;
			else if (strcmp(p->name, DTAB_PATHNAME) == 0) matched = FALSE;
			else {
			    q = ent->attrlist;
			    matched = TRUE;
			    while (matched && q) {
				if (strcmp(p->name, q->attr) == 0) matched = FALSE;
				else q = q->next;
			    }
			}
		    }

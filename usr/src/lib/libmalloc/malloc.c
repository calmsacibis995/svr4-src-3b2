...
/* we initialize the following 2 structures to make sure
 * they are assigned addresses in the order of their declarations -
 * later code depends on this order
 */
static struct header arena[2] = {0,0,0,0,0,0};  
				/* the second word is a minimal block to
				   start the arena. The first is a busy
				   block to be pointed to by the last
				   block.       */
static struct header freeptr[2] = {0,0,0,0,0,0};
		...
		if (blk == &(freeptr[1]))  {
			...
			/* Three cases - 1. There is space between arenaend
					 ...
					 3. Case 1 is not true, and the last
					    block is free
			*/
			if ((newblk = (struct header *)_sbrk(0)) != 
...
rstalloc()
{
	...
	maxfast = MAXFAST;
	change = 0;
	if(freeptr[0].nextfree == GROUND) return;

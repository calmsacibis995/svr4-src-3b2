...
#ident	"@(#)nametoaddr:resolv/resolv.c	1.2"

/*
 * Domain Name System name to address translation routines. These
 * routines are written to the getXXXbyYYY() interface that the BSD routines
 * use. This allows us to simply rewrite those routines to get various flavors
 * of translation routines. Thus while they look like they have socket
 * dependencies (the sockaddr_in structures) in fact this is simply the
 * internal netbuf representation that the TCP and UDP transport providers use. 
 */
	...
	struct nd_hostserv *serv;
{
	 ...
	 */
	se = (struct servent *)_resolv_getservbyname(serv->h_serv,
	...
	if ((strcmp(serv->h_host, HOST_SELF) == 0)) {
		...
		he = (struct hostent *)_resolv_gethostbyname(serv->h_host);
	}
	...
	struct netbuf		*addr;
{
	...
	he = (struct hostent *)_resolv_gethostbyaddr(&(sa->sin_addr.s_addr),
			4, sa->sin_family);
	if (!he) {
		_nderror = ND_NOHOST;
		return (NULL);
	...
	se = (struct servent *)_resolv_getservbyport(sa->sin_port,
		(strcmp(tp->nc_proto, NC_TCP) == 0) ? "tcp" : "udp");
	if (!se) {
	...
	char			*uaddr;	/* the address */
{
	...
	if (addrlist == (char **)NULL) {
		...
		he = (struct hostent *)_resolv_gethostbyname(u.nodename);
		if (he == NULL) {
			_nderror = ND_NOHOST;
			return ((char *)NULL);
		}
	...
	struct in_addr *addrs;
{
	...
	for (i = 0, n = ifc.ifc_len/sizeof (struct ifreq);
		...
		if ((ifreq.ifr_flags & IFF_BROADCAST) &&
			...
			if (ifioctl(fd, SIOCGIFBRDADDR, (char *)&ifreq, 0) < 0) {

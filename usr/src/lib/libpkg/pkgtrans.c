...
/*LINTLIBRARY*/
#ident	"@(#)libpkg:pkgtrans.c	1.15.3.1"
...
static int	ds_volno;
static int	ds_fd = -1; /* open file descriptor for data stream */
static void	(*func)();
...
int	options;
{
	...
	ids_name = NULL;
	if(srcdev.bdevice) {
		if(n = getvol(srcdev.bdevice, NULL, NULL, "Insert %v into %p.")) {
			cleanup();
			if(n == 3)
				return(3);
			progerr(ERR_TRANSFER);
	...
	if(ids_name) {
		if(srcdev.cdevice && !srcdev.bdevice && (n = getvol(srcdev.cdevice, NULL, NULL, NULL))) {
			cleanup();
			if(n == 3)
				return(3);
			progerr(ERR_TRANSFER);
	...
	if(options & PT_ODTSTREAM) {
		char line[128];
		if(!dstdev.pathname && (n = getvol(ods_name, NULL, NULL, NULL)) ) {
			cleanup();
			if(n == 3)
				return(3);
			progerr(ERR_TRANSFER);

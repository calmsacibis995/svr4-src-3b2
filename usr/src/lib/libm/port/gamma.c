...
double y;
{
	extern double _lgamma();
	
	gammaflag = 1; /*signal _gamma */
	return _lgamma(y);
}
 	...
 	if (y <= 0) {
		...
		if (fract == 0 && I % 2 == 0) {
			...
			if (_lib_version == strict_ansi)
				...
				if (_lib_version == c_issue_4)
					if (gflag)
						(void)_write(2, "gamma: SING error\n", 18);
					else
						(void)_write(2, "lgamma: SING error\n", 19);
				errno = EDOM;
			}
			return exc.retval;
		} else {
			 ...
			 */
			res = -_lgamma(-y) + log(M_PI / (-y * sinepiy));
		}
	} else if (y <= eps) {	
		/*y>0*/
 	      	res = -log(y);

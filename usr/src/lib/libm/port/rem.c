	   ...
	   ((yexp == MAXEXP) && (LOWORD(y) || (HIWORD(y) & 0xfffff)))){
		...
		if (_lib_version == strict_ansi)
			...
			if (_lib_version == c_issue_4)
				(void)_write(2,"remainder: DOMAIN error\n",24);
			errno = EDOM;
		}
		return (exc.retval);
	}
...
#endif
	/* subnormal number */
	mask = _fpsetmask(0);  /* mask all exceptions */
	nx = 0;
	if (yexp == 0) {
		t = 1.0, EXPONENT(t) += 57;
		y *= t; 
...
loop:
	...
	(void)_fpsetmask(mask);  /* reset exception masks */
	return(x);
}
#else

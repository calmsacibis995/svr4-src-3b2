	...
	u_int recvsz;			/* buffer send size */
{
	...
	case T_OUTCON:
		if (svcaddr == (struct netbuf *)NULL) {
			...
			ct->ct_addr = *svcaddr;
			ct->ct_addr.buf = malloc(svcaddr->maxlen);
			...
			memcpy(ct->ct_addr.buf, svcaddr->buf, (int)svcaddr->len);
		}
		break;
	default:
		rpc_createerr.cf_stat = RPC_UNKNOWNADDR;
	...
	register int len;
{
	...
	while (TRUE) {
		...
		switch (select(_rpc_dtbsize(), &readfds, (fd_set *)NULL, (fd_set *)NULL,
				&(ct->ct_wait))) {
		case 0:
	...
	int len;
{
	...
	if (maxsz == -2)	/* Transfer of data unsupported */
		return (-1);
	if ((maxsz == 0) || (maxsz == -1)) {
		if ((len = t_snd(ct->ct_fd, buf, (unsigned)len, (int)0)) == -1) {
			ct->ct_error.re_terrno = t_errno;
			ct->ct_error.re_errno = 0;
			ct->ct_error.re_status = RPC_CANTSEND;
		}

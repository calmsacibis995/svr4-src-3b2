	...
	u_int recvsz;			/* buffer send size */
{
	...
	if (cu == (struct cu_data *)NULL)
		goto err;
	cu->cu_raddr = *svcaddr;
	if ((cu->cu_raddr.buf = mem_alloc(svcaddr->maxlen)) == NULL)
		goto err;
	(void) memcpy(cu->cu_raddr.buf, svcaddr->buf, (int)svcaddr->maxlen);
	...
	struct timeval	utimeout;	/* seconds to wait before giving up */
{
	...
	struct t_unitdata tu_data, *tr_data;
	...
	for (;;) {
		...
		if (_svc_getreqset_proc) {
			for (fds = 0; fds < howmany(FD_SETSIZE, NFDBITS); fds++)
				readfds.fds_bits[fds] |= svc_fdset.fds_bits[fds];
		}
		...
		do {
			...
			moreflag = 0;
			tr_data = (struct t_unitdata *)t_alloc(cu->cu_fd, T_UNITDATA, T_ADDR);
	...
	CLIENT *cl;
{
	...
	(void) mem_free((caddr_t)cu, (sizeof(*cu) + cu->cu_sendsz + cu->cu_recvsz));

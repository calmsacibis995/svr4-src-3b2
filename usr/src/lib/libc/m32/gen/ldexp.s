.ident	"@(#)libc-m32:gen/ldexp.s	1.9.1.6"
	.file	"ldexp.s"
...
.huge_val:
	.word	0x7ff00000,0x0	# positive infinity
.2TO52:
	.word	0x43300000,0
...
.nan_inf:			# Returns the input number
	movw	&EDOM,_dref_(errno)	# indicate domain error
	ret	&1
#-------------------------------------------------------------------------
.denormal:			# Multiply by 2 ** 52
	pushw	_sref_(.2TO52)	# to guarantee a normalized number
	pushw	_sref_(.2TO52+4)
	pushw	%r0
	pushw	%r1
	call	&4,_fref_(_fmuld)
	EXTFW	&DEXP_WIDTH-1,&DEXP_OFFSET,%r0,%r2	# biased exponent
...
.under:
	...
	pushw	%r2
	pushw	&0
	call	&4,_fref_(_fmuld)	# Multiply to produce denormalized result
	jmp	.done
#-------------------------------------------------------------------------
#	Return zero result
.zero:
...
#	Overflow
.over:
	cmpw	_dref_(_lib_version),&0	# if (_lib_version == c_issue_4)
	jne	.ansi
	movw	_sref_(.huge),%r0	# Largest possible floating magnitude
	movw	_sref_(.huge+4),%r1
	jmp	.ntest
.ansi:
	movw	_sref_(.huge_val),%r0	# Positive infinity
	movw	_sref_(.huge_val+4),%r1
.ntest:
	jbc	&31,0(%ap),.err	# Jump if argument was positive
	orw2	&SIGN_BIT,%r0	# If arg < 0, make result negative
#-------------------------------------------------------------------------
.err:
	movw	&ERANGE,_dref_(errno)	# Indicate range error
	jmp	.done
#------------------------------------------------------------------------

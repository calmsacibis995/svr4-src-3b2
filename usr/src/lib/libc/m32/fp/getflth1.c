...
#ident	"@(#)libc-m32:fp/getflth1.c	1.5.1.11"
...
#if DSHLIB
extern const unsigned char _flth_op_table[];
#else
static const unsigned char _flth_op_table[ 256 ] = {
 ...
 */
#define	opsize(x)	((int)(_flth_op_table[(int)(x)]) >> 4)
#define	opcount(x)	(_flth_op_table[(unsigned char)(x)] & 0x0f)
...
#if DSHLIB
extern const char _flth_op_size[16];
extern const char _flth_pc_op_size[16];
#else
    static const char	_flth_op_size[16] = {
    ...
    static const char  _flth_pc_op_size[16] = {
	1,1,1,1, /* 0-literal, 1-literal, 2-literal, 3-literal	*/
	...
	return ( pc + _flth_pc_op_size[ hinib ] );
    if ( hinib == 0xe )				/* expand-o-mode */
	return ( nextop( pc + 1 ) );
    else					/* everything else */
	return ( pc + _flth_op_size[ hinib ] );
    }
...
#if DSHLIB
extern const struct mau_op _flth_src[8];
extern const struct mau_op _flth_dst[16];
#else
    static const struct mau_op _flth_src[8] = {
	{ IS_REG,	0,	3,	0 },
	{ IS_REG,	1,	3,	0 },
	{ IS_REG,	2,	3,	0 },
	{ IS_REG,	3,	3,	0 },
	...
	{ IS_MEM,	0,	3,	3 },
	{ IS_NULL,	0,	0,	0 } };
    static const struct mau_op _flth_dst[16] = {
	{ IS_REG,	0,	3,	1 },
	{ IS_REG,	1,	3,	1 },
	{ IS_REG,	2,	3,	1 },
	{ IS_REG,	3,	3,	1 },
    ...
    if ( flag == 0 )
	return ( _flth_src[ desc ] );
    else
	return ( _flth_dst[ desc ] );
    }

 ...
 * determined, return UNKNOWN. Remember that inexact can occur at the
 * same time as over or underflow.
 * Or ASR sticky bits into shadow sticky bits. Clear all unmasked ASR
 * sticky bits
 *
...
mau_except()
	...
	/* overflow can happen with inexact */
	...
	/* underflow can happen with inexact */
     	if ( (temp.pm == 1) && (temp.ps == 1) ) {
		temp.us = 0;
		temp.ps = 0;
		mau_asr( (int *)&temp,HW_WRITE ); 
     ...
     /* we must do this test last in case inexact has occurred 
      * with under or overflow
      */
     if ( (temp.pm == 1) && (temp.ps == 1) ) {
	temp.ps = 0;
	mau_asr( (int *)&temp,HW_WRITE ); 
...
format_data( faultp, insp, drp, ftype, r0ptr )
     ...
     */
#if DSHLIB
extern const fp_op _flth_op_map[32];
#else
    static fp_op	_flth_op_map[32] = {
     ...
     */
    faultp->operation = _flth_op_map[ insp->opcode ];
 ...
 * ASSUMPTION:
 *	the instruction stream sanity has been checked, so that
...

	...
	else (insp->wr_op)[0] = *(int *)(&faultp->result.value.f);
	if (faultp->result.value.f < 0.0) {
		t_asr.n = 1;		/* set N bit */
		t_psw.NZVC |= PS_N;
	}
	else if (faultp->result.value.f == 0.0) {
		t_asr.z = 1;		/* set Z bit */
		t_psw.NZVC |= PS_Z;
	}
	...
	if (faultp->result.value.l < 0) {
		t_asr.n = 1;		/* set N bit */
		t_psw.NZVC |= PS_N;
	}
	else if (faultp->result.value.l == 0) {
		t_asr.z = 1;		/* set Z bit */
		t_psw.NZVC |= PS_Z;
	}
	break;
	...
	if (faultp->result.value.d < 0.0) {
		t_asr.n = 1;		/* set N bit */
		t_psw.NZVC |= PS_N;
	}
	else if (faultp->result.value.d == 0.0) {
		t_asr.z = 1;		/* set Z bit */
		t_psw.NZVC |= PS_Z;
	}
	break;
	...
	if ( insp->wr_op == (int *)0 ) {
		...
		(insp->wr_op)[0] = ((int *)&(faultp->result.value.x))[0];
		(insp->wr_op)[1] = ((int *)&(faultp->result.value.x))[1];
		(insp->wr_op)[2] = ((int *)&(faultp->result.value.x))[2];
	    }
	if (((int *)&(faultp->result.value.x))[0] & 0x8000) {
		/* sign bit of extended set */
		t_asr.n = 1;		/* set N bit */
		t_psw.NZVC |= PS_N;
	}
	else if ( (((int *)&(faultp->result.value.x))[0] == 0) &&
		(((int *)&(faultp->result.value.x))[1] == 0) &&
		(((int *)&(faultp->result.value.x))[2] == 0) ) {
		t_asr.z = 1;		/* set Z bit */
		t_psw.NZVC |= PS_Z;
	}
	    break;
	...
	if (((int *)&(faultp->result.value.p))[2] % 2) {
		/* negative packed decimal*/
		t_asr.n = 1;		/* set N bit */
		t_psw.NZVC |= PS_N;
	}
	else if ( (((int *)&(faultp->result.value.p))[0] == 0) &&
		(((int *)&(faultp->result.value.p))[1] == 0) &&
		((((int *)&(faultp->result.value.p))[2] & 0xfffffffa) == 0xa) ) {
		t_asr.z = 1;		/* set Z bit */
		t_psw.NZVC |= PS_Z;
	}
	break;
...
#if DSHLIB
extern const struct expval _flth_expval[4];
#else
static const struct expval _flth_expval[4] = {
	    { 0,		0,		0 }, /* ignore [0]   */
	    { 0 +              (16*K - 1) - (128 - 1),
		(256 - 2) +    (16*K - 1) - (128 - 1),
		3*(1<<6) },				/* single */
...

	...
	default:	/* should not happen!			*/
	    return;
	case FP_OVFLW:
	    if ( exp <= _flth_expval[tindex].maxexp )
	    ...
	    exp -= _flth_expval[tindex].bias;

	    if ( exp > _flth_expval[tindex].maxexp )	/* STILL out of range*/
		qnan_gen( drp, r0ptr );
	    else {
		drp->word0.postword0.sign = sign;
	...
	case FP_UFLW:
	    if ( exp > _flth_expval[tindex].minexp )
		/* UNDERFLOW with a large exponent?	*/
		return;

	    exp += _flth_expval[tindex].bias;

	    if ( exp < _flth_expval[tindex].minexp ) /* STILL out of range*/
		qnan_gen( drp, r0ptr );
	    else {
		drp->word0.postword0.sign = sign;
		drp->word0.postword0.exp  = exp & 0x7fff;
...

	...
	case FP_INXACT:
	     ...
	     */
	    if ( (exp < _flth_expval[tindex].minexp)||
		 ( (exp > _flth_expval[tindex].maxexp)&&
		   (exp != INFEXP) ) )
	 ...
	 */
	if ( tindex != 3 )
	    reformat( (int *)&(faultp->t_value.value), 3, tindex );
	return;
	}

    ...
    if ( tindex != 3 )
	reformat( (int *)&(faultp->t_value.value), tindex, 3 );
...
#ifdef __STDC__
    drp->word1 = 0xffffffffU;
    drp->word2 = 0xffffffffU;
#else

	.file	"dround.s"
.ident	"@(#)libc-m32:fp/dround.s	1.9"
...
.check_underflow:
	...
	extzv	&RC_OFFSET,&2,_dref_(_asr),%r0	# The result is -0 in RM mode.
	cmpw	%r0,&TO_MINUS
	jne	.plus_zero
	movw	&1,%r4
...
.set_tiny:				# if non zero, we have underflow
	...
	bitw	&UM_MASK,_dref_(_asr)	# if (underflow mask)
	je	.denorm_loop		#
...
#----------------------------------------------------------------------------
.round:
	extzv	&RC_OFFSET,&2,_dref_(_asr),%r0	# get  rounding mode
...
.check_inexact:
	...
	bitw	&UM_MASK,_dref_(_asr)		#	if (underflow enabled)
	jne	.pack_double		#		don't raise inexact

.check_inexact_trap:
	bitw	&IM_MASK,_dref_(_asr)		# is inexact trap enabled ?
...
.pack_plus2:
	...
	pushaw	_sref_(.format)		# push format for src etc.
...
#--------------------------------------------------------------------------
.overflow:
	orw2	&OVFLW_STICKY,_dref_(_asr)	# set overflow sticky
	bitw	&OM_MASK,_dref_(_asr)		# if (!overflow mask)
	je	.overflow_masked

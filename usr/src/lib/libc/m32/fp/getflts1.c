...
#ident	"@(#)libc-m32:fp/getflts1.c	1.9"
...
_getfltsw( ftype, optype, format, op1, op2, rres )
    ...
    const char *	format;	/* vector of formats	*/
    const fp_union *	op1;	/* input operand 1	*/
    const fp_union *	op2;	/* input operand 2	*/
    const fp_union *	rres;	/* rounded result	*/
    {
    struct fp_fault fault; /* _fpfault == &fault*/
    fp_union	retval;	/* value back to emul.	*/
    void (*	hand)();/* user's signal handler*/

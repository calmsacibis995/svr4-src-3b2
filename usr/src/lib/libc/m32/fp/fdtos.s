	.file	"fdtos.s"
	.ident	"@(#)libc-m32:fp/fdtos.s	1.6.2.4"
...
#---------------------------------------- denormal processing
	...
	bitw	&UM_MASK,_dref_(_asr)	# if (underflow trap==enabled)
	je	.no_u_trap
...
.test_inexact:
	...
	je	.pack			#	try to pack
	orw2	&INEXACT_STICKY,_dref_(_asr)	# set inexact sticky
#----------------------------------------
	extzv	&RC_OFFSET,&2,_dref_(_asr),%r5	# get rounding mode
	je	.to_near
	cmpw	%r5,&TO_ZERO
...
#--------------------------------------------------------------------------
	.text
.under:
	bitw	&UM_MASK,_dref_(_asr)	# if ( UM == 1 )
	...
	je	.ret6
	orw2	&UFLW_STICKY,_dref_(_asr)	# set underflow sticky
	jmp	.pack2
#--------------------------------------------------------------------------
.check_inxact_trap:
	bitw	&IM_MASK,_dref_(_asr)	# if (inexact trap==enabled)
	je	.ret6
					#	raise inexact trap
...
#--------------------------------------------------------------------------
.over:
	orw2	&OVFLW_STICKY,_dref_(_asr)
	bitw	&OM_MASK,_dref_(_asr)	# if ( OM == 1 )
...
.share:
	...
	pushaw	_sref_(.format)		# push format for source & result
	...
	call	&6,_fref_(_getfltsw)	# raise exception
...
#--------------------------------------------------------------------------
.tnan:
	orw2	&INVLD_STICKY,_dref_(_asr)	# set invalid sticky
	bitw	&INVLD_MASK,_dref_(_asr)	# if (invalid trap == enabled)
...
.invalid_trap:
	...
	pushaw	_sref_(.invalid_format)	# push format for operand, results
	pushaw	0(%ap)			# push address of operand 1
	pushw	&FP_NULL		# no second operand

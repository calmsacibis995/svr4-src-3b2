	.file	"sround.s"
.ident	"@(#)libc-m32:fp/sround.s	1.4"
...
#----------------------------------------
	extzv	&RC_OFFSET,&2,_dref_(_asr),%r1	# get rounding mode
	jne	.other_modes
.to_near:
	bitw	&2,%r7			# if ( R == 0 )
...
.under:
	...
	TSTW	%r7				# if ( ! inexact)
	je	.ret6				#	return
	orw2	&UFLW_STICKY,_dref_(_asr)	# else	set inexact flag
...
.over:
	...
	orw2	&INEXACT_STICKY,_dref_(_asr)	# if overflow trap is masked
						# set inexact sticky
	extzv	&RC_OFFSET,&2,_dref_(_asr),%r1	# get rounding mode
	je	.over_near
	cmpw	%r1,&TO_ZERO

	.file	"fdivd.s"
.ident	"@(#)libc-m32:fp/fdivd.s	1.10"
...
_fgdef_(_fdivd):
	MCOUNT
	TSTW	_dref_(_fp_hw)		# is hardware present
...
#-----------------------
	.text
.nan_inf:
	JSB	_fref_(_type.double)	# find the type of the operands
	movw	&FP_DIV,%r1
_m4_ifdef_(`DSHLIB',
`	cmpw	%r0,&2
	jl	_invalid.double
...
.div_zero:				# divide by zero exception
	orw2	&DIVZ_STICKY,_dref_(_asr)	# set divide by zero sticky
	bitw	&DIVZ_MASK,_dref_(_asr)		# the the mask
	...
	pushaw	_sref_(.format)		# format of operand etc.

...
#ident	"@(#)libc-port:gen/getsetitimer.c	1.1"
...
#define INT_CMD	 cmd[1]				/* interval timer cmd */
	...
	if (want_interval && timerisset(&(val->it_value))) {
		 ...
		 * the system clock.
                 */
		hrtnewres(&(INT_CMD.hrtc_int), HZ, HRT_RNDUP);
		hrtnewres(&(INT_CMD.hrtc_int), MICROSEC, HRT_RNDUP);
		hrtnewres(&(INT_CMD.hrtc_tod), HZ, HRT_RNDUP);
		hrtnewres(&(INT_CMD.hrtc_tod), MICROSEC, HRT_RNDUP);
	}
	...
	if (want_interval)
		...
		hrtnewres(&(REG_CMD.hrtc_int), HZ, HRT_RNDUP);
		hrtnewres(&(REG_CMD.hrtc_int), MICROSEC, HRT_RNDUP);
		cmdp = &REG_CMD;
	}

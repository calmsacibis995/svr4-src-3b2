...
short coffmagic = 0560;		/* magic number for cunix */
...
exhda_t *ehdp;
{
	...
	if (filhdrp->f_magic != M32MAGIC || !(filhdrp->f_flags & F_EXEC)) {
		error = ENOEXEC;
		goto bad;
	}

	if (((filhdrp->f_flags & F_BM32B) && !fast32b())
	  || (!(filhdrp->f_flags & F_BM32RST) && !fast32b())
	  || ((filhdrp->f_flags & F_BM32MAU) && !mau_present)) {
		error = EFAULT;
		goto bad;
	}
	...
	int sig;
{
	...
	*/
	...
	if (ctob(USIZE + up->u_dsize + up->u_ssize) > rlimit) {
		up->u_dsize = 0;
		if (ctob(USIZE + up->u_ssize) > rlimit)
			up->u_ssize = 0;
	}
	...
	error = vn_rdwr(UIO_WRITE, vp, (caddr_t)up, ctob(USIZE),

...
#ident	"@(#)kernel:vm/vm_swap.c	1.39"
 ...
 * Otherwise return the back pointer, which will either be
 * NULL, or an ap that is indirectly pointing to this ap.
	...
	rval_t *rvp;
{
	...
	case SC_LIST:
		 ...
		 * for thw whole table.  We also have to check
	...
	/* Allocate the space to read pathname in the kernel space */
	...
	char	*swapname;
{
	...
	if (error = VOP_GETATTR(cvp, &vattr, 0, u.u_cred))
		goto out;
	...
	register int	lowblk;	/* Low block number of area to delete. */
{
	...
	if ((anoninfo.ani_max - anoninfo.ani_resv) - osip->si_npgs
	    < tune.t_minasmem) {
	 ...
	 * swapadd may add this file back and turn off ST_INDEL.
	 ...
	 * We added back the pages in swapadd.
	 */

	if ((osip->si_flags & ST_INDEL) == 0) {
		undelswap(osip);
		anoninfo.ani_max += osip->si_npgs;
		availsmem += osip->si_npgs;
		return (0);
...
#define delswap_done(sip) ((sip->si_nfpgs == sip->si_npgs) || ((sip->si_flags & ST_INDEL) == 0))

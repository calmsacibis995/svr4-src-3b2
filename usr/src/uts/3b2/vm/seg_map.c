...
#ident	"@(#)kernel:vm/seg_map.c	1.26"
 ...
 */
void
segmap_pagecreate(seg, addr, len, softlock, dblist, bsize)
	struct seg *seg;
	register addr_t addr;
	u_int len;
	int softlock;
	...
	int bsize;
{
	...
	for (; addr < eaddr; addr += PAGESIZE, off += PAGESIZE) {
		...
		if (pp == NULL) {
			pp = rm_allocpage(segkmap, addr, PAGESIZE, P_CANWAIT);
			trace6(TR_SEG_ALLOCPAGE, segkmap, addr,
				TRC_SEG_SEGKMAP, vp, off, pp);
			if (page_enter(pp, vp, off))
				cmn_err(CE_PANIC, "segmap_page_create page_enter");
			page_unlock(pp);
			if (softlock) {
		...
		if (dblist != NULL) {
			...
			if (blkpp >= 1) {
				...
				pp->p_dblist[0] = *dbptr;
				if (((pp->p_offset + PAGESIZE) & (bsize - 1)) == 0)
					dbptr++;
			}
		}
	}
...
/*
 * segmap_inval -
 *	Step through freelist of segment for entries still on the
 *	hashlist.  If found, and if they belong to vfsp, invalidate
 *	the entries.
 */
void
segmap_inval(seg, vfsp)
	register struct seg *seg;
	register struct vfs *vfsp;
{
	register struct segmap_data *smd;
	register struct smap *smp, *nsmp;
	...
	do {
		nsmp = smp->sm_next;
		if (smp->sm_hash && smp->sm_vp &&
		    (smp->sm_vp->v_vfsp == vfsp)) {
			segmap_hashout(smd, smp);
			hat_unload(seg, seg->s_base + ((smp - smd->smd_sm) *
			    MAXBSIZE), MAXBSIZE, HAT_NOFLAGS);
		}
		smp = nsmp;
	} while (smp != smd->smd_free);
}

	...
	_VOID *argsp;
{
	 ...
	 * it is illegal to also specify a vp or a PRIVATE mapping.
	 */
	if (a->amp != NULL && (a->vp != NULL || a->type == MAP_PRIVATE))
		cmn_err(CE_PANIC, "segvn_create anon_map");

	/*
	 * If segment may need private pages,
	 * reserve them now.
	 */
	if (a->type == MAP_PRIVATE && (a->prot & PROT_WRITE)) {
	 ...
	 */
	error = hat_map(seg,
		(a->vp == NULL) ? NULL : a->vp->v_pages,
		a->offset & PAGEMASK,
		(a->type == MAP_PRIVATE) ? a->prot & ~PROT_WRITE : a->prot,
		HAT_PRELOAD|HAT_VNLIST);
	if (error != 0) {
		if (swresv != 0)
			anon_unresv(swresv);
		return(error);
	...
	if (a->vp != NULL) {
		VN_HOLD(a->vp);
	} else if (a->amp != NULL && a->type == MAP_SHARED) {
		a->amp->refcnt++;
	}
	...
	struct seg *seg, *newseg;
{
	...
	if ((newsvd->vp = svd->vp) != NULL) {
		...
		/*
		 * We have a shared mapping for which there is no vp and no
		 * anon map structure yet.  We need to allocate the anon
		 * map struct now so that it will be properly shared.
		 */
		...
		svd->amp->anon = (struct anon **)kmem_zalloc(npages *
		    sizeof (struct anon *), KM_SLEEP);
	}
	...
	if ((newsvd->amp = svd->amp) == NULL) {
		...
		if (svd->type == MAP_SHARED) {
			...
			newsvd->amp = (struct anon_map *)kmem_fast_alloc(
			    (caddr_t *)&anonmap_freelist,
			    sizeof (*anonmap_freelist), anonmap_freeincr,
			    KM_SLEEP);

			newsvd->amp->refcnt = 1;
			newsvd->amp->size = seg->s_size;
			newsvd->amp->anon = (struct anon **)kmem_alloc(
			    npages * sizeof (struct anon *), KM_SLEEP);
	...
	u_int len;
{
	...
	if (addr == seg->s_base) {
		...
		if (svd->amp != NULL && (svd->amp->refcnt == 1 ||
			...
			anon_free(app, len);
			bzero((caddr_t)app, dpages * sizeof (*app));
	...
	if (svd->vpage == NULL)
		...
		bcopy((caddr_t)&ovpage[opages - dpages],
	...
	struct seg *seg;
{
	...
	if (svd->amp != NULL) {
		...
		if (--svd->amp->refcnt == 0) {
			...
			bzero((caddr_t)app, npages * sizeof (*app));
		}
	}
 ...
 * to be the largest range which includes non-anoymous memory.
 */
STATIC int
...
/*
 * Currently ld.so is not setting protections up with execute
 * permissions on jump tables which are kept in ``data space''.
 * So for now, we skip checking for exec permissions by default.
 * This kludge will go away later after the runtime loader is fixed.
 */

STATIC int exectst = 0;
 ...
 *		If page might be needed
 *			Use anon_getpage to get the page
 *	else if page needed
 *		Lookup primary page if locks are involved
 *		Find page in pl[] list passed in
 *	endif
 *	If we have primary page
	...
	enum seg_rw rw;
{
	...
	int pagesteal = 0;
	page_t *anon_pl[1 + 1];
	page_t *opp;		/* original object page */
	u_int prot;
	int err;
	int readyforCOW;
	...
	if (svd->pageprot) {
		...
		case S_WRITE:
			...
			break;
		case S_EXEC:
			protchk = exectst ? PROT_EXEC : PROT_READ;
			break;
		case S_OTHER:
		default:
	...
	/*
	 * Set flag used to determine if things are already set up
	 */
	readyforCOW = (type == F_PROT && rw == S_WRITE &&
				svd->type == MAP_PRIVATE);

	/*
	 * This code works very hard at avoiding work.  In the case
	 * of faults on the private copy of an object (the result of
	 * a previous copy-on-write), an attempt to find its page
	 * structure is optimized away if it appears we never need
	 * the structure.  We don't need it if this is a simply copy-
	 * on-write of a page already in the hat -- we'll just use it's
	 * mapped address to implement the copy.  This is allowed iff:
	 *	It's a copy-on-write fault (page is already in memory);
	 *	The page is shared (and thus can not be stolen); and
	 *	The page is not locked in this mapping.
	 * Otherwise, anon_getpage must be called to obtain the page
	 * structure.
	 */
	if (app != NULL && *app != NULL) {
		if (readyforCOW && (*app)->an_refcnt > 1 &&
		    vpage != NULL && vpage->vp_pplock == 0) {
			/*
			 * Skip calling anon_getpage() - the
			 * page is already loaded in the hat
			 * layer with a read-only translation
			 * and won't be made writable.
			 */
			opp = pp = NULL;
		} else {
		...
		/*
		 * No need to bother getting original page unless
		 * a lock is involved.  And if it is, then we already
		 * know the page is in the hat layer, so just look it
		 * up.
		 */
		pp = NULL;
		if ((vpage != NULL) && vpage->vp_pplock) {
			if ((opp = page_lookup(svd->vp, off)) == NULL)
				cmn_err(CE_PANIC,"segvn_faultpage lookup failed");
		} else
			opp = NULL;
	} else {
		/*
	...
	if (pp != NULL) {
		...
		/*
		 * This is the primary page we need.
		 * Check to see if we really want to
		 * lock this page/translation or not.
		 */
		trace4(TR_PG_SEGVN_FLT, pp, pp->p_vnode, pp->p_offset, 0);
		pp->p_ref = 1;

		if (stealCOW && app != NULL && *app == NULL &&
		    freemem < minfree && pp->p_keepcnt == 1 &&
		    pp->p_mod == 0 && pp->p_mapping == NULL &&
		    pp->p_lckcnt == 0 && pp->p_cowcnt == 0)
			pagesteal = 1;

		noCOW = (((prot & vpprot) & PROT_WRITE) != 0 ||
		    (type != F_PROT && rw != S_WRITE) ||
		    svd->type == MAP_SHARED);

		if (type == F_SOFTLOCK && noCOW) {
			/*
			 * Load up the translation keeping it
			 * locked and don't PAGE_RELE the page.
			 */
			hat_memload(seg, addr, pp, prot & vpprot, 1);
		} else {
			hat_memload(seg, addr, pp, prot & vpprot, 0);
			PAGE_RELE(pp);
		}

		/*
		 * If not a copy on write case return now.
		 */
		if (noCOW)
			return (0);
	}
	...
	if (vpage != NULL && vpage_lock(&svd->lock, vpage) < 0) {
		/*
...
#ifdef sun386
	/*
	 * We need to lock down the translation to prevent another
	 * write fault while we are processing the copy-on-write.
	 */
	hat_lock(seg, addr);
	PAGE_HOLD(pp);
#endif
	 ...
	 * page.  If we have the original page structure, hold it to
	 * prevent it from being paged out; we can't tolerate a page
	 * fault during the copy because it may have been elicited by
	 * some other process (e.g. through a /proc operation).
	 *
	 * XXX -- if we don't have the original page structure, it is
	 * implicitly assumed that this is a copy-on-write fault (type
	 * F_PROT) elicited in the context of the currently-running
	 * process, and therefore that the associated segment address
	 * is mapped in.  This depends on the fact that if /proc is
	 * faulting it in, the type will be F_SOFTLOCK rather than
	 * F_PROT.
	 */
	 ...
	 */
	pp = anon_private(app, seg, addr, opp, pagesteal);
	 ...
	 * ``locked'' the page we map -- if so, transfer the locking resource
	 * to the new page.  In any case, remove the lock on the vpage itself.
	 */
	if (vpage != NULL) {
		if (vpage->vp_pplock)
	...
	enum seg_rw rw;
{
	...
	*plp = (page_t *)NULL;
	 ...
	 * was no original vnode or if this is write
	 * protection fault to a private mapping.
	 */
	if (svd->vp != NULL && !(type == F_PROT && rw == S_WRITE &&
	    svd->type == MAP_PRIVATE)) {
		u_int vp_off, vp_len;
		int dogetpage;
	...
	struct seg *seg;
{
	...
	for (page = 0; page < npages; page++) {
		...
		if ((pp = page_find(vp, off)) == NULL || pp->p_free)
			continue;
	...
	u_int flags;
{
	...
	} else if ((svd->amp == NULL) && (flags & MS_INVALIDATE) == 0) {

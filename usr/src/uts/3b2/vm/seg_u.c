...
#if defined(__STDC__)
STATIC u_int segu_swapout(struct proc *);
STATIC u_int segu_swapin(struct proc *);
STATIC int segu_getslot(struct seg*, addr_t, u_int);
#else
STATIC u_int segu_swapout();
STATIC u_int segu_swapin();
...
/*
 * Private seg op routines.  This set will flesh out more
 * as seg_u comes closer to being a real segment type.
 */
#ifdef notdef
STATIC	int segu_free(/* seg */);
STATIC	faultcode_t segu_faulta(/* seg, addr */);
STATIC	int segu_unload(/* seg, addr, ref, mod */);
#endif
...
STATIC int segu_softunload(struct seg *, addr_t, u_int, int, int);
#else
...
extern	struct seguser	*p0seguser;
	...
	caddr_t		argsp;
{
	 ...
	 */
	seg->s_data = kmem_alloc(sizeof (struct segu_segdata), KM_NOSLEEP);
	if (seg->s_data == NULL)
		return (ENOMEM);
	...
	/* reserve the 1st entry for process 0 */
	sdp->usd_free = sdp->usd_slots[0].su_next;
	sdp->usd_slots[0].su_flags = SEGU_ALLOCATED | SEGU_LOCKED;
	p0seguser = (struct seguser *)seg->s_base;
...
/*
 * Swap in the u-area designated by sup.  Return the number
 * of bytes brought back in.
 */
STATIC u_int
segu_swapin(p)
	register proc_t *p;
{
	register int		bytesin, i;
	register faultcode_t	fc;
	register pte_t 		*ppte, *upte;
	register sde_t		*sdeptr;
	caddr_t 		addr;

	if (segu_debug)
		cmn_err(CE_CONT, "segu_swapin(%x) [pid %d]\n", p, p->p_pid);

	bytesin = ptob(SEGU_PAGES);
	fc = segu_fault(segu, (addr_t)p->p_segu, bytesin, F_SOFTLOCK, S_OTHER);
	if (fc != 0) {
		cmn_err(CE_PANIC, "segu_fault() in segu_swapin() failed to swap in the u area");
	}

	/* 
	 * setup the pte's in proc structure 
	 */
	ppte = (pte_t *)ubptbl(p);
	addr = (caddr_t)p->p_segu;
	for (i=0; i < SEGU_PAGES; i++) {
		sdeptr = (sde_t *)kvtokstbl(addr);
		upte = (pte_t *)vatopte(addr, sdeptr);
		ppte->pg_pte = upte->pg_pte;
		ppte++;
		addr += PAGESIZE;
	}
		
	return(ptob(SEGU_PAGES));
}

/*
 * Swap out the u-area designated by p.  Return the number
 * of bytes swapped out.
 */
STATIC u_int
segu_swapout(p)
	register proc_t *p;
{
	...
	if (segu_debug)
		cmn_err(CE_CONT, "segu_swapout(%x) [pid %d]\n", p, p->p_pid);

	fc = segu_fault(segu, (addr_t)p->p_segu, ptob(SEGU_PAGES), F_SOFTUNLOCK, S_OTHER);
	if (fc != 0) {
		cmn_err(CE_PANIC, "seg_fault() in segu_swapout() failed to swap u area out");
	}
	
	/* 
	 * zeroed out the pte's in proc structure
	 */
	ppte = (pte_t *)ubptbl(p);
	for (i=0; i < SEGU_PAGES; i++) {
		ppte->pg_pte = 0;
	...
	register proc_t	*p;
{
	...
	/*
	 * If "freeswap" is zero, we don't free up the swap space;
	 * this is used by /proc so that a process u-block can be
	 * cheaply "swapped out" again if it had to be swapped in.
...
/*
 * Swap out a process u-block.  If the u-block has not actually been
 * modified, no disk I/O is necessary.  Return 0 if swap space must
 * be allocated and isn't available, 1 otherwise.
 */
int
swapoutub(p)
	register proc_t *p;
	...
	enum seg_rw	rw;
{
	...
	if (type == F_SOFTLOCK || type == F_SOFTUNLOCK) {
		if (vaddr != segu_stom(vbase) || len != ptob(SEGU_PAGES))
			return (FC_MAKE_ERR(EFAULT));
	}
	...
	if (type == F_INVAL) {
		/*
		 * Normal fault (e.g., by uaccess).  The processing required
	...
	register u_int len, *protv;
{
	u_int pgno = seg_page(seg, addr+len) - seg_page(seg, addr) + 1;
        ...
        if (pgno != 0) {
		do protv[--pgno] = (u_int)(PROT_READ|PROT_WRITE);
		while (pgno != 0);
	}
        return 0;
}
	...
	struct proc *cp;
{
	 ...
	 */
	if (anon_resv(ptob(SEGU_PAGES)) == 0) {
		if (segu_debug)
	 ...
	 */
	pp = rm_allocpage(segu, segu_stom(vbase), ptob(SEGU_PAGES), P_CANWAIT);
	if (pp == NULL) {
		if (segu_debug)
	...
	for (i = 0, va = vbase; i < SEGU_PAGES; i++, va += ptob(1)) {
		...
		if ((ap = anon_alloc()) == NULL)
			cmn_err(CE_PANIC, "segu_get: swap allocation failure");
		sup->su_swaddr[i] = ap;
	...
	sup->su_proc = cp;
	addr = (caddr_t)segu_stom(vbase);
	ptptr = (pte_t *)ubptbl(cp);
	for (j=0; j<USIZE; j++, ptptr++) {
		sdeptr = (sde_t *)kvtokstbl(addr);
		fuptr = vatopte(addr, sdeptr);
		ptptr->pg_pte = fuptr->pg_pte;
		addr += PAGESIZE;
	}
	...
	struct proc *p;
{
	...
	s = spl7();
	((sde_t *)dflt_sdt_p)->wd2.address = phys_ubptbl(nproc[0]->p_ubptbl);
	...
	if (sup->su_flags & SEGU_LOCKED) {
		sup->su_flags &= ~SEGU_LOCKED;
		hat_unload(segu, vaddr, ptob(SEGU_PAGES), HAT_UNLOCK|HAT_RELEPP);
	} else {
		hat_unload(segu, vaddr, ptob(SEGU_PAGES), HAT_NOFLAGS);
	}
...
struct segu_data *
segu_getentry(seg, vaddr)
	register struct seg	*seg;
	addr_t			vaddr;
{
	register int	 slot;
	struct segu_data *entry;

	ASSERT(seg == segu);
	slot = segu_getslot(seg, vaddr, ptob(SEGU_PAGES));
	entry = ((struct segu_segdata *)seg->s_data)->usd_slots;
	entry = entry + slot;
	return(entry);
}
	...
	int		locked;
{
	 ...
	 */
	if (locked) {
		hat_unload(seg, vaddr, len, HAT_UNLOCK|HAT_RELEPP);
	} else {
		hat_unload(seg, vaddr, len, HAT_RELEPP);
	}
	...
	ppte = (pte_t *)ubptbl(sup->su_proc);

	/*
	 * The code below is machine dependent. We have to sync
	 * the ref/mod bits of the page table contained in the
	 * proc structure back to the page structures. We could
	 * do the following computations:
	 *
	 * i = (vaddr - (seg->s_base + slot * ptob(SEGU_PAGES))) / ptob(1);
	 * swap_xlate(sup->su_swaddr[i], &vp, &off);
	 * if ((pp = page_find(vp, off)) == NULL)
	 * 	cmn_err(CE_PANIC, "segu_softunload: missing page");
	 *
	 * But, as long as we have the page frame numbers we may as well
	 * use them.
	 */
	for (i = 0; i < USIZE; i++, ppte++) {
		pp = page_numtopp(ppte->pgm.pg_pfn);
		pp->p_ref |= ppte->pgm.pg_ref;
		pp->p_mod |= ppte->pgm.pg_mod;

		if (pp->p_mapping == NULL && pp->p_keepcnt == 0 &&
		    !pp->p_lock && pp->p_mod) {
			/*
			...
			(void) VOP_PUTPAGE(pp->p_vnode, pp->p_offset, PAGESIZE,
				B_ASYNC | B_FREE, (struct ucred *)NULL);
		}
	}
	...
	u_int		lock;
{
	...
	for ( ; va < vlim; va += ptob(1), i++) {
		...
		if (err) {
			 ...
			 */
			(void) segu_softunload(seg, vaddr, (u_int)(va - vaddr),
						 slot, lock);
			return (err);
		}
		/*

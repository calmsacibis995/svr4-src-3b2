...
#endif
	ns = (struct nfsattrstat *)kmem_alloc(sizeof (*ns), KM_SLEEP);
	/*
	...
	struct cred *cred;
{
	...
	rlock(vtor(tdvp));
	/* LINTED pointer alignment */
	error = rfscall(vtomi(svp), RFS_LINK, xdr_linkargs, (caddr_t)&args,
...
again:
	if (len == 0) {
		 ...
		 */
		if ((rp->r_flags & RLOCKED) && curproc->p_pid == 2) {
...
printf("nfs_putpages: searching list for dirty pages\n");
		...
		if (dirty == NULL && off == 0 && (flags & B_ASYNC) == 0) {
			 ...
			 */
#ifdef NFSDEBUG
printf("nfs_putpages: no dirty pages\n");
#endif
			rp->r_flags &= ~RDIRTY;
		}
#ifdef NFSDEBUG
printf("nfs_putpages: calling runlock\n");
#endif
		runlock(rp);
	} else {
		/*

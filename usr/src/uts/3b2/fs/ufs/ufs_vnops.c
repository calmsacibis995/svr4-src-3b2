...
out:
	if (!(flags & ATTR_EXEC)) {
		ip->i_flag |= ISYNC;
		ufs_iupdat(ip, 1);	/* XXX - should be async for performance */
	}
	IUNLOCK(ip);
	IRWUNLOCK(ip);
	return (error);
	...
	struct cred *cr;
{
	 ...
	 * either own the file, or own the directory, or is the
	 * super-user.
	 */

	if ((sdp->i_mode & ISVTX) && cr->cr_uid != 0 &&
	    cr->cr_uid != sdp->i_uid && sip->i_uid != cr->cr_uid) {
		error = EPERM;
		goto out;
	}
	...
	struct cred *cr;
{
	...
	/*
	 * We may be getting called as a side effect of a ufs_bmap using
	 ...
	 * calling ufs_bmap for a non write case here.  We also might have
	 * to read some frags from the disk into a page if we are
	 * extending the number of frags for a given lbn in ufs_bmap().
	 */

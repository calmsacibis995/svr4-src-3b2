	...
	struct cred *cr;		/* user credentials */
{
	...
	if (tip) {
		...
		if (err = ufs_diraddentry(tdp, namep, namlen, &slot, sip, sdp)) {
			if (op == DE_CREATE || op == DE_MKDIR) {
				/*
	...
	struct cred *cr;		/* credentials */
{
	...
	if ((tip->i_mode & IFMT) == IFDIR) {
		...
		if (ITOV(tip)->v_vfsmountedhere)
			return (EBUSY);
		if ((error = ufs_iaccess(tip, IWRITE, cr)) != 0
		  || (error = ufs_iaccess(sip, IWRITE, cr)) != 0)
			return error;
		if (!ufs_dirempty(tip, tdp->i_number) || tip->i_nlink > 2)
 ...
 */
STATIC int
ufs_diraddentry(tdp, namep, namlen, slotp, sip, sdp)
	struct inode *tdp;
	char *namep;
	int namlen;
	struct slot *slotp;
	...
	struct cred *cr;
{
	...
	if (op == DR_RMDIR) {
		...
		if (dp == ip || ITOV(ip) == cdir)
			...
			error = ENOTDIR;
		else if ((ip->i_nlink != 2) || !ufs_dirempty(ip, dp->i_number))
		{

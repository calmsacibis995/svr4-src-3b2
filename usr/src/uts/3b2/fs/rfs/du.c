	...
	cred_t			*crp;		/* credentials structure */
{
	...
	if (!error && !(error = RF_RESP(bp)->rp_errno)) {
		/* Eitherdusys_mknod completed successfully or
	...
	cred_t 			*crp;		/* credentials structure */
{
	...
	for (ntries = nacked = 1; ntries < RFCL_MAXTRIES && nacked; ntries++) {
		...
		rfcl_reqsetup(bp, chansdp, crp, DUUTIME, ULIMIT);
		reqp = RF_REQ(bp);
	...
	cred_t			*crp;
{
	...
	for (ntries = nacked = 1; ntries < RFCL_MAXTRIES && nacked; ntries++) {
		...
		rfcl_reqsetup(bp, chansdp, crp, RFFCNTL, ULIMIT);
		reqp = RF_REQ(bp);
	...
	if (!error && RF_MSG(bp)->m_stat & RF_VER1 &&
	  ...
	  VREG && rp->rp_rval == F_FREESP) {
		...
		if (rp->rp_fcntl.isize < chansdp->sd_size) {
			 ...
			 */
			rfc_pageabort(chansdp, (off_t)(rp->rp_fcntl.isize - 1),
			  (off_t)0);
		}
	...
	register struct statfs	*csbp;	/* pass client pointer back */
{
	...
	if (!(error = VFS_STATVFS(vfsp, &statvfs))) {
		...
		sbp->f_frsize = (statvfs.f_frsize == statvfs.f_bsize) ?
		  0 : statvfs.f_frsize;
		sbp->f_blocks = statvfs.f_blocks * (statvfs.f_frsize / 512);
	...
	mblk_t		**bpp;
{
	...
	*bpp = NULL;
	...
	for (ntries = nacked = 1; ntries < RFCL_MAXTRIES && nacked; ntries++) {
		...
		rfcl_reqsetup(*bpp, chansdp, crp, opcode, ULIMIT);
		rqp = RF_REQ(*bpp);
		rqp->rq_arg = *rqargp;

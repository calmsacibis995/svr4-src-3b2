...
/*
 * TO DO: these should all be imported via header files, the last two in
 * rf_comm.h.
 */
extern time_t	lbolt;
	...
	size_t			nbytes;
{
	...
	while (!error && nbytes) {
		...
		if (pp) {
			...
			PAGE_RELE(pp);
			if (error) {
				rfc_pageabort(VTOSD(vp), (off_t)pp->p_offset,
				  (off_t)pp->p_offset);
			}
		} else {
		...
		if (rdup->ru_cflag & RU_CACHE_ON &&
		  (!RF_SERVER() || !rdu_match(rdup, u.u_procp->p_sysid,
		   u.u_srchan->sd_mntid))) {
			...
			rfc_dismsout(rdup, RDTOV(vrdp), sdp, rdp, vcode);
		}
	}
	rcvd_free(&rdp);
	sndd_free(&sdp);
		...
		if (sdp->sd_stat & SDCACHE) {
			if (sdp->sd_fhandle != fhandle) {
				...
				if (!isgift && size < sdp->sd_size) {
					...
					rfc_pageabort(sdp, (off_t)(size - 1),
					  (off_t)0);
				}
 ...
 *
 * NOTE:  depends on the fact that pages on v_pages are sorted in
	...
	register off_t		endoff;
{
	...
	while (vp->v_pages &&
	  pp->p_offset >=  startoff && (!endoff || pp->p_offset < endoff) &&
	  (nextpp = pp->p_vpnext) != vp->v_pages) {
		rfc_dopageabort(pp);
		pp = nextpp;
	}
	/*
	 * Here's where we deal with singleton lists and with pages
	 * that have migrated to the head of the list because of updates
	 * in page_hashout().
	 */
	if (pp && pp->p_offset >=  startoff &&
	  (!endoff || pp->p_offset < endoff)) {
		rfc_dopageabort(pp);
	}
}
	...
	off_t		*offp;
{
	...
	ASSERT(!pp ||
	  pp->p_intrans && pp->p_intrans && pp->p_lock &&
...
/*
 * Pages not held have their vnode associations destroyed and are aborted.
 * Others are marked gone; they will be aborted when the last references to
 * them go away.
 */
STATIC void
rfc_dopageabort(pp)

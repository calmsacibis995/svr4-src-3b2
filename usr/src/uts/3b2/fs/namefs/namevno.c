	...
	struct cred *crp;
{
	 ...
	 * If it was, close and return error.
	 */
	if (setjmp(&u.u_qsav)) {
		(void) nm_close(*vpp, flag & FMASK, 1, (off_t) 0, crp);
		return (EINTR);
	}
	...
	struct cred *crp;
{
	 ...
	 */
	if (crp->cr_uid != nmvap->va_uid && crp->cr_uid != 0) {
		error = EPERM;
		goto out;
	}
	/*
	...
	if (mask & (AT_ATIME|AT_MTIME)) {
		...
		if (chtime) {
			nmvap->va_ctime.tv_sec = hrestime.tv_sec;
			nmvap->va_ctime.tv_nsec = 0;
		}
	}
out:
	nm_rwunlock(vp);
	...
	struct vnode *vp;
{
	...
	while (nodep->nm_flag & NMLOCK) {
		nodep->nm_flag |= NMWANT;
		(void) sleep((caddr_t) nodep, PINOD);
	}

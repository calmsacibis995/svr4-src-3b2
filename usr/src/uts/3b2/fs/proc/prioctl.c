...
#ident	"@(#)fs:fs/proc/prioctl.c	1.28"
#include "sys/types.h"
#include "sys/param.h"
	...
	int *rvalp;
{
	...
	if (cmd != PIOCSTOP && cmd != PIOCWSTOP) {
		...
		/*
		 * Identify ioctls that don't need deadlock prevention.
		 * The operation is faster without it.
		 */
		switch (cmd) {
		case PIOCPSINFO:
			force = 1;
	...
	case PIOCGETU:		/* read struct user */
		up = prumap(p);
		if (copyout((caddr_t) up, cmaddr, ctob(USIZE)))
			error = EFAULT;
		prunmap(p);
		break;
		...
		if (error == 0) {
			if (pnp->pr_proc == NULL || p->p_stat == SZOMB)
				...
				if (pnp->pr_flags & PRINVAL)
					...
					prgetstatus(p, &prstat);
					if (copyout((caddr_t) &prstat, cmaddr,
					  sizeof(prstat)))
						error = EFAULT;
				}
				prunlock(pnp);
	...
	case PIOCSTRACE:	/* set signal trace mask */
	{
		...
		if (copyin(cmaddr, (caddr_t) &smask, sizeof(smask)))
			...
			prdelset(&smask, SIGKILL);
			prassignset(&p->p_sigmask, &smask);
	...
	case PIOCGFPREG:	/* get floating-point registers */
	{
		...
		if (prhasfp()) {
			prgetfpregs(p, &fpregs);
			if (copyout((caddr_t) &fpregs, cmaddr, sizeof(fpregs)))
				error = EFAULT;
		} else
			error = EINVAL;	/* No FP hardware */
		break;
	}
	...
	register struct prrun *prp;
{
	...
	if (flags & (PRSABORT|PRSTEP|PRSVADDR)) {
		umapped++;
		up = prumap(p);
	}

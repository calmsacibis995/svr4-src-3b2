	...
	register struct cred *cr;
{
	...
	if (mask & AT_SIZE) {
		...
		if (error = bfs_access(vp, VWRITE, 0, cr))
			goto out;
	...
	if (mask & (AT_ATIME|AT_MTIME)) {
		...
		battrs->va_mtime = hrestime.tv_sec;
		battrs->va_atime = hrestime.tv_sec;
	}
out:
	...
	struct cred *cr;
{
	...
	if (error = bfs_access(sdvp, VWRITE, 0, cr))
		return error;
	...
	struct cred *cr;
{
	...
	if (dir_off) {
		if (excl == NONEXCL) {
			 ...
			 */
			if (mode &&
			    (error = bfs_access(avp, mode, 0, cr)) == 0) {
	 ...
	 */
	bcopy(fname, name, BFS_MAXFNLEN);

	name[BFS_MAXFNLEN] = '\0';

	if (error = bfs_access(dvp, VWRITE, 0, cr))
		return error;
	 ...
	 */
	error = bfs_addirent(dvp->v_vfsp, name, dir->d_ino, cr);
	if (error) {
		kmem_free(dir, sizeof(struct bfs_dirent));
		return(error);
	}

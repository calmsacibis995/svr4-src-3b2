	...
	struct cred *cr;
{
	...
	if (ino == 0  &&  offset > 0) {
		 ...
		 * Null-pad name.
		 */
		if (strlen(nm) < BFS_MAXFNLEN)
			for (i = strlen(nm); i <= BFS_MAXFNLEN; i++)
				ld.l_name[i] = '\0';
		ld.l_ino = inode;
	...
	struct cred *cr;
{
	...
	if (ino > 0  &&  offset > 0) {
		ld.l_ino = ino;
		strcpy(ld.l_name, tnm);
		/*
		 * Null-pad name.
		 */
		if (strlen(tnm) < BFS_MAXFNLEN)
			for (i = strlen(tnm); i <= BFS_MAXFNLEN; i++)
				ld.l_name[i] = '\0';
		error = vn_rdwr(UIO_WRITE, bs->bsup_devnode, (caddr_t)&ld,
				sizeof(struct bfs_ldirs), offset, UIO_SYSSPACE,
				IO_SYNC, BFS_ULT, cr, (int *)0);
	...
	off_t *offp;
{
	...
	for (offset = (dir->d_sblock * BFS_BSIZE); len > 0 && found == 0;
		...
		for (i = 0; i < buflen; i += sizeof(struct bfs_ldirs)) {
			...
			if (ld->l_ino != 0 && strcmp(ld->l_name, nm) == 0) {
				*inop = ld->l_ino;
				found++;
				if (offp)
					*offp = offset + i;

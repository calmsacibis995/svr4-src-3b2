...
#ifdef SVR32
int ldtermopen(/*queue_t *q, int dev, int oflag, int sflag*/);
int ldtermclose(/*queue_t *q*/);
#endif /* SVR32 */
#ifdef SVR40
...
void chgstropts(/*struct termios *oldmodep, struct ldtermstd_state_t *tp,
    queue_t *q*/);
...
static struct termios initmodes = {
	BRKINT|ICRNL|IXON|ISTRIP|IXANY,	/* iflag */
...
#endif /* SVR40 */
{
	...
	if ((bp = allocb((int)sizeof (ldtermstd_state_t), BPRI_MED)) == NULL) {
		cmn_err(CE_WARN, "ldtermopen: open fails, can't allocate state structure\n");
		return (OPENFAIL);
	}
	...
	while ((bp = allocb((int)sizeof (struct stroptions), BPRI_MED)) == NULL) {
		...
		(void) sleep((caddr_t)&q->q_ptr, STIPRI);
		(void) splx(s);
	}
	...
	mblk_t *mp;
{
	...
	case M_BREAK:
		/*
		 * This comes up only if a framing error with a
		 * zero byte came in, and IGNBRK was not set.
		 */
...
M_CTL message to SXT driver */
						...
						swb = allocb ( sizeof( struct iocblk), BPRI_HI);
...
escaped:
	 ...
	 */
	if (tp->t_msglen >= (MAX_CANON - (int)tp->t_maxeuc)) {
		/*
	...
	register queue_t *q;
{
	...
	if (!(tp->t_state & TS_RTO )) {
		...
		timeout (ldterm_vmin_timeout, (caddr_t)q, (int)(V_TIME * (HZ / 10)));
		return;
	}

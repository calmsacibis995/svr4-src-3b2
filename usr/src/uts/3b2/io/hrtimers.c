...
#ident	"@(#)kernel:io/hrtimers.c	1.24"
...
psw_t		ps;
{
	...
	*/
	...
	if (--unix_tick == 0) {
		...
		return(clock(pc, ps));		/* Give Unix a tick */
	}
...
/*	Start the clock.  The high resolution timing services and
**	periodic UNIX clock are provided from this one clock.
...
clkstart()
{
	register uint		interval;
	register uint		lsb;
	register uint		msb;
	register int		oldpri;

	/*	Initialize some globals based on the resolution.
	**	The variable "tick" is used in hrt_alarm for the
	**	HRT_RALARM case.
	*/
	...
	*/
	...
	interval = MAX_RESOLUTION / timer_resolution;
	lsb      = interval & 0xFF;
	msb      = (interval & 0xFF00) >> 8;

	(void)sitcmd(ITINIT, lsb, msb);

	splx(oldpri);
}

/*	SIT (System Interval Timer) communication routine.
*/
...
register unsigned char	msb;	/* Most significant byte.	*/
{
	...
	/*	Decode the counter select field of the command.
	*/
	...
	/*	Decode the read/load select field of the command.
	*/

...
static struct qinit sprinit ={ NULL, NULL, spopen, spclose, NULL, &spm_info, NULL };
static struct qinit spwinit ={ spput, NULL, NULL, NULL, NULL, &spm_info, NULL};
...
register int dev;
{
	...
	case CLONEOPEN:
		for (dev=0, spp = sp_sp; ((dev<spcnt) && spp->sp_rdq); dev++, spp++);
		break;
	}
	if ((dev < 0) || (dev >= spcnt)) 
		return(OPENFAIL);

	...
	case M_PCPROTO:
		...
		if (tp->tim_flags & WAITIOCACK) {
			...
			if (ackp->ADDR_length > tp->tim_mymaxlen) {
			    ...
			    if (p == NULL) {
				putbq(q, mp);
				(void) timeout(qenable, (caddr_t)q, TIMWAIT);
				return (0);
			    }
			    kmem_free(tp->tim_myname, tp->tim_mymaxlen);
			    tp->tim_myname = p;
...
out:
		...
		if (tp->tim_peermaxlen != 0) {
			...
			putbq(q, mp);
			if (!bufcall((uint) sizeof(mblk_t), BPRI_MED, qenable,
				(caddr_t)q))
					(void) timeout(qenable, (caddr_t)q, TIMWAIT);
			return (0);
		    }
...
wgetnext:
	...
	case M_IOCTL:
		...
		if ((tmp = copymsg(mp->b_cont)) == NULL) {
		    ...
		    if (!bufcall(i, BPRI_MED, qenable, (caddr_t)q))
			 (void) timeout(qenable, (caddr_t)q, TIMWAIT);
		    return (0);
		}
		tp->tim_iocsave = mp;
...
getname:
		if ((tmp = copymsg(mp)) == NULL) {
		    ...
		    for (tmp = mp; tmp; tmp = tmp->b_next)
			i += (int)(tmp->b_wptr - tmp->b_rptr);
		    if (!bufcall((uint) i, BPRI_MED, qenable, (caddr_t)q))
			 (void) timeout(qenable, (caddr_t)q, TIMWAIT);
		    return (0);
		}
		tp->tim_iocsave = mp;
	...
	register mblk_t *mp;
{
	...
	if (mp->b_datap->db_type == M_DATA) {
		...
		if (up->DEST_length != 0)
			return (mp);
		bp = allocb((bp->b_wptr - bp->b_rptr) + tp->tim_peerlen,
		    BPRI_MED);
		if (bp == NULL)
			return(NULL);
		bp->b_datap->db_type = M_PROTO;

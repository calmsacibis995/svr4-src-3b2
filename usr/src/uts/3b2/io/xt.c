...
/* The name nxt means "new" xt or, in other words, streams xt rather than
 * the old pre-SVR4 character xt driver. In other words it is historical
 * (and harmless).
 */
 ...
 * For release SVR3.2 we do the old style of setting up
 * controlling terminal and of SVR4.0 we send an 
 * M_SETOPTS upstream with SO_ISTTY flag set
 *
 * RETURNS: none
 */
STATIC int
...
#ifdef SVR32
	if ((u.u_procp->p_pid == u.u_procp->p_pgrp) && (u.u_ttyp == NULL) && (chanp->xt_pgrp == 0)) {
		u.u_ttyp = &chanp->xt_pgrp;
		chanp->xt_pgrp = u.u_procp->p_pgrp;
	}
#endif /* SVR32 */
...
mblk_t *bp;
{
	...
	if (GET_CNTL(header) ) {			  /* CONTROL MESSAGE */
		...
		*/
		lastseq1 = (tchanp->xt_inseq + (SEQMOD-1)) % SEQMOD;
		lastseq2 = (tchanp->xt_inseq + (SEQMOD-2)) % SEQMOD;
...
struct xt_chan *chanp;
{
	...
	case M_IOCDATA:
		...
		if( xtioccont(bp, chanp) ) {
			return(1);
		}
		else {
			/* Blocked by flow control. Try again later */
			putbq(tq, bp);
			return(0);
		}
...
unsigned int m;
{
	...
	*to_bp->b_wptr = 0x40 | (chanp->xt_channo & 0x7);
	...
	if( !(chanp->xt_chflg&XT_NONETFLOW) ) {
...
mblk_t *bp;
{
	...
	*/
	if( ctlp->xt_maxpkt == 1 &&
	   ( (chanp->xt_chflg&XT_NONETFLOW)
	      || size + chanp->xt_bytesent <= NETXT_HIWAT
	      || (chanp->xt_chflg&XT_HOLDFLOW) ) ) {

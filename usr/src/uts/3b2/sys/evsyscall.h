...
#ident	"@(#)head.sys:sys/evsyscall.h	1.6"
/*			File Contents
**			=============
**
...
typedef struct evsys_poll {
	...
	const hrtime_t	*evs_poll_top;	/* Ptr to the hrtime_t	*/
...
typedef struct evsys_trap {
	...
	const evta_t	*evs_trap_tap;	/* The optional trap	*/
					/* argument.		*/
} evsys_trap_t;

...
typedef struct evsys_exit {
	const procset_t	*evs_exit_psp;	/* Ptr to the process	*/
					/* set structure which	*/
					/* defines the set of	*/
					/* processes which are	*/
					/* to be operated on.	*/
	...
	const ecb_t	*evs_exit_ecbp;	/* Ptr to the ecb for	*/
					/* the event to be	*/
					/* posted.		*/
} evsys_exit_t;
...
typedef struct evsys_sig {
	const sigset_t	*evs_sig_setp;	/* Ptr to the set of	*/
					/* signals to which 	*/
					/* this call applies.	*/

	const ecb_t	*evs_sig_ecbp;	/* Ptr to the ecb for	*/
					/* the event to be 	*/
					/* posted.		*/

	...
	register struct route *ro;
{
	...
	if (!(flags & SSF_SWITCH)) {
		...
		default:
#ifdef SYSV
			cmn_err(CE_CONT, "rtswitch: bad state %x\n",
				rt->rt_flags);
#else
	...
	register struct rtentry *rt;
{
	...
	default:
#ifdef SYSV
		cmn_err(CE_CONT, "rtunswitch: bad state %x\n", rt->rt_flags);
#else
	...
	register struct in_addr *addr;
{
	...
	if (Slnumqueued >= Slmaxqueued) {
		splx(s);
#ifdef SYSV
		cmn_err(CE_CONT, "switched slip: dropping request\n");
#else
...
#ifdef SYSV
		cmn_err(CE_CONT, "switched slip: dropping deferral\n");
#else
...
caddr_t         arg;
{
	...
	if (Slnumevents >= Slmaxevents) {
		splx(s);
#ifdef SYSV
		cmn_err(CE_CONT, "switched slip: dropping timeout\n");
#else

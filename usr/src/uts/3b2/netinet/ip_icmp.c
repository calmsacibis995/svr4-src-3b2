...
deliver:
		...
		genctlmsg(code, icmpsrc, (int) icp->icmp_ip.ip_p);
		break;
...
reflect:
	...
	case ICMP_REDIRECT:
		...
		if (code == ICMP_REDIRECT_NET || code == ICMP_REDIRECT_TOSNET) {
			...
			icmpsrc = icp->icmp_ip.ip_dst;
			genctlmsg(PRC_REDIRECT_NET, icmpsrc, -1);
		} else {
			icmpsrc = icp->icmp_ip.ip_dst;
			rtredirect(icmpsrc, icmpdst, RTF_GATEWAY | RTF_HOST,
				   icmpgw);
			genctlmsg(PRC_REDIRECT_HOST, icmpsrc, -1);
		}
		break;
...
raw:
	...
	for (inp = icmb.inp_next; inp != &icmb; inp = inp->inp_next) {
		if (inp->inp_q && (inp->inp_state & SS_CANTRCVMORE) == 0
		    && canput(inp->inp_q->q_next) && (bp = dupmsg(Obp))) {
	...
	struct ip_provider *prov;
{
	...
	if (optlen > 0) {
		 ...
		 * strip out other options.  Adjust the IP length.
		 */
		opts = ip_srcroute();
		ip->ip_len -= optlen;
...
struct inpcb *inp;
{
	...
	if (!inp->inp_q)
		return;
	if (!(mp = allocb(sizeof(struct T_uderror_ind)+sizeof(*sin), BPRI_HI)))
		return;
	mp->b_wptr += sizeof(struct T_uderror_ind) + sizeof(*sin);
	mp->b_datap->db_type = M_PROTO;
	uderr = (struct T_uderror_ind *) mp->b_rptr;
	uderr->PRIM_type = T_UDERROR_IND;
	uderr->DEST_length = sizeof(*sin);
...
mblk_t *bp;
{
	...
	in_pcbnotify(&icmb, &sin, uderr->ERROR_type, icmp_snduderr);
}

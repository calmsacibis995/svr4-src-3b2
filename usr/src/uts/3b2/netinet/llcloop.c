...
#ident	"@(#)netinet:netinet/llcloop.c	1.4"
	...
	mblk_t		*bp;
{
	...
	case DL_UNITDATA_REQ:
		...
		freeb(bp);
		qreply(q, hdr);
		loopstats.ifs_opackets++;
		loopstats.ifs_ipackets++;
		return;		/* Avoid OK_ACK */
...
loopinit()
{
	...
	bzero((char *) &loopstats, sizeof(loopstats));
	loopstats.ifs_name = "lo";
	loopstats.ifs_active = 1;	/* Always up! */
	loopstats.ifs_next = ifstats;
	loopstats.ifs_mtu = loopm_info[IQP_WQ].mi_maxpsz;
	ifstats = &loopstats;
	loopinited = 1;
}

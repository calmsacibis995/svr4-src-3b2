 ...
 * consists of two streams drivers, arp and arpproc.  Arp implements the
 * Address Resolution Protocol itself using broadcasts and direct responses
 * to talk to other ARP implementations on the local net. arpproc functions
 ...
 * arpproc so that programs based on the sockets paradigm can manipulate
 * these tables through the higher level. 
 *
 * On initialization an arp module gets an ethernet interface linked below it.
 * The slink process should also have linked a corresponding arpproc process
...
char           *ether_sprintf();
static int	memcmp();
#ifdef	STRINGS	/* that is here, we didn't build with STREAMS */
char	*tcp_strcpy();
int	 tcp_strcmp();
#define	strcpy	tcp_strcpy
#define strcmp	tcp_strcmp
#endif	/* STRINGS */

#define N_ARP	10		/* number of available arp units */
...
int             arptimerid;
static int      arpinited;

/*
 * The first section implements the stream interface to arpproc 
 */

int             appopen(), appclose(), appwput(), apprput();

static struct module_info appm_info[MODL_INFO_SZ] = {
	APPM_ID, "arpproc", 0, 8192, 8192, 1024,
	APPM_ID, "arpproc", 0, 8192, 8192, 1024
};
...
static struct module_info arpm_info[MUXDRVR_INFO_SZ] = {
	ARPM_ID, "arpproc", 0, 8192, 8192, 1024,
	ARPM_ID, "arpproc", 0, 8192, 8192, 1024,
	ARPM_ID, "arpproc", 0, 8192, 8192, 1024,
	ARPM_ID, "arpproc", 0, 8192, 8192, 1024,
	ARPM_ID, "arpproc", 0, 8192, 8192, 1024
...
struct arp_pcb {
	...
	struct app_pcb *app_pcb;/* cross pointer to arpproc module */
...
/*
 * These are the stream module routines for ARP processing 
 */
	...
	mblk_t         *bp;
{
	...
	default:
		STRLOG(APPM_ID, 0, 5, SL_ERROR,
		       "arpproc: unexpected type received in wput: %d.\n",
		       bp->b_datap->db_type);
		freemsg(bp);
		break;
	}
	...
	mblk_t         *bp;
{
	...
	if (msgblen(bp) >= sizeof (struct iocblk_in)) {
		/*
		 * XXX - It probably came from from IP.  Pass our
		 * flags back up.
		 */
	...
	mblk_t		*bp;
{
	...
	case DL_UNITDATA_REQ:
		...
		if (arpresolve(ap, bp, (char *) &enaddr, &temp)
		    == 0) {
			break;
		}
	...
	queue_t        *q;
{
	...
	if (arp_pcb[dev].arp_qtop == NULL) {
		arp_pcb[dev].arp_qtop = q;
		arp_pcb[dev].arp_qbot = NULL;
		q->q_ptr = (char *) &arp_pcb[dev];
		WR(q)->q_ptr = (char *) &arp_pcb[dev];
	} else if (q != arp_pcb[dev].arp_qtop) {
		return (OPENFAIL);
	}
	/*
	...
	mblk_t         *bp;
{
	...
	if (!memcmp(&null_etheraddr, &ac->ac_enaddr, sizeof(null_etheraddr))) {
		STRLOG(IPM_ID, 0, 9, SL_ERROR, "null ethernet address, arp %x", ar);
		if (ar->arp_saved) {

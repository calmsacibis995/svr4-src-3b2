...
#define strlen	tcp_strlen
#define strncmp tcp_strncmp
#define strncpy	tcp_strncpy
	...
	mblk_t         *bp;
{
	...
	case SIOCSIFDSTADDR:
		...
		if (prov->ia.ia_flags & IFA_ROUTE) {
			rtinit(prov->if_dstaddr, *PROV_INADDR(prov),
	...
	mblk_t         *bp;
{
	...
	if (prov->if_flags & IFF_LOOPBACK)
		rtinit(*PROV_INADDR(prov), *PROV_INADDR(prov), (int) SIOCADDRT,
		       RTF_HOST | RTF_UP);
	else if (prov->if_flags & IFF_POINTOPOINT)
		rtinit(prov->if_dstaddr, *PROV_INADDR(prov), (int) SIOCADDRT,
		       RTF_HOST | RTF_UP);
	else {
		netaddr = in_makeaddr(prov->ia.ia_subnet, INADDR_ANY);
		rtinit(netaddr, *PROV_INADDR(prov), (int) SIOCADDRT, RTF_UP);
	}
	prov->ia.ia_flags |= IFA_ROUTE;
	return;
}

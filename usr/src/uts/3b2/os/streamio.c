	...
	cred_t *crp;
{
	 ...
	 * read queue, null out the other write queue.
	 */
	if (qp->q_next && !SAMESTR(qp))
		WR(qp->q_next)->q_next = NULL;
	...
	int *rvalp;
{
	...
	case I_PEEK:
		...
		while (bp && bp->b_datap->db_type != M_DATA && uio.uio_resid >= 0) {
			if ((n = MIN(uio.uio_resid, bp->b_wptr - bp->b_rptr)) &&
			    (error = uiomove((caddr_t)bp->b_rptr, n, UIO_READ, &uio)))
				return (error);
			bp = bp->b_cont;
		}
		...
		while (bp && uio.uio_resid) {
			if ((n = MIN(uio.uio_resid, bp->b_wptr - bp->b_rptr)) &&
			    (error = uiomove((char *)bp->b_rptr, n, UIO_READ, &uio)))
				return (error);
			bp = bp->b_cont;
		}
	...
	case I_SWROPT: 
		...
		stp->sd_flag |= ((arg & SNDZERO) ? STRSNDZERO : 0);
		splx(s);
		return (0);
	...
	case I_GWROPT:
		...
		wropt = ((stp->sd_flag & STRSNDZERO) ? SNDZERO : 0);
	...
	case I_ATMARK:
		...
		if (!(q->q_first))
			*rvalp = 0;
		else if ((arg == ANYMARK) && (q->q_first->b_flag & MSGMARK))
			*rvalp = 1;
		else if ((arg == LASTMARK) && (q->q_first == stp->sd_mark))
			*rvalp = 1;
		else
			*rvalp = 0;
		splx(s);
	...
	struct pollhead **phpp;
{
	...
	if ((events & POLLPRI) && (stp->sd_flag & STRPRI)) {
		ASSERT(mp != NULL);
		retevents |= POLLPRI;
	}
	*reventsp = retevents;

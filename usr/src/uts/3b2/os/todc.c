...
#ifdef __STDC__
STATIC int getseg(struct clkseg *, short *);
#else
STATIC int getseg();
#endif
...
STATIC int
getseg(seg,dest)
struct clkseg *seg;  /* a pointer to the Clock segment to read */
...
wtodc()
{
	...
	if (day >= 0)
		...
		for (d1=70; day < 0; d1--)
			day += dysize(d1-1);
	SBDTOD->years = 1<< ((d1 - 1) % 4);
	nv_data[1] = (char)d1;	/* year to write to nvram */

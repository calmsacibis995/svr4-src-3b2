...
#ident	"@(#)krpc:krpc/key_call.c	1.8"
	...
	des_block *deskey;
{
	...
	if (res.status != KEY_SUCCESS) {
		RPCLOG(1, "key_encryptsession: encrypt status is nonzero\n", 0);
		return (-1);
	}
	...
	des_block *deskey;
{
	...
	if (res.status != KEY_SUCCESS) {
		RPCLOG(1, "key_decryptsession: decrypt status is nonzero\n", 0);
		return (-1);
	}
	*deskey = res.cryptkeyres_u.deskey;
	return (0);
	...
	char				*rslt;
{
	...
	if (error != 0) {
		RPCLOG(1, "key_call: clnt_tli_kcreate rtned error %d", error);
		return (0);
	}

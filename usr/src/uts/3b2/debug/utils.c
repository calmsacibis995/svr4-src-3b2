...
/*
 * Disk read/write utilities (handle with care).
 */

extern argcount();

wdsk(drive, part, block)
{
	if (argcount() < 3) {
		cmn_err(CE_CONT, "usage: wdsk drive partition sector\n");
		return;
	}
	iodsk(drive, part, block, B_WRITE);
}

rdsk(drive, part, block)
{
	if (argcount() < 3) {
		cmn_err(CE_CONT, "usage: rdsk drive partition sector\n");
		return;
	}
	iodsk(drive, part, block, B_READ);
}

struct buf dumpbh;

iodsk(drive, part, block, flags)
{
	register struct buf *bp = &dumpbh;
	dev_t dev;
	extern int dumpbuf[];		/* this is declared in ml/cdump.c */

	dev = makedevice(0x11, (drive * 16 + part));
	bp->b_error = 0;
	bp->b_resid = 0;
	bp->b_bcount = 512;
	bp->b_bufsize = 512;
	...
	bp->b_dev = cmpdev(dev);
	bp->b_blkno = block;
	bp->b_un.b_addr = (caddr_t)dumpbuf;

	cmn_err(CE_NOTE, "%s dev=0x%x sector 0x%x into addr=0x%x",
	  (flags & B_READ) ? "reading 512 bytes from" : "writing 512 bytes to",
	  dev, bp->b_blkno, bp->b_un.b_addr);

	(*bdevsw[getmajor(dev)].d_strategy)(bp);

	cmn_err(CE_NOTE, "returned from strategy flags=%x", bp->b_flags);
}

#define	ADDR1	(char *)0x40000000
#define	ADDR2	(char *)0x40500000
#define badaddr(x) (((x) < ADDR1) || ((x) > ADDR2) || ((int)(x) & 3))
...
char *addr;
{
	...
	if ( (addr == NULL) || badaddr(addr)) {
		cmn_err(CE_CONT, "bad starting address 0x%x\n", addr);
		return;
	}
	while ((addr = *(char **)addr) != NULL) {
		if ( ++cnt > 10000 ) {
			cmn_err(CE_CONT, "Infinite loop(?) at 0x%x\n", addr);
			break;
		}
		if (badaddr(addr)) {
			cmn_err(CE_CONT, "bad pointer 0x%x\n", addr);
			break;
		}
	}
	cmn_err(CE_CONT, "list pointed to %d items\n", cnt);
}

...
#ident	"@(#)debug:debug/disasm.c	1.4"
...
static uint addr;
static int scount, absflg;

/* call to continue disasmbly from where we left off */
discont(arg)
int arg;
{
	int	argc, *argp;
	...
	if (argc > 2) {
		cmn_err(CE_CONT, "^Usage: call discont [ #instr [ flag ] ]\n");
		return;
	}
	...
	disasm(addr, count, absflg);
}

#define MAXCOUNT 0x100		/* question a disasm of more than 256 inst. */
...
int arg;
{
	...
	if (argc > 1) {
		...
		if ( (count > MAXCOUNT) || (count < 0) ) {
			if (count != scount) { /* first time bad number */
			   cmn_err(CE_CONT,
			    "^if you really want to dump 0x%x instructions\n",
			    count);
			   cmn_err(CE_CONT,
			    "^type that same command again\n");
			   scount = count;
			   return;
			}

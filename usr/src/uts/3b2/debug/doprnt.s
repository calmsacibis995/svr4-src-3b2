...
#	actual or intended publication of such source code.
	...
	.ident	"@(#)debug:debug/doprnt.s	1.1"
...
#	.word	._,._,._,._,._,.E,._,.G			# @ A B C D E F G
	.word	._,._,._,._,._,._,._,._			# @ A B C D E F G
...
#	.word	._,._,._,.c,.d,.e,.f,.g			# ` a b c d e f g
	.word	._,._,._,.c,.d,._,._,._			# ` a b c d e f g
	.word	.h,._,._,._,.l,._,._,.o			# h i j k l m n o
...
.putpfx:
	...
	jz	.putlzer		# If zero--skip prefix.
	movw	prefix(%sp),%r0
	movw	ptr,%r1
	addw2	%r2,ptr
	cmpw	ptr,buff_end		# Room in buffer before buffer end?

...
#if defined(__STDC__)
static int addraw(char *,struct res_rec *);
static int wrdom(struct domain *, char *, FILE *);
static int rdfind(struct domain *, char *name);
#endif
...
struct domain	*d;
{
	...
	if (d->d_rec) {
		rp = d->d_rec[0];
		for (i=0; d->d_rec[i] && i < d->d_size && rp != NULL; rp=d->d_rec[++i])

...
#ident	"@(#)nserve:nsfunc.c	1.14.6.1"
#include <stdio.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
int		pd;
{
	...
	case NS_QUERY:
		...
		else if ((Primary && domauth(dom)) || addind(dom,nreq) == FAILURE) {
...
char *newpass, *mach, *dom;
{
	...
	if ((fp = fopen(filename, "r")) != NULL) {
		...
		if (rename(tempfile, filename) || chmod(filename,
			S_IRUSR|S_IWUSR))
			return(R_EPASS);
	}
	return(ret);

...
#ident	"@(#)nserve:nsrec.c	1.21.11.1"
#include <stdio.h>
#include <string.h>
#include <signal.h>
...
/*
 * name server recovery functions and other related functions.
 */

#define NLS_Q		"nlsadmin -q"
#define NLS_ADDR 	"nlsadmin -l -"
#define UNADV_CMD 	"/usr/sbin/unadv"
#define ADV_CMD		"/usr/sbin/adv"
#define AUTH_INFO	"/etc/rfs/auth.info"
#define MNT_CMD		"/sbin/mount -d"
#define UMNT_CMD	"/sbin/umount -d"
#define R_TIMEOUT	25
...
struct request	*req;
{
	...
	if (domauth(dname) == SOA) {
		LOG3(L_COMM,"(%5d) setmaster: don't rewrite %s rfmaster\n",Logstamp, Net_spec);
		return(R_NSFAIL);
	}
...
char	*dom;
{
	...
	if ((rlist = findrr(abuf,NSTYPE)) == NULL ||
		...
		PLOG2("RFS %s name server can't find primary name server address\n", Net_spec);
		return(FAILURE);
	}
	...
	if (j == 0) {
		PLOG2("RFS %s name server can't find good primary ns address\n",Net_spec);
		return(FAILURE);
	}
...
getmyaddr()
{
	...
	if (fgets(buf,BUFSIZ,fp) == NULL) {
		PLOG2("RFS %s name server: can't get listen address\n",Net_spec);
		pclose(fp);
	}
		...
		for (j=0, rec=srlist[0]; j < count; rec = srlist[++j]) {
			...
			if (nptr->n_state == R_UNUSED) {
				...
				if ((nptr->n_addr = astoa(arlist[j]->rr_a,NULL))
					== NULL) {
					PLOG3("RFS %s name server: bad NS addr for %s\n",
						Net_spec, nptr->n_dname);
					freelist(rlist);
					return(FAILURE);
				}
			...
			if (strcmp(rec->rr_data,nptr->n_dname)) {
				PLOG4("RFS %s name server: NS setup conflict, %s & %s\n",
					Net_spec,rec->rr_data,nptr->n_dname);
				freelist(rlist);
				return(FAILURE);
			}
		}
...
setrstate()
{
	...
	for (np=Nsinfo; np < &Nsinfo[MAXNS]; np++) {
		...
		case R_PRIME:
			if (pnp != NULL) {
				PLOG4("RFS %s name server: WARNING: found 2 name servers, %s and %s\n",
					Net_spec, pnp->n_dname, np->n_dname);
				PLOG3("RFS %s name server: Using %s as primary\n",
					Net_spec, pnp->n_dname);
				break;
			}
			pnp = np;
	...
	if (!pnp && !(pnp = npnp))  {
		PLOG2("RFS %s name server: recovery: no primary assignment possible\n", Net_spec);
		sigset(SIGUSR2,usr2sig);
		return(FAILURE);
	}
	pnp->n_state = R_PRIME;
...
int		pd;
{
	...
	case NS_IM_P:
		if (Primary) {
			PLOG3("RFS %s name server: NS %s, primary collision\n",
				Net_spec,hp->h_dname);
...
int	pd;
{
	...
	case RECOVER:
		...
		remfrsel(pptr);
		if (setrstate() == FAILURE) {
			PLOG2("RFS %s name server: FATAL recovery error\n",Net_spec);
			Done = TRUE;
			return(R_NSFAIL);
		}
		addtosel(pptr);
...
nsrel()
{
	...
	if (setrstate() == FAILURE) {
		PLOG2("RFS %s name server: FATAL error, nsrel recovery failure\n",Net_spec);
		Done = TRUE;
		return(R_NSFAIL);
	}

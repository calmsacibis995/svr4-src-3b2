...
creat_hdr()
{
	...
	case CRC:
		Gen.g_magic = CMN_CRC;
		break;
	case TARTYP:	/* TAR and USTAR do not use g_magic */
		T_lname[0] = '\0';
		break;
	default:
		msg(EXT, "Impossible header type."); 
	...
	do {
		...
		if (!link(name1_p, name2_p)) {
			...
			if (!(Args & OCu) && G_p->g_mtime <= DesSt.st_mtime)
				msg(ERR, "Existing \"%s\" same age or newer", name2_p);
...
file_in()
{
	...
	G_p = &Gen;
	if (Hdr_type == TARTYP && G_p->g_nlink == 1) { /* TAR and USTAR */
	...
	if (G_p->g_nlink == 1 || (Hdr_type == TARTYP)) {
		if (Aspec) {
			if (ckname() != F_SKIP && creat_spec() > 0)
				VERBOSE((Args & (OCv | OCV)), G_p->g_nam_p);
		} else {
	...
	if (!Onecopy) {
		...
		if (ckname() == P_SKIP) {
			...
			if (!lnkem) {
				...
				(void)creat_lnk(Lnknam_p, Full_p); 
				data_in(P_SKIP);
...
file_out()
{
	...
	if (Hdr_type == TARTYP) { /* TAR and USTAR */
		if (Adir) /* Tar and USTAR do not archive directories */
			return;
		if (G_p->g_nlink == 1) {
			data_out();
			return;
	...
	case TARTYP: /* TAR and USTAR */
		if (*Buffr.b_out_p == '\0')
			*Gen.g_nam_p = '\0';
			...
			(void)sscanf(Thdr_p->tbuf.t_devmajor, "%lo", &Gen.g_dev);
			(void)sscanf(Thdr_p->tbuf.t_devminor, "%lo", &Gen.g_rdev);
			(void)sscanf(Thdr_p->tbuf.t_cksum, "%lo", &Gen.g_cksum);
			if (Thdr_p->tbuf.t_linkflag == '1')
				Gen.g_nlink = 1;
			else
				Gen.g_nlink = 0;
...
write_hdr()
{
	...
	case TARTYP: /* TAR and USTAR */
		...
		(void)sprintf(Thdr_p->tbuf.t_mode, "%6lo ", G_p->g_mode);
		(void)sprintf(Thdr_p->tbuf.t_uid, "%6ol ", G_p->g_uid);
		(void)sprintf(Thdr_p->tbuf.t_gid, "%6ol ", G_p->g_gid);
		(void)sprintf(Thdr_p->tbuf.t_size, "%11lo ", G_p->g_filesz);
		(void)sprintf(Thdr_p->tbuf.t_mtime, "%11lo ", G_p->g_mtime);
		(void)sprintf(Thdr_p->tbuf.t_devmajor, "%8lo", G_p->g_rdev);
		(void)sprintf(Thdr_p->tbuf.t_devminor, "%8lo", G_p->g_rdev);
		if (T_lname[0] != '\0')
			Thdr_p->tbuf.t_linkflag = '1';
		else
			Thdr_p->tbuf.t_linkflag = '\0';
		(void)strncpy(Thdr_p->tbuf.t_linkname, T_lname, strlen(T_lname));
		(void)sprintf(Thdr_p->tbuf.t_cksum, "%6lo", (int)cksum(TARTYP, 0));
		break;
	default:
		msg(EXT, "Impossible header type.");
	} /* Hdr_type */

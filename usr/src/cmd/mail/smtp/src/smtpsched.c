...
#ident	"@(#)mail:smtp/src/smtpsched.c	1.3"
#ident "@(#)smtpsched.c	1.3 'attmail mail(1) command'"
#include <sys/types.h>
#include <stdio.h>
#include <ctype.h>
#include <signal.h>
...
#define	VOLDW	2*3600L		/* wait time between tries at very old files */
	...
	char *av[];
{
	...
	/*
	 *  if specific directories, do just them.  the lock for these directories
	 *  is blocking to make sure we get all files in the directory processed.
	 */
	if(optind!=ac){
		for(; optind<ac; optind++)
			dodir(av[optind], BLOCK);
		return 0;
	}
...
openlog()
{
	...
	if(open("/dev/null", 0)==0){
		...
		close(2);
		close(3);
		if(open(s_to_c(f), 2)==1
		|| creat(s_to_c(f), 0666)==1){
			lseek(1, 0L, 2);
			dup(1);
			dup(1);
		}
	...
	char *f;
{
	...
	now = time((long *)0);
	strcpy(anow, ctime(&now));
	anow[16] = 0;
	lseek(1, 0L, 2);
	printf("%d %s: ", mypid, anow+4);
	printf(f, a, b, c);
	fflush(stdout);
}
#endif

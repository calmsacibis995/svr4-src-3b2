...
#ident	"@(#)mail:smtp/src/smtpd.c	1.4"
#ident "@(#)smtpd.c	1.4 'attmail mail(1) command'"
#ifndef lint
...
open_transports()
{
	...
	while ((ncp = getnetpath(handle)) != NULL) {
		if (netdir_getbyname(ncp, &ndh, &nap))
			continue;
...
process()
{
	...
	(void) gotodir(SMTPQROOT);	/* force creation of queuedir */
...
/*loadav - return the 5 minute load average.
...
/*	return(avenrun[1]);	five minute average */
	return(avenrun[0]);	/* one minute average */
}

...
#ident	"@(#)mail:mail/send.c	1.7"
#ident "@(#)send.c	2.32 'attmail mail(1) command'"
/*
    NAME
...
char		*name;
{
	...
	*/
	if (failsafe && (access(SAVEDIR, A_EXIST) != A_OK)) {
		...
		rc = send(&list, letnum, buf, level + 1);
	...
	*/
	...
	if (areforwarding(file)) {
		...
		while (*tsp && !fini) {
	       ...
	       "=============== Start of stderr from surrogate ============\n");
				    rewind (SURRerrfile);
				    while ((cnt = fread(buf,1,sizeof(buf),SURRerrfile)) > 0) {
					fwrite (buf,cnt,1,dbgfp);
				    }
				    fprintf(dbgfp,
	...
	/* Disallow links. */
	if (!safefile(file)) {
		fprintf(stderr, "%s: '%s' has more than one link or is a symbolic link\n",
			program, file);
		unlock();
		error = E_FILE;
		Dout(pn, 0, "%s has more than one link or is a symbolic link, error set to %d\n", file, error);
		goback(0);
		rc = FALSE;
		goto rtrn;
	}

...
#define J_FOREGND	0200	/* job is currently running */
...
freejobs()
{
	...
	if (jobnote) {
		...
		for (jp = joblst; jp; jp = jp->j_nxtp) {
			propts = (PR_NEW|PR_STAT);
			if (jp->j_jid)
				propts |= (PR_CUR|PR_JID|PR_CMD);
	...
	if (jobdone) {
		for (njp = &joblst; jp = *njp; ) {
			if ((jp->j_flag & (J_NOTIFY|J_DONE)) == J_DONE) {
...
register struct job *jp;
{
	...
	while (waitpid(jp->j_pid, &stat, WUNTRACED|WNOWAIT) != jp->j_pid)
		continue;
	statjob(jp,stat,1,1);
	waitpid(jp->j_pid, 0, WUNTRACED);
	if ((jp->j_flag & J_DONE) && exitval && (flags & errflg))
		exitsh(exitval);
	flags |= eflag;
}
...
char *argv[];
{
	...
	while ((c = getopt(argc, (const char **)argv, "lpx")) != -1) {
		if (propts) {

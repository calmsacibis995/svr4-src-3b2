...
ein:
	...
	case 't':
		if (!subop) {
			int length;
			wchar_t wchar;
			length = mbftowc(lastcp, &wchar, getesc, &Peekkey);
			if (length <= 0 || wchar == 0) {
				beep();
				return;
			}
...
errlab:
	 ...
	 */
	case 'N':
		vglobp = vscandir[0] == '/' ? (unsigned char *)"?" : (unsigned char *)"/";
		c = *vglobp++;
		goto nocount;
	...
	wchar_t c;
{
	...
	for(;;) {
		...
		if(dir == 1)
			...
			wcursor = lastchr(linebuf, wcursor);
		if ((length = mbtowc(&wchar, (char *)wcursor, MULTI_BYTE_MAX)) > 0 && wchar == c)
			return (1);
	}
}
	...
	int cnt;
{
	...
	if (dir == 1) {
		...
		while (wordof(which, wcursor)) {
			length = mbtowc(&wchar, (char *)wcursor, MULTI_BYTE_MAX);
			if(length <= 0)
				length = 1;

...
static int termiosflag = -1; /* flag for using termios ioctl
			      * structure */
...
register wchar_t c;
{
	...
	if ((int)(c & QUOTE))
		...
		putchar('^'), c = ctlof(c);
	else if (c >= 0200 && (putoctal || !wisprint(c))) {
		outchar('\\');
		outchar(((c >> 6) & 07) + '0');
		outchar(((c >> 3) & 07) + '0');
		outchar((c & 07) + '0');

...
unsigned char	tfname[40];
unsigned char	rfname[40];
int	havetmp;
short	tfile = -1;
short	rfile = -1;
...
dumbness:
	...
	ichanged = 0;
	ichang2 = 0;
	(void)strcat(tfname, "/ExXXXXXXXXXX");
	for (p = strend(tfname), i = 10, j = getpid(); i > 0; i--, j /= 10)
		*--p = j % 10 | '0';
...
#ifdef	VMUNIX
#define	INCORB	64
unsigned char	incorb[INCORB+1][BUFSIZ];
#define	pagrnd(a)	((unsigned char *)(((int)a)&~(BUFSIZ-1)))
...
struct	rbuf {
	short	rb_prev;
	short	rb_next;
	unsigned char	rb_text[BUFSIZ - 2 * sizeof (short)];

	...
	char **argv;
{
	...
	if (argc == 1)
		...
		if (hp = gethostbyname("localhost"))
			(void) memcpy((caddr_t)&server_addr.sin_addr,
	...
	char *host;
{
	...
	if (addr->sin_addr.s_addr == -1 || addr->sin_addr.s_addr == 0) {
		...
		(void) memcpy((char *)&addr->sin_addr, hp->h_addr,
				hp->h_length);
	}
	...
	char **argv;
{
	...
	if (getuid()) {
		/*
		 * This command allowed only to root, just for sanity reasons!
		 */

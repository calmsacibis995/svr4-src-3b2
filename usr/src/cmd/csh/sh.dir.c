...
gotcha:
	if (*cp != '/') {
		...
		/*
		 * The following codes are replaced because 3b2 has problem
		 * with pointer arithmetic
		 * for (p = dcwd->di_name; *p++;)
		 *	;
		 * if ((cwdlen = p - dcwd->di_name - 1) == 1)	** root **
		 */
		if ( ( cwdlen = (strlen_(dcwd->di_name) ) ) == 1 ) /* root */
			cwdlen = 0;
		/*
		 *
		 * for (p = cp; *p++;)
		 ...
		 */
		len = strlen_(cp);
		dp = (tchar *)xalloc((unsigned) (cwdlen + len + 1)*sizeof(tchar));
			...
			if ((hardpaths || sp > cp) &&
				...
				if (*link != '/') {
					...
					*sp = '\0';
					 ...
					 */
					p1 = newcp = (tchar *)xalloc((unsigned)
						(strlen_(sp) + cc +strlen_(p))*sizeof
						(tchar));
					/*

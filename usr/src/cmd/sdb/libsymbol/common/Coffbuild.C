...
#ident	"@(#)sdb:libsymbol/common/Coffbuild.C	1.7"
#include	"Coffbuild.h"
#include	"Locdesc.h"
#include	"Syminfo.h"
#include	"Type.h"
...
Coffbuild::find_arcs( long & sibofs, long & childofs )
{
	...
	{
		childofs = nextofs;
		sibofs = file_offset + sym.n_value*18;
	}
	...
	{
		sibofs = file_offset + aux.x_sym.x_fcnary.x_fcn.x_endndx*18;
		if ( sym.n_scnum < 0 )
		{
			sibofs = nextofs;
			childofs = 0;
...
Coffbuild::get_type_C()
{
	...
	switch (typ)
	{
		...
		case T_ENUM:
			...
			{
				form = af_coffrecord;
				value.word = file_offset + aux.x_sym.x_tagndx * 18;
			}
			break;
		case T_MOE:
			form = af_none;
...
Coffbuild::get_pc_info( long offset, Iaddr & lopc, Iaddr & hipc )
{
	...
	while ( get_syminfo( offset , syminfo ) )
	{
		...
		{
			size = syminfo.sibling - syminfo.child + 36;
...
Coffbuild::get_lineinfo( long loffset, long lncnt, long foffset )
{
	...
	while ( (nextofs = coff.get_symbol(offset, sym, aux )) != 0 )
	{
		...
		{
			b = get_fcn_lineinfo( b, cofflbuf + lncnt * 6 );
		}
		else if ( sym.n_sclass == C_FILE )
		{
			break;

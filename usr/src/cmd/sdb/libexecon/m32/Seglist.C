...
#ident	"@(#)sdb:libexecon/m32/Seglist.C	1.24"
...
//		fd = ::open_object( key, 0 );
		fd = ::open_object( key, 0x80000200 ); // until /proc is fixed
		sym.symtable = ::get_symtable( fd );
		::close( fd );
		return 1;
	}
...
Seglist::rtld_base(Key key)
{
	...
	Iaddr base = 0;
	Iaddr argp = proc->getreg( REG_AP ); /* arg ptr */
...
//	if ( (fd = ::open_object(key,0)) == -1 )
	if ( (fd = ::open_object(key,0x80000200)) == -1 ) // til /proc is fixed
	{
		return 0;
	}
	...
	{
		seg = new Segment( key, executable_name, 0xc0020000, 0xdffff, 0xc0020000, 0 );
		prepend( seg);
		return 1;
	}
}
...
Seglist::readproto( int textfd, int corefd, Core * core, int flatmap )
{
	...
	else if ( flatmap )
	{
		...
		seg = new Segment(textkey, executable_name, 0, size, 0, 0);
		prepend(seg);
		return 1;
	}
...
Seglist::setup( int fd, int & rtl_used )	
{
	...
	else if ( ::read( fd, ld_so, LD_SO_BUFSIZ ) != LD_SO_BUFSIZ )
	{
		result = 0;
	}
	else
	{
//	can't find base addr until after successful exec()
...
int
Seglist::in_stack( Iaddr pc )
{
	return INSTACK( pc );
}

int
Seglist::in_text( Iaddr pc )
{
	return INTEXT( pc );
}

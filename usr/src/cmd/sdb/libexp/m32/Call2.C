...
#ident	"@(#)sdb:libexp/m32/Call2.C	1.3"
...
void runnit( Process *process, Iaddr target, Execstate oldstate )
{
	...
	int ret = process->run( 1, target, 0x7fffffff, -1 );
...
Rvalue *get_ret( Process *process, TYPE *ret_type )
{
	...
	if ( ret_type->fund_type(ft) ) {
		switch( ft ) {
		default:
			word = process->getreg( REG_R0 );
			ret_val = new Rvalue( word );
			break;
		case ft_lfloat:
			p = (unsigned long *)&itype.idfloat;
			*p++ = process->getreg( REG_R0 );
			*p   = process->getreg( REG_R1 );

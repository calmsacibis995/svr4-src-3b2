...
int	flag;
{
	...
	switch(sh.wdval)
	{
		...
		case IFSYM:
		{
			...
			t->if_.eltre=((w=sh.wdval)==ELSYM?sh_parse(FISYM,NLFLG):
...
#define POSIX 1
...
#else
					argp->argnxt.ap=(struct argnod*) argset;
			...
			*settail = 0;
#else
			t->com.comset=(struct argnod*) argset;
#endif
			sh.wdset &= ~CAN_ALIAS;
			return(t);
		}

...
#ident	"@(#)oampkg:pkgadd/main.c	1.12.3.1"
...
static char	*device,
		...
		*ids_name;	/* name of data stream device */
...
char	**argv;
{
	...
	device = NULL;
	while ((c = getopt(argc,argv,"s:d:a:r:n?")) != EOF) {
		switch(c) {
		  case 's':
			spoolto = optarg;
			break;
	...
	ids_name = NULL;
	if(pkgdev.bdevice) {
		if(n = getvol(pkgdev.bdevice, NULL, NULL, "Insert %v into %p.")) {
			if(n == 3)
				quit(3);
			if(n == 2)
	...
	repeat = ((optind >= argc) && pkgdev.mount && !ids_name);
	beenhere = 0;
...
again:
	...
	if(!ids_name && pkgdev.mount) {
		pkgdev.rdonly++;
		if(n = pkgmount(&pkgdev, NULL, 0, 0, beenhere))
			quit(n);
		beenhere = 1; /* Need to prompt for floppy next time */
	}

	if(ids_name) {
		/* use character device to force rewind of datsream */
		if(pkgdev.cdevice && !pkgdev.bdevice && (n = getvol(pkgdev.name, NULL, NULL, NULL))) {
			if(n == 3)
				quit(3);
			if(n == 2)
				progerr("unknown device <%s>", pkgdev.name);
		...
		if(ds_init(ids_name, &argv[optind])) {
...
pkginstall()
{
	...
	n = pkgexecv(NULL, NULL, arg, ds_curpartcnt >= 0 ? ds_fd : -1);

...
%{
#ident	"@(#)mkboot-3b2:mkboot.y	1.14.4.1"
...
element		:	'%' NUMBER
		...
		|	'%' DWORD
				...
				if (strlen($2) == 1 && ((p = strchr("cCsSiIlL",*$2)) != NULL)){
					...
					tmp = getsymvalue($2);
					if (tmp == -1){
						yyerror( "\"%%%s\" can't find symbol ", $2 );
						YYERROR;
					}
					nformat->value = (unsigned short)tmp;
...
node(va_alist)
	...
	left = va_arg(ap, struct tnode *);
	right = va_arg(ap, struct tnode *);
	ntree->left = left;
	ntree->right = right;
	...
	case '"':
	case 'I':
		ntree->name = va_arg(ap, char *);
		break;
	case 'N':
		ntree->value = va_arg(ap, int);
		break;
	default:
	/* yyfatal won't return, so do the va_end here */
		va_end(ap);

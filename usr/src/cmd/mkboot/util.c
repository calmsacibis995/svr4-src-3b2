...
#ident	"@(#)mkboot-3b2:util.c	1.14.3.3"
...
check_master( name, mp )
	...
	if ( mp->flag & KERNEL )
		...
		if ( mp->nvec || mp->prefix[0] || mp->soft != -1 || mp->ndev || mp->ipl )
			{
	...
	if ( ((dev_t)mp->soft != NODEV) && 
		(((!(mp->flag & NEWDRV)) && (unsigned long)mp->soft > 127) ||
		(mp->flag & NEWDRV && (unsigned long)mp->soft > 255 )))
		{
		warn( "%s: major number > 127 for old driver or major > 255 for new driver", name );
		skip = TRUE;
		}
	...
	if ( mp->flag & (EXECTYP) && !(mp->flag & (NOTADRV)))
		{
		warn( "%s: illegal flag combination -- EXECTYP only valid for NOTADRV", name );
		skip = TRUE;
		}

	if ( mp->flag & (SCHEDCLASS) && !(mp->flag & (NOTADRV)))
		{
		warn( "%s: illegal flag combination -- SCHEDCLASS only valid for NOTADRV", name );
		skip = TRUE;
		}
...
print_flag( flag )
	...
	if ( flag & EXECTYP ) 
		strcat(buffer, ",EXECTYP");

	if ( flag & SCHEDCLASS ) 
		strcat(buffer, ",SCHEDCLASS");

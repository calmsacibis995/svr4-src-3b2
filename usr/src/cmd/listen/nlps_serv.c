...
#ident	"@(#)listen:nlps_serv.c	1.3.3.2"
		...
		if (dbp = getdbfentry(svc_code_p)) {
			if (dbp->dbf_flags & DBF_OFF)  {
				...
				if (dbp->dbf_sflags & CFLAG) {
					...
					if ((i = doconfig(0, dbp->dbf_svc_code, NOASSIGN)) != 0) {
...
register char **o_argv;
{
	...
	if (wdbp->dbf_flags & DBF_UTMP) {
		...
		/* 
		 * create a utmp entry.  extra fork makes parent init,
		 * which will clean up the entry.
		 */

		DEBUG((9, "Creating a utmp entry for this service "));
		if ((tmp = fork()) < 0) {
			logmessage("NLPS: Can't fork to create utmp entry");
			(void)exit(2);
		}
		if (tmp)
			(void)exit(0);	/* kill parent */
		...
		setpgrp();
		if (fstat(0, &sbuf) < 0) {
			logmessage("NLPS: Stat failed on fd 0: no line field available for utmp entry");
			*device = '\0';
		}
		else {
			/* 
		...
		strncpy(utline.ut_user, wdbp->dbf_id, 8);

...
#ident	"@(#)listen:listen.c	1.19.8.2"
 ...
 */
	...
	logmessage("@(#)listen:listen.c	1.19.8.2");
...
register dbf_t *dbp;
{
	...
	if (dbp->dbf_flags & DBF_UTMP) {
		...
		/* 
		 * create a utmp entry --
		 * we do an extra fork here to make init this processes'
		 * parent.  this lets init clean up the utmp entry when
		 * this proc dies.
		 *
		 * the utmp routines need a file descriptor!
		 */

		DEBUG((9, "Creating a utmp entry for this service "));
		if ((tmp = fork()) < 0) {
			logmessage("Can't fork to create utmp entry");
			exit(2);
		}
		if (tmp)
			exit(0);	/* kill parent */
		...
		setpgrp();
		if (fstat(0, &sbuf) < 0) {
			logmessage("Stat failed on fd 0: no line field available for utmp entry");
			*device = '\0';
		}
		else {
			if (minor(sbuf.st_rdev) < 100)
				sprintf(device, "%.9s%02d", Minor_prefix, minor(sbuf.st_rdev));
			else
				sprintf(device, "%.8s%03d", Minor_prefix, minor(sbuf.st_rdev));
			DEBUG((9, "Device: %s", device));
		}
		/*
		 * prepend a "." so this can be distinguished as a "funny"
		 * utmp entry that may never get a DEAD_PROCESS entry in
		 * the wtmp file.
		 */
		sprintf(dummy, ".%s", Mytag);
		strncpy(utline.ut_user, dummy, 8);
		sprintf(utline.ut_id, "ls%c%c", SC_WILDC, SC_WILDC);
		strncpy(utline.ut_line, device, 12);
		utline.ut_pid = (o_pid_t)getpid();
		utline.ut_type = USER_PROCESS;
		utline.ut_exit.e_termination = 0;
		utline.ut_exit.e_exit = 0;
		utline.ut_time = (time_t) time((long *)0);
		makeut(&utline);
	}
 ...
 */
	...
	if (getenv("PATH") == NULL)
		putenv("PATH=/usr/bin:/sbin:/usr/sbin:/etc");

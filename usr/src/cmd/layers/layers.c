...
#ifdef SVR40
	sid = getsid(0);
	trace("Layers session sid = %d", sid);
#endif /* SVR40 */
 ...
 */
	...
	/* Reset the line.
	 */
	if (ioctl(1, I_PUSH, nonlayersld) == SYSERROR)
		error(DISP_ERR, errno, "I_PUSH of nonlayersld failed");
				...
				/* under the mux			     */
{
	...
	for (;;) {
		...
		switch( errno ) {
		  ...
		  case EAGAIN: /* STREAMS buf alloc failure; go again*/
		        trace("multiplex: Out of STREAMS bufs. trying again");
		        continue; 
...
unsigned char maxpktarg;
{
	...
	/*
	** The following has two functions. First, it allows maxpktarg
	...
	**
	** This also lowers packet size for slower baud rates. This is
	** necessary because at lower baud rates output gets too bursty
	...
	*/
	if(maxpktarg || maxpkt != 1)  {
		if(maxpktarg && (maxpkt == 1 || maxpktarg <= maxpkt) ) {
			/* use maxpktarg if possible */
			maxpkt = maxpktarg;
		}
		else {
			/* adjust maxpkt to baud rate */
			if((int)(ttysave.c_cflag&CBAUD) < B9600)
				maxpkt = 128;
			if((int)(ttysave.c_cflag&CBAUD) < B2400)
				maxpkt = 64;

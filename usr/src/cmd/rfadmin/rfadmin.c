...
#ident	"@(#)rfadmin:rfadmin.c	1.3.11.2"
#include <sys/types.h>
#include <sys/vnode.h>
#include <sys/list.h>
#include <sys/stat.h>
...
char *argv[];
{
	...
	while ((c = getopt(argc, argv, "t:pqa:r:o:")) != EOF) {
		switch (c) {
			case 't':
				tplist = optarg;
				break;
	...
	if (error) {
		fprintf(stderr, "%s: usage: %s [-p][-t tp1,tp2,...]\n", cmd_name, cmd_name);
		fprintf(stderr, "                %s [-q]\n", cmd_name);
...
int   cmd;
{
	...
	if ((sem = creat(SEM_FILE, S_IRUSR|S_IWUSR)) == -1 ||
...
char	*name;
{
	...
	if (unlink(filename) == -1 || link(tempfile, filename) == -1 ||
	    unlink(tempfile) == -1 || chmod(filename, S_IRUSR|S_IWUSR) == -1) {
...
int	flag;
{
	...
	{
		p = NULL;
		if (is_prime1(tp, mach, domain, flag) == YES)
			return(YES);
	}
	return(NO);
}
...
int	flag;
{
	...
	(void) sprintf(netmaster, TPNETMASTER, tp);
	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		while (buf[size-2] == '\\' && buf[size-1] == '\n') {
			...
			if ((size = (size - 2 + strlen(morebuf))) > BUFSIZ) {
				fprintf(stderr, "%s: warning: line %d of rfmaster too long\n", cmd_name, line);
				break;
			}
			strcat(buf, morebuf);
		}
...
pr_primary()
{
	...
	while(1)
	{
		if ((rtn = ns_getblocks(sndp)) == (struct nssend *)NULL)
		{
			if (ns_errno == R_INREC)
				nserror(cmd_name);
			else
		...
		printf("the acting name server for domain %s on %s is %s\n", dname, rtn->ns_addr->protocol, *rtn->ns_mach);
...
char	filename[];
{
	...
	if (stat(filename, &sbuf) == -1)
		close(creat(filename, S_IRUSR|S_IWUSR));
...
char *opt;
{
	...
	if (strcmp(opt, "loopback") == 0) {
		...
		tmp = rfsys(RF_DEBUG, -2);
		rfsys(RF_DEBUG, 0);
		rfsys(RF_DEBUG, tmp & ~DB_LOOPBCK);
	}
		...
		tmp = rfsys(RF_DEBUG, -2);	/* -2 returns dudebug level */
		if (tmp & DB_LOOPBCK)
			printf("on\n");
		else
			printf("off\n");

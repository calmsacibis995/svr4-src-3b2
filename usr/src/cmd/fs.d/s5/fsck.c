...
register DIRECT *dirp;
{
	...
	if((inum = dirp->d_ino) == 0)
		return(KEEPON);
	thisname = pathp;
	if((&pathname[MAXPATHLEN] - pathp) < DIRSIZ) {
		if((uint_t)(&pathname[MAXPATHLEN] - pathp) < strlen(dirp->d_name)) {
			printf("%c %sDIR pathname too deep\n",id,devname);
			printf("%c %sIncrease MAXPATHLEN and recompile.\n",
			id,devname);

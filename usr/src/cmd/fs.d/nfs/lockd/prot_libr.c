...
#ident	"@(#)nfs.cmds:nfs/lockd/prot_libr.c	1.5"
/*
...
extern int debug, used_reclock, used_prolock;
...
init()
{
	...
	for (i=0; i < MAXPROCESS; i++) {
		proc_lists[i].pid = 0;
		proc_lists[i].lox = (struct data_lock *)NULL;
	}
	cancel_mon();
}
...
pr_all()
{
	...
	if (used_prolock) {
		printf("***** PROC_LISTS *****\n");
		for (i=0, ii=0; i < MAXPROCESS && ii < used_prolock; i++) {
			if ( proc_lists[i].pid ) {
				printf("PROC (%d) : \n", proc_lists[i].pid);
				ii++;
				if ( proc_lists[i].lox != NULL ) {
					for (tt = proc_lists[i].lox; tt;
						tt = tt->Next) {
						print_lock(tt);
					}
				}
	...
	printf("used_reclock=%d, used_prolock=%d\n", used_reclock, used_prolock);
	(void) fflush(stdout);
}

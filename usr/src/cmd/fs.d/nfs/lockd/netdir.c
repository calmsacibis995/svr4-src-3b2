...
#ident	"@(#)nfs.cmds:nfs/lockd/netdir.c	1.1"
/*
...
static struct translator {
	...
	char			*(*ma)();	/* _netdir_mergeaddr	*/
	int			(*cont)();	/* _netdir_control	*/
...
extern char			*_netdir_mergeaddr();
extern int			_netdir_control();
extern char			*_taddr2uaddr();
extern struct netbuf		*_uaddr2taddr();
extern int			_netdir_control();

static struct translator tcp_xlate = {
		_netdir_getbyname,
		_netdir_getbyaddr,
		_netdir_mergeaddr,
		_netdir_control,
 ...
 */
int
netdir_mergeaddr(tp, muaddr, ruaddr, uaddr)
	struct netconfig	*tp;	/* the netconfig entry		*/
	char			**muaddr;/* the answer			*/
	char			*ruaddr;/* Clients universal addr	*/
	char			*uaddr;	/* Its universal address	*/
{
	struct translator	*t;	/* pointer to translator list 	*/
	char			*lr,	/* routines to try		*/
				*x;	/* the answer 			*/
...
/*
 * This is the library routine to do the transport specific thing.
 * Other parameters can be passed through "par". If par has to be used to
 * return some values, it should have enough space for it.
 */
int
netdir_control(tp, option, fd, par)
	struct netconfig	*tp;	/* the netconfig entry		*/
	...
	char			*par;	/* parameters if any		*/
{
	...
	for (i = 0; i < tp->nc_nlookups; i++) {
		...
		for (t = xlate_list; t; t = t->next) {
			if (strcmp(lr, t->tr_name) == 0) {
				return ((*(t->cont))(tp, option, fd, par));
			} 
		}
		/* If we didn't find it try loading it */
		if (!t) {
	...
	char	*name;		/* file name to load */
{
	...
	if (access(name, R_OK)) {
		...
		t->gbn = dllink(lr, "_netdir_getbyname");
		if (!(t->gbn)) {
			_nderror = ND_NOSYM;
			free((char *)t);
			return (0);
		...
		t->gba = dllink(lr, "_netdir_getbyaddr");
		if (!(t->gba)) {
			_nderror = ND_NOSYM;
			free((char *)t);
			return (0);
		...
		/* resolve the netdir_mergeaddr symbol */
		t->u2t = dllink(lr, "_netdir_mergeaddr");
		if (!(t->u2t)) {
			_nderror = ND_NOSYM;
			free((char *)t);
			return (0);

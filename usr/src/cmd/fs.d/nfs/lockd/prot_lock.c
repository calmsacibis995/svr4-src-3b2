...
#ident	"@(#)nfs.cmds:nfs/lockd/prot_lock.c	1.10"
/*
	...
	struct reclock *a;
{
	...
	for (i=0; (i < MAXPROCESS) && (proc_lists[i].pid != a->lck.lox.lld.l_pid);
		i++);
	if ( i == MAXPROCESS ) {
		if (debug)
			printf("creating a proc entry .....\n");
		/*
		 * create
		 */
		t = get_proc_list((long)a->lck.lox.lld.l_pid);
		if ( t == NULL )
...
void
delete_prolock(fp, a)
	struct process_locks *fp;
	reclock *a;
{
	if (debug)
		printf("enter delete_prolock()....\n");

	if (a->prev != NULL)
		a->prev->next = a->next;
	else {
		fp->lox = (struct data_lock *)&a->next->lck.lox;
		if (a->next == NULL)
	...
	struct data_lock *a;
{
	...
	struct data_lock *t, *next;

	/*
	 * Go through table and match rpid and pid
	 */
	if (debug)
		printf("enter add_process_lock().....\n");
	for (p = &proc_lists[0]; p && p->pid != a->lld.l_pid; p = p->next);
	if (p && p->pid == a->lld.l_pid) {
		if (debug)
			printf("PID : %d\n", p->pid);
		if ((t = p->lox) == NULL) {
			p->lox = a;

...
#ident	"@(#)nfs.cmds:nfs/lockd/svc_create.c	1.1"
/*
	...
	while (nconf = _rpc_getconf(net)) {
		...
		if (l == (struct xlist *) NULL) {
			...
			if (!netdir_getbyname(nconf, &ns, &nas)) {
				...
				syslog(LOG_ERR,
				    "svc_create_local_service: no well known address for %s on transport %s\n",
				    servname, nconf->nc_netid);
				fprintf(stderr,
				    "svc_create_local_service: warning: no well known address for %s on transport %s\n",
				    servname, nconf->nc_netid);

...
#ident	"@(#)nfs.cmds:nfs/lockd/prot_alloc.c	1.3"
/*
...
release_proclock(a)
	struct process_locks *a;
{
	struct process_locks *t;

	used_prolock--;
	for (t = proc_locks; t; t = t->next) {
		if (t->pid == a->pid) {
			t->pid = 0;
			t->lox = NULL;
			break;
...
{
	used_prolock++;
	return ((struct process_locks *) xmalloc(sizeof (struct process_locks)));
}

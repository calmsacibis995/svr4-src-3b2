...
#ident	"@(#)nfs.cmds:nfs/lockd/prot_proc.c	1.15"
/*
...
extern struct process_locks proc_lists[];
	...
	int choice;
{
	...
	if ((fp = search_fh(a, 1)) == (struct lm_vnode *)-1) {
		...
		for (i=0; i < MAXPROCESS &&
			proc_lists[i].pid != a->lck.lox.lld.l_pid; i++);
		if (i < MAXPROCESS && proc_lists[i].pid == a->lck.lox.lld.l_pid)
			a->lck.lox.MyProc = &proc_lists[i];
		else
			a->lck.lox.MyProc = get_proc_list((long)a->lck.lox.lld.l_pid);
		/*
	...
	remote_result *resp;
{
	...
	case nlm_granted:
		...
		for (i=0; i < MAXPROCESS &&
                        proc_lists[i].pid != a->lck.lox.lld.l_pid; i++);
                if (i < MAXPROCESS && proc_lists[i].pid == a->lck.lox.lld.l_pid)                        a->lck.lox.MyProc = &proc_lists[i];
                else
                        a->lck.lox.MyProc = get_proc_list((long)a->lck.lox.lld.l_pid);
		if (add_mon(a, 1) == -1)
...
a);                                                     
			...
			/*
			 * Update proc struct table
			 */
			for (t = &proc_lists[0]; t && a->lck.lox.lld.l_pid != t->pid;
				t = t->next);
			if (t)
				a->lck.lox.MyProc = t;
			else
	...
	case nlm_granted:
		if (search_fh(a, 0) == (struct lm_vnode *)NULL) { /* lock is not found */
			...
			if (t)
				...
				a->lck.lox.MyProc =
					get_proc_list((long)a->lck.lox.lld.l_pid);
			if (t && 
				blocked(t->lox->filocks, &(a->lck.lox.lld), &insrt)) {
				/* redundant, no way to avoid */
				fprintf(stderr,
					"rpc.lockd: cont_cancel: remote and local lock tbl inconsistent\n");
				release_res(resp); /* discard this response */
				return (NULL);
			}
			else {
				a->rel = 0;
				sf=insflck(&(t->lox->filocks), 
					&(a->lck.lox.lld), (struct filock *)NULL);
				return (resp);
			}
		}
		return (resp);

...
#ident	"@(#)nfs.cmds:nfs/lockd/flock.c	1.5"
/*
	...
	struct	flock	*lckdat;
{
	...
	if (debug)
		printf("enter insflck() : start %d end %d type %d\n",
			lckdat->l_start, END(lckdat), lckdat->l_type);

	if((new = (filock_t *)xmalloc(sizeof(filock_t),KM_SLEEP)) == NULL)
                return NULL;
	++flckinfo.reccnt;
	++flckinfo.rectot;
	new->set = *lckdat;
	...
	struct filock *flp;
{
	...
	if (debug)
		printf("enter regflck(): ld->l_start %d flp->set.l_start %d ld->l_end %d flp->set.l_end %d\n",
			ld->l_start, flp->set.l_start, END(ld), 
			END(&flp->set)); 
	...
	struct flock	*ld;
{
	...
	if (debug)
		printf("enter flckadj(): start %d end %d type %d\n",
			ld->l_start, END(ld), ld->l_type);
	...
	while (flp = nflp) {
		...
		if( SAMEOWNER(&(flp->set), ld) ) {
			...
			case S_MIDDLE|E_MIDDLE:
				...
				/* Does this make sense in the case 
				 * of an unlock?                  
				 */
	...
	if (debug)
		printf("ld->l_type=%d ld->l_start %d ld->l_end %d END(ld) %d\n",
			ld->l_type,ld->l_start,ld->l_end,END(ld));
	...
	struct filock **insrt;
{
	...
	if (debug)
		printf("enter blocked() : start %d end %d type %d\n",
			lckdat->l_start, END(lckdat), lckdat->l_type);

...
#ident	"@(#)nfs.cmds:nfs/statd/svc_dg.c	1.1"
/*
	...
	u_int recvsize;
{
	...
	if (t_getinfo(fd, &tinfo) == -1) {
		if ((sendsize == 0) || (recvsize == 0)) {
			syslog(LOG_ERR, "svc_dg_create: could not get transport information");
			return ((SVCXPRT *)NULL);
		}
	} else {
		/*
	...
	u_long size;
{
	...
	if (uc->uc_entries == NULL) {
		(void) syslog(LOG_ERR, "enablecache: could not allocate cache data");
	...
	if (uc->uc_fifo == NULL) {
		(void) syslog(LOG_ERR, "enablecache: could not allocate cache fifo");
	...
	u_long replylen;
{
	...
	if (victim != NULL) {
		...
		*vicp = victim->cache_next;	/* remote from cache */
		...
		if (victim == NULL) {
			(void) syslog(LOG_ERR, "cache_set: victim alloc failed");
			return;
		}
		newbuf = mem_alloc(su->su_iosz);
		if (newbuf == NULL) {
			(void) syslog(LOG_ERR, "cache_set: could not allocate new rpc_buffer");
	...
	(void) memcpy(victim->cache_addr.buf, uc->uc_addr.buf, (int)uc->uc_addr.len);
	loc = CACHE_LOC(xprt, victim->cache_xid);
...
#define EQADDR(a1, a2)	(memcmp((char*)&a1, (char*)&a2, sizeof(a1)) == 0)
	...
	(void) memcpy(uc->uc_addr.buf, xprt->xp_rtaddr.buf, (int)uc->uc_addr.len);
	return (0);
}

...
#ident	"@(#)nfs.cmds:nfs/statd/sm_svcreate.c	1.1"
/*
	...
	char *servname;
{
	...
	if ((net = _rpc_setconf(nettype)) == 0) {
		(void) syslog(LOG_ERR, "svc_create_statd: unknown protocol");
		return ((SVCXPRT *)NULL);
	}
	while (nconf = _rpc_getconf(net)) {
			...
			if (strcmp(l->xprt->xp_netid, nconf->nc_netid) == 0) {
				...
				*/
				...
				if (svc_reg(l->xprt, prognum, versnum,
					...
					(void) syslog(LOG_ERR,
					    "svc_create_statd: could not register prog %d vers %d on %s",
		...
		if (l == (struct xlist *) NULL) {
			...
			if (!netdir_getbyname(nconf, &ns, &nas)) {
				...
				syslog(LOG_ERR,
				    "svc_create_statd: no well known address for %s on transport %s\n",
				    servname, nconf->nc_netid);
				fprintf(stderr,
				    "svc_create_statd: warning: no well known address for %s on transport %s\n",
				    servname, nconf->nc_netid);

...
#ident	"@(#)nfs.cmds:nfs/nfsd/nfsd.c	1.9"
...
struct netbuf	**addr;
{
	...
	/*
	 * 	Backward compatibility:  if using UDP/IP as a transport, ensure
	 *	that we try to bind to port number NFS_PORT.
	 */
	if (NETSELEQ(nconf->nc_protofmly, NC_INET) &&
	    NETSELEQ(nconf->nc_proto, NC_UDP)) {
		if ((tb = (struct t_bind *) t_alloc(fd, T_BIND, T_ALL)) ==
		   (struct t_bind *) NULL) {
			syslog(LOG_ERR, "t_alloc failed:  t_errno %d, %m",
			   t_errno);
			(void) t_close(fd);
			return (-1);
		}
		((struct sockaddr_in *) tb->addr.buf)->sin_port = htons(NFS_PORT);
		tb->addr.len = sizeof(struct sockaddr_in);
	}
	...
	/*
	 * 	Backward compatibility:  if using UDP/IP as a transport, ensure
	 *	that we have bound to port number NFS_PORT.
	 */
	if (NETSELEQ(nconf->nc_protofmly, NC_INET) &&
	   NETSELEQ(nconf->nc_proto, NC_UDP)) {
		short bound_port = 
			   ntohs(((struct sockaddr_in*) ntb->addr.buf)->sin_port);

		if (bound_port != NFS_PORT) {
			syslog(LOG_ERR, "t_bind to wrong port (%d)", 
			      bound_port);
			(void) t_free((char *) ntb, T_BIND);
			if (tb != (struct t_bind *) NULL)
				(void) t_free((char *) tb, T_BIND);

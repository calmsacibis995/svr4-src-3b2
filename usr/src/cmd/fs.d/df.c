...
int flag;
{
	...
	switch(flag) {
	case 'g':
		printf("%-18s(%-15s):  %6d block size  %7d frag size\n", 
			mountb->mnt_mountp, 
			mountb->mnt_special,
			statbuf->f_bsize, 
			statbuf->f_frsize);
		printf("%7d total blocks%7d free blocks%7d available   %7d total files\n", 
			statbuf->f_blocks*physblks, 
			statbuf->f_bfree*physblks,
			statbuf->f_bavail*physblks,
			statbuf->f_files);
		printf("%7d free files  %7d filesys id %32s \n", 
			statbuf->f_ffree,
			statbuf->f_fsid,
			statbuf->f_fstr);
		printf("%7s fstype      %7d bitmask    %7d filename length\n\n", 
			statbuf->f_basetype,
			statbuf->f_flag,
			statbuf->f_namemax);
		break;
	...
	case 'k':
		...
		avail = statbuf->f_bfree*statbuf->f_frsize/1024;
		used = kbytes - avail;

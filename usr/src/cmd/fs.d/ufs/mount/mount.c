...
#ident	"@(#)ufs.cmds:ufs/mount/mount.c	1.14"
...
/*	(void) eatmntopt(mnt, MNTOPT_QUOTA); */
	...
	if (mount(mnt->mnt_special, mnt->mnt_mountp, flags | MS_FSS, "ufs") < 0) {
		perror("mount");
		if (print) {
			(void) fprintf(stderr, "mount: %s on ",	mnt->mnt_special);
	...
	FILE *outp;
{
	...
	while ((status = getmntent(mnttab, &mntp)) == NULL) {
		dump_mntent (&mntp);
		if (strlen(mntp.mnt_special) > maxfsname) {
			maxfsname = strlen(mntp.mnt_special);
		}
		if (strlen(mntp.mnt_mountp) > maxdir) {
			maxdir = strlen(mntp.mnt_mountp);
		}
		if (strlen(mntp.mnt_fstype) > maxtype) {
			maxtype = strlen(mntp.mnt_fstype);
		}
		if (strlen(mntp.mnt_mntopts) > maxopts) {

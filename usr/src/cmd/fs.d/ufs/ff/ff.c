...
#ident	"@(#)ufs.cmds:ufs/ff/ff.c	1.7"
	...
	pr:
		...
		if ((!Aflg || cmp((Today - dip->di_un.di_icom.ic_atime)/DAY, Adelay, Asign)) &&
			...
			if (lookup(dp->d_ino, 0))
				(void) printf("/.");
			if (Sflg)
				(void) printf("\t%6d", dip->di_un.di_icom.ic_size.val[1]);
			if (uflg)
	...
	int lev;
{
	...
	if (pflg)
		(void) fprintf(stream, "%s/%s", prefix, hp->h_name);
	else
...
usage ()
{
	(void) fprintf (stderr, "ufs usage: ff [-F ufs] [-I -a -i #list -p prefix -s -u -m mod_time -a access_time -c change_time -l] [-o m] special\n");
	exit (32);
}

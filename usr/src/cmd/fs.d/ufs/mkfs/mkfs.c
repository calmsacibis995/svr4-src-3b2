...
#ident	"@(#)ufs.cmds:ufs/mkfs/mkfs.c	1.8"
	...
	char *argv[];
{
	...
	while ((c = getopt (argc, argv, "bmo:V")) != EOF) {
		switch (c) {
...
next:
	...
	if (argc > 9) {
		...
		if (nbpi <= 0) 
			printf("%d: bogus nbpi reset to %d\n", nbpi, inos);
		else
			inos = nbpi;
	}
	...
	if (argc > 7) {
		...
		sblock.fs_minfree = minfree;
		if (sblock.fs_minfree < 0 || sblock.fs_minfree > 99) {
			printf("%d: bogus minfree reset to %d%%\n", minfree, 
				MINFREE);
			sblock.fs_minfree = MINFREE;
		}
	}
	...
	if (argc > 10)
		...
		if (opt  == 's')
			...
			printf("%c: bogus optimization preference %s\n",
				opt, "reset to time");
			sblock.fs_optim = FS_OPTTIME;
		}
	}

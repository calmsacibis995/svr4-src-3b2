...
#ident	"@(#)ufs.cmds:ufs/ufsrestore/symtab.c	1.1"
/*
	...
	char *name;
{
	...
	cp = name;
	for (ep = lookupino(UFSROOTINO); ep != NIL; ep = ep->e_entries) {
		for (np = buf; *cp != '/' && *cp != '\0'; )
			*np++ = *cp++;
		*np = '\0';
	...
	int type;
{
	...
	if (ep == NIL) {
		if (inum != UFSROOTINO || lookupino(UFSROOTINO) != NIL)
			panic("bad name to addentry %s\n", name);
		np->e_name = savename(name);
		np->e_namlen = strlen(name);
		np->e_parent = np;
		addino(UFSROOTINO, np);
		return (np);
	}

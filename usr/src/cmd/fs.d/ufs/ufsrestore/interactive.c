...
#ident	"@(#)ufs.cmds:ufs/ufsrestore/interactive.c	1.3"
	...
	if (dir) {
		...
		if (rescan) {	
			...
			bcopy((char *)&ap->head[lindex],
			     (char *)&ap->head[sindex],
			     (ap->last - &ap->head[rindexa]) * sizeof *ap->head);
...
onintr()
{
	if (command == 'i' && reset_OK)
		longjmp(reset, 1);
	if (reply("ufsrestore interrupted, continue") == FAIL)
		done(1);
}

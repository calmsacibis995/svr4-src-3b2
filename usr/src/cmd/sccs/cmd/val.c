...
static char	*getval();	/* function returning adjusted ptr to line */
...
static void	getdel(), sinit(), setkeep();
static void	process(), validate(), read_to(), report();
static int	getstats(), do_delt(), read_mod(), invalid();
void	ffree(), exit();
void	addq(), remq();
	...
	while (get_line(pkt) != NULL) {
		...
		if (*p++ != CTLCHAR)
			...
			if (iord == END)
				...
				addq(pkt,ser,iord == INS ? NO : NULL,iord,0);
		}
	}
	if (pkt->p_q)
		infile_err |= CORRUPT_ERR;

...
char **argv;
{
	...
	char	*fgets(), str[MAXL], errstr[128];
	FILE	*ifile, *fopen();
	extern	int	optind;
	extern	char	*optarg;
	...
	while ((c=getopt(argc,argv,"f:p:")) != EOF)
		...
		case 'p':
			g_path = optarg;
			if (stat(g_path, &statb) == -1) {
				sprintf(errstr,
				    "%s: can't stat specified prefix path %s",
				    g_cmd, g_path);
				perror(errstr);
				exit(1);
			} else if ((statb.st_mode & S_IFMT) != S_IFDIR) {
				fprintf(stderr, "%s: specified prefix path %s \
is not a directory\n", g_cmd, g_path);
				exit(1);
			} else {
			    break;
...
int init()
{
	...
	if ((g_lbbuf=(char *)malloc(g_bsize)) <= (char *)0) 
...
struct	Ireq	*pi;
{
	...
	int mkdir();	/* define the local definition */
	...
	case S_IFREG:
		if (!exist) {
			pave(pi);
			fp=creat(pi->fp, (mode_t)dip->di_mode);
			if (fp != -1) {
				close(fp);
				fp=0;
			}
	...
	default:
		if (!exist) {
			pave(pi);
			fp=mknod(pi->fp, (mode_t)dip->di_mode, (dev_t)dip->di_uid);
			break;
		}
	}
	if (fp == -1) {
	...
	if (chmod(pi->fp, (mode_t)dip->di_mode) < 0) {
	...
	if (chown(pi->fp, (uid_t)dip->di_uid, (gid_t)dip->di_gid)<0) {
...
register char *namep;
{
	...
	if(pid = fork())
		...
		close(2);
		execl("/usr/bin/mkdir", "mkdir", namep, 0);
		exit(2);
	}
...
daddr_t	blk;
{
	...
	if ((new = (struct Bdata *)malloc(sizeof(struct Bdata))) <= (struct Bdata *)0) 

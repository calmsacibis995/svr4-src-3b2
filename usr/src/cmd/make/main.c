...
char	viewpath[MAXLEN],	/* viewpaths live here, from env or command */
	*directs[MAXDIR];	/* pointers to all directory levels */
	...
	if (BLDCALL) {
		...
		char 	*vptr,		/* viewpath string pointer */
			*cptr, *dptr,	/* temp pointers used for viewpaths */
			temp[ARY100],	/* pathname to  .cms file built here */
			*dirname;
		...
		if (!vpthset) {	/* -v flag used already? */
			if ((vptr = getenv("VPATH")) &&
				...
				(void) cat(temp, getenv("HOME"), "/.cms", 0);
				CHKARY(main, temp, ARY100)  /*macro defined in defs*/
				if ( fid = fopen(temp, "r") ) {
					(void)fgets(viewpath, MAXLEN, fid);
					(void)fclose(fid);
					if (*viewpath)
						vpthset++;
				}
		...
		if (*viewpath) {
			...
			if ( !(cdir = getcwd((char *)NULL, MAXLEN)) )
c_error:			fatal("getcwd (bu5)");
			if (chdir(directs[1]))
				fatal("viewpath contains non-existent directory (bu6)");
			if ( !(dirname = getcwd((char *)NULL, MAXLEN)) )
				goto c_error;
			(void) chdir(cdir);
			cptr = cdir;
			dptr = dirname;
			while ((*dptr) && (*cptr) && (*cptr++ == *dptr++))
				;
			if (*dptr || (*cptr && *cptr != '/'))
			...
			for (i = 1; i <= ndir; i++) {
				while (chdir(directs[i]) && (i <= ndir)) {
					...
					for (tj = ti + 1; tj <= ndir; )
						directs[ti++] = directs[tj++];
					ndir--;
				}
				if (i > ndir)
					break;
				if (*directs[i] != '/') {
					if (!(dirname = getcwd(NULL, MAXLEN)))
						goto c_error;
					directs[i] = dirname;
				}
				(void) chdir(cdir);
				(void) cat(temp, directs[i], cptr, 0);
				CHKARY(main, temp, ARY100)  /*macro defined in defs*/
				directs[i] = copys(compath(temp));
#ifdef MKDEBUG
				if ( IS_ON(DBUG) ) 
...
char *desfnam;
{
	...
	char	descf[MAXLEN];	/* buffer for full path name of description file */
	...
	for (vdir = 1; vdir <= ndir; vdir++) {
		...
		CHKARY(opdescf, descf, MAXLEN)  /*macro defined in defs*/
		(void)compath(descf);

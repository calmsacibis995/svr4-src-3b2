	...
	if (a)
		while (*a)
			if (*a != DOLLAR)
				...
				*s = CNULL;
				CHKARY(subst, vname, ARY100)  /*macro defined in defs*/
				if (amatch(&vname[0], "*:*=*"))
...
CHARSTAR rlse;			/* pointer to release desired */
{
	...
	char	temp[MAXLEN],	/* holder for complete viewpath name */
		where[MAXLEN];	/* directory where "gotten" file goes */

	/* set up the destination directory */

	(void) cat(where, directs[1], "/", ssfile, 0);	/* get full pathname */
	CHKARY(vpget, where, MAXLEN)  /*macro defined in defs*/
	(void)compath(where);
	...
	*p = '\0';		/* terminate the directory portion */
	...
	for (ctr = 1; ctr <= ndir; ctr++) {
		...
		CHKARY(vpget, temp, MAXLEN)  /*macro defined in defs*/
		(void)compath(temp);
...
unlnk()			/* unlink linked/copied files */
{
	...
	for (lnkp = firstname; lnkp; lnkp = lnkp->nextname)
		if (lnkp->lnkdfl || (lnkp->cpydfl && IS_OFF(TOUCH))) {
			...
			CHKARY(unlnk, temp, ARY100)  /*macro defined in defs*/

...
time_t	*tval;
{
	...
	*	modification times of dependents	*/
	...
	char	sourcename[ARY100],	/* area to expand file name & path */
		prefix[ARY100], 
		temp[ARY100],
	...
	for (lp = sufflist; lp; lp = lp->nextline)
		...
		for (suffp = lp->depp ; suffp ; suffp = suffp->nextdep) {
			...
			if (suffix(p->namep , pnamep , prefix)) {
				CHKARY(doname, prefix, ARY100)  /*macro defined in defs*/
#ifdef MKDEBUG
				if (IS_ON(DBUG)) {
					blprt(reclevel);
...
searchdir:			
				...
				CHKARY(doname, temp, ARY100)  /*macro defined in defs*/
	...
	/* concatenate suffixes */		(void)concat(p1namep, pnamep, concsuff);
						...
						CHKARY(doname, sourcename, ARY100)  /*macro defined in defs*/
...
endloop:
	if ( (!found &&			/* if not found AND */
		...
		(	!p->linep ||		/* no lines for target OR */
			(!p->linep->depp &&	/* no dep action AND */
			!p->linep->shp))) ) {	/* no shell action */
	...
	if ( rebuild && !errstat ) {
		...
		if (IS_ON(DBUG))
			printf("library: p->alias, access(p->alias,0), ptime: %s %d %ld\n",
				p->alias, access(p->alias, 0), ptime);
#endif
...
NAMEBLOCK np;
{
	...
	temp_size = strlen(temp_string) + strlen(p) + 2;
...
DEPBLOCK fname;
{
	...
	for (fnomad = fname; fnomad; fnomad = fnomad->nextdep) {
		...
		if (depp->flevel > 1) {
			...
			CHKARY(lnk, temp, ARY100)  /*macro defined in defs*/
			if (access(compath(temp), 04)) {
				(void) cat(pname, directs[depp->flevel], "/",
				    temp, 0);
				CHKARY(lnk, pname, ARY100)  /*macro defined in defs*/
				if (is_sccs(name)) {
cp_it:					depp->cpydfl = 1;
...
char	*oldfile, *newfile;
{
	...
	char	buf[BUFSIZ], nfile[ARY100], ofile[ARY100];
	struct stat statbuf;
	...
	CHKARY(cp, ofile, ARY100)  /*macro defined in defs*/
	(void)compath(ofile);
	(void)cat(nfile, newfile, 0);
	CHKARY(cp, nfile, ARY100)  /*macro defined in defs*/
	(void)compath(nfile);
#ifdef MKDEBUG
	if (IS_ON(DBUG)) {
...
register CHARSTAR s;
{
	...
	char	s1[ARY100], s2[ARY100];

	if (p->lnkdfl) {
		(void) cat(touch_t, s, 0);
		(void)dname(touch_t);
		(void) cat(touch_t, "BtXXXXXX", 0);
		CHKARY(touch, touch_t, ARY100)	/*macro defined in defs*/
		(void)mktemp(touch_t);
		if (link(s, touch_t)) {
bad:			fprintf(stderr, "cannot touch %s (bu25)", s);
		...
		CHKARY(touch, s1, ARY100)	/* macro defined in defs */
		p->cpydfl = 1;
		if (cp(s1, s)) {
			p->cpydfl = 0;
			goto bad;

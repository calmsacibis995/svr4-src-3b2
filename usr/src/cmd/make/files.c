...
DEPBLOCK nextdbl;       /* final value for chain */
{
	...
	char	temp[ARY100], temp2[ARY200];
	char	pattemp[MAXLEN], dirtemp[MAXLEN];
	...
	CHKARY(srchdir, pattemp, MAXLEN)  /*macro defined in defs*/
	(void)copstr(dirtemp, pat);
	CHKARY(srchdir, dirtemp, MAXLEN)  /*macro defined in defs*/
	(void)dname(dirtemp);
	if (STREQ(dirtemp, ".")) {
		dirpref = "";
		dirname = "";
		...
		CHKARY(srchdir, temp, ARY100)  /*macro defined in defs*/
		dirname = concat("/", dirtemp, temp2);
		CHKARY(srchdir, temp2, ARY200)  /*macro defined in defs*/
	}
	...
	while((entry = readdir(dirf)) != NULL) {
			if (entry->d_ino) {
				...
				if ( amatch(n15, filepat) ) {
					(void)concat(dirpref, n15, fullname);
					CHKARY(dodir, fullname, ARY100)  /*macro defined in defs*/
...
char *dirname;
{
	...
	cat(temp, dirname, 0);
	CHKARY(getfid, temp, ARY100)  /*macro defined in defs*/
	(void) compath(temp);

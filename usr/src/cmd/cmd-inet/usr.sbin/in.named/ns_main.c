...
#ident	"@(#)cmd-inet:usr.sbin/in.named/ns_main.c	1.4"
	...
	char *argv[], *envp[];
{
	...
	*/
	...
	ifc.ifc_buf = buf;
	if (ifioctl(vs, SIOCGIFCONF, (char *)&ifc) < 0) {
	...
	for (ifr = ifc.ifc_req; n > 0; n--, ifr++) {
		...
		if (ifr->ifr_addr.sa_family != AF_INET)
			continue;
		ifreq = *ifr;
		if (ifioctl(vs, SIOCGIFFLAGS, (char *)&ifreq) < 0) {
			syslog(LOG_ERR, "get interface flags: %m");
			continue;
		}
		if ((ifreq.ifr_flags & IFF_UP) == 0)
			continue;
		if (ifioctl(vs, SIOCGIFADDR, (char *)&ifreq) < 0) {
		...
		if (ifioctl(vs, SIOCGIFNETMASK, (char *)&ifreq) < 0) {
...
#endif
		{
			...
			if (ifioctl(vs, SIOCGIFDSTADDR, (char *)&ifreq) < 0) {

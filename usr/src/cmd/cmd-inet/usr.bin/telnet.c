...
#ident	"@(#)cmd-inet:usr.bin/telnet.c	1.7"
...
ttyflush()
{
	...
	if (!(SYNCHing||flushout)) {
	    ...
	    if (ioctl(fileno(stdout), TIOCFLUSH, (char *) 0) == -1)
...
getconnmode()
{
    ...
    if (remopts[TELOPT_ECHO]) {
	modeindex += 2;
    }
    if (remopts[TELOPT_SGA]) {
	modeindex += 4;
    }
...
telnet()
{
	...
	if (ioctl(net, FIONBIO, (char *)&on) == -1)
		perror ("ioctl FIONBIO");
...
telrcv()
{
	...
	while ((scc > 0) && (TTYROOM() > 2)) {
		...
		case TS_WILL:
			printoption(">RCVD", will, c, !remopts[c]);
			if (c == TELOPT_TM) {
				if (flushout) {
					flushout = 0;
				}
		...
		case TS_WONT:
			printoption(">RCVD", wont, c, remopts[c]);
			if (c == TELOPT_TM) {
				if (flushout) {
					flushout = 0;
				}
...
bye()
{
	...
	if (connected) {
		...
		close(net);
		connected = 0;
		/* reset remote options */
		for (op = remopts; op < &remopts[256]; op++)
			*op = 0;
	}
	return 1;
}
	...
	char *argv[];
{
	...
	sin.sin_port = htons(IPPORT_TELNET);
	if (argc == 3) {
	...
	char *argv[];
{
	...
	if (ioctl(0, TIOCGLTC, (char *)&oltc) == -1)
		perror ("ioctl TIOCGLTC");
#if	defined(LNOFLSH)
	if (ioctl(0, TIOCLGET, (char *)&autoflush) == -1)

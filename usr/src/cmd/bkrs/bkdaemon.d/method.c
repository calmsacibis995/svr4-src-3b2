...
#ident	"@(#)bkrs:bkdaemon.d/method.c	1.13"
...
method_t *method;
{
	...
	options |= owner->options
		& (S_AUTOMATED|S_ESTIMATE|S_NO_EXECUTE|S_SEND_DOTS|S_SEND_FILENAMES);
...
bkrotate_t date;
{
...
#ifdef TRACE
	brlog( "md_match(): tid %d eptr 0x%x dot %d", tid, eptr, *dot );
#endif
	...
	for( entryno = *dot; ; entryno++ ) {
		...
		case TLOK:
			...
			if( fname && *fname && strcmp( (char *)fname, (char *)entry.oname ))
				continue;

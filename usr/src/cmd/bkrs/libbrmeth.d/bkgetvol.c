...
#ident	"@(#)bkrs:libbrmeth.d/bkgetvol.c	1.1"
...
static char	insert(), *chkdevice(), *getattr();
...
{
	FILE	*tmp;
	char	*advice, *ptr, *type, buffer[ PATH_MAX + 1 ];
	long capacity = 0;
	...
	if( !(device = chkdevice( device, label, type, buffer ) ) )
	...
	if( (options & DM_AUTO) && result ) {
		switch( GET_LABEL( device, dchar, result, result_sz ) ) {
		case -1:
			break;
	...
	for( ptr = prompt_txt; ; ptr = sec_prompt ) {
		if( !(options & DM_BATCH) && volume ) {
			...
			case 'f':
				/* FORMAT */
				/*
					if override is allowed, we could remember the label
					on the volume in case override is selected later.
					But, it is probably not worth it.  (Also consider
					what happens if the user types format,format,override --
					there is no way to tell if both formats apply to the
					same volume or not.)
				*/
				if( !(options & DM_OLABEL) && (options & DM_CHKLBL) ) {
					/*
						If override is not allowed, do not format the
						'wrong' volume!
					*/
					switch( GET_LABEL( device, dchar, result, result_sz ) ) {
					case -1:
		...
		if((tmp = fopen( device, "r")) == NULL) {
			/* device was not accessible */
			if( !volume || (options & DM_BATCH) )
				return( DMR_BADDEVICE );

			(void) fprintf(stderr, ERR_ACCESS, pname, device);
			if(advice = getattr( device, "advice"))

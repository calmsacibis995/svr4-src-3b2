...
#ident	"@(#)bkrs:bkreg.d/bkreg.c	1.14"
...
struct disp_fld {
	...
	R_DAY, (unsigned char *)"days", 5,
		(unsigned char *)"Days",
	R_DCHAR, (unsigned char *)"dchar", 18,
		(unsigned char *)"Dcharacteristics",
	R_DDEVICE, (unsigned char *)"ddevice", 15,
...
char *argv[];
{
	...
	if( prtflag & !( flags & (CFLAG | AFLAG | OFLAG) ) )
		if( unlink( table ) < 0 )
...
synopsis()
{
	...
	(void)fprintf( stderr,
		"%s -a tag -o orig -c weeks:days -m method -d ddev [-t table] [-b moptions] [-D depend] [-P prio]\n",
		brcmdname );
	(void)fprintf( stderr,
...
valid_dmnames()
{
	...
	while( name != NULL ){
		if( strlen( (char *)name ) > BKLABEL_SZ ) return( FALSE );
...
sort_reg()
{
	...
	(void)sprintf( tmpfile, "/tmp/bkreg%d", getpid() );
	...
	(void)sprintf( cmdstr, "sort -b -t: +2 -3 %s | grep -v '^\#' >> %s", table, tmpfile );
	if( system( cmdstr ) < 0 ) {
		bkerror( stderr, ERROR16, table, errno );
		(void)unlink( tmpfile );
		exit( 2 );
...
ENTRY entry;
{
	...
	for( i = 0; i < fld_cnt; i++) {
		...
		if ( strcmp( (char *)fields[fldindex].fldname, "cweek" ) == 0 )
			...
			TLgetfield( tid, entry, fields[fldindex].fldname),
			(i < fld_cnt-1) ? fs : '\n' );
	}
}
		...
		for( i = 0; i < fld_cnt; i++ ) {
			...
			if( i == 0 )
				...
				(void)fprintf( stdout, " %-*.*s", flen, flen,
					next_char[i] );
			if( *next_char[i] != NULL ) {
				next_char[i] += min( strlen( (char *)next_char[i] ),
							flen);
				if( *next_char[i] != NULL )
					done = FALSE;
			}

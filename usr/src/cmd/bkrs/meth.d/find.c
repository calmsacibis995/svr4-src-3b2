...
#define A_DAY 86400L	/* a day of seconds */
	...
	if (total_bytes) {	/* include trailer for estimates */
		total_bytes += (CHARS + strlen("TRAILER!!!") + 1);
	}
...
print()
{
	...
	if ((Statb.st_mode&S_IFMT) == S_IFREG) {
		total_bytes += (Statb.st_size + CHARS + strlen(Pathname) + 1);
	}
	else {
		total_bytes += (CHARS + strlen(Pathname) + 1);
	}
	return(1);
} /* print() */
...
#ifdef TRACE
		brlog("st_dev=%lx cur_dev=%lx",Statb.st_dev, cur_dev);
#endif
		return;
	}

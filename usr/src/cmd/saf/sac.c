 ...
 */
	...
	if ((ret = doconfig(-1, SYSCONFIG, 0)) != 0) {
		if (ret == -1)
			...
			(void) sprintf(Scratch, "Error in _sysconfig: line %d", ret);
			log(Scratch);
 ...
 */
				...
				(void) sprintf(Scratch, "Could not open _pmpipe for port monitor <%s>", sp->sc_tag);
				log(Scratch);
...
/*
 * readutmp - read the utmp file to find out the ids of running port
			...
			for (sp = Sactab; sp; sp = sp->sc_next) {
				if (!strncmp(up->ut_name, sp->sc_tag, NAMELEN)) {
					if (savesp) {
						...
						savesp = (struct sactab *) NULL;
						(void) sprintf(Scratch, "ambiguous utmp entry <%.8s>", sp->sc_tag);
						log(Scratch);
						break;
					}
					else {
...
# endif
	if (checklock(sp)) {
		(void) sprintf(Scratch, "could not start <%s> - _pid file locked", sp->sc_tag);
		log(Scratch);
		return(-1);
	}
	...
	if (access(Scratch, 0) != 0) {
		...
		if (mknod(Scratch, S_IFIFO | 0600, 0) < 0) {
			(void) sprintf(Scratch, "Could not create _pmpipe for port monitor <%s>, errno is %d", sp->sc_tag, errno);
			log(Scratch);
			return(-2);
		}
	}
	sp->sc_fd = open(Scratch, O_RDWR);
	if (sp->sc_fd < 0) {
		(void) sprintf(Scratch, "Could not open _pmpipe for port monitor <%s>, errno is %d", sp->sc_tag, errno);
		log(Scratch);
		return(-2);
	}
	...
	if ((pid = fork()) < 0) {
		(void) sprintf(Scratch, "Could not fork port monitor <%s>", sp->sc_tag);
		log(Scratch);
		return(-2);
	}
	else if (!pid) {
...
/*
 * cleanut - clean out a utmp record for a port monitor
 *
 *	args:	sp - pointer to sac's port monitor information for
 *		     designated port monitor
 */
 ...
 */
	...
	if ((up = getutid(up)) == NULL) {
		(void) sprintf(buf, "missing utmp entry for <%s>", sp->sc_tag);
		log(buf);
	}
	else {
		...
		if (modut(up) == NULL) {
			(void) sprintf(buf, "could not remove utmp entry for <%s>", sp->sc_tag);
			log(buf);
		}
	}
	endutent();
 ...
 */
	...
	if (chdir(sp->sc_tag) < 0) {
		(void) sprintf(Scratch, "Cannot chdir to <%s/%s>, port monitor not started", HOME, sp->sc_tag);
		log(Scratch);
		exit(1);
	}
 ...
 */
	...
	if ((ret = doconfig(-1, "_config", 0)) != 0) {
		if (ret == -1) {
			(void) sprintf(Scratch, "system error in _config script for <%s>", sp->sc_tag);
			log(Scratch);
			exit(1);
		}
		else {
			(void) sprintf(Scratch, "Error in _config script for <%s>: line %d", sp->sc_tag, ret);
			log(Scratch);
			exit(1);
		}
	}
 ...
 */
	...
	if (putenv(istate)) {
		(void) sprintf(Scratch, "can't expand port monitor <%s> environment", sp->sc_tag);
		log(Scratch);
		exit(1);
	}
 ...
 */
				...
				for (;;) {
					if (*p == '\0') {
						(void) sprintf(Scratch, "invalid command line, non-terminated string for port monitor %s",
							sp->sc_tag);
						log(Scratch);
						exit(1);
					}
 ...
 */
		...
		if (sp->sc_rscnt < sp->sc_rsmax) {
			/* try to restart it */
			if (reason == RESP)
				(void) sprintf(buf, "<%s> stopped responding to sanity polls - trying to restart", sp->sc_tag);
			else
				(void) sprintf(buf, "<%s> has died - trying to restart", sp->sc_tag);
			log(buf);
			sp->sc_rscnt++;
			(void) close(sp->sc_fd);
			(void) startpm(sp);
 ...
 */
		...
		switch (pp->pm_type) {
		case PM_UNKNOWN:
			(void) sprintf(Scratch, "port monitor <%s> didn't recognize message", sp->sc_tag);
			log(Scratch);
			/* fall through */
		case PM_STATUS:
			/*
			...
			if (!validstate(pp->pm_state)) {
				pp->pm_state = UNKNOWN;
				(void) sprintf(Scratch, "port monitor <%s> reporting invalid state", sp->sc_tag);
				log(Scratch);
			}
			if (sp->sc_sstate == sp->sc_pstate) {
			...
			break;
		default:
			(void) sprintf(Scratch, "port monitor <%s> sent an invalid message - ignoring it", sp->sc_tag);
			log(Scratch);
			break;
		}

...
char	**av;
{
	...
	struct vtoc	vtoc;
	char		bdev[80];
	static int	swp_sz; /* Limit swap size for temp filsys boot */
	...
	if ((partno = findswap(vtoc.v_part, vtoc.v_nparts, &swp_sz)) < 0)
		fatal("No swap partition on device", av[1]);
	bdev[strlen(bdev) - 1] = "0123456789abcdef"[partno];
	if (sys3b(S3BSWAP, bdev, 0, swp_sz) < 0)
		fatal(syserr(), bdev);
	exit(0);
}
 ...
 */
static int
findswap(ptab, nparts, swp_szptr)

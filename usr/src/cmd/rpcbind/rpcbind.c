	...
	struct netconfig *nconf;	/* Transport provider info */
{
	...
	struct t_info tinfo;
	int bounds_checking;	/* bound checking ? */
	...
	if ((fd = t_open(nconf->nc_device, O_RDWR, &tinfo)) < 0) {
		syslog(LOG_ERR, "%s: bind check cannot open connection: %s",
				nconf->nc_netid, t_errlist[t_errno]);
	} else {
		if (t_bind(fd, taddr, baddr) != 0) {
			syslog(LOG_ERR, "%s: bind check cannot bind: %s",
				nconf->nc_netid, t_errlist[t_errno]);
		} else {
			if (!memcmp(taddr->addr.buf,
				baddr->addr.buf, (int)baddr->addr.len)) {
				syslog(LOG_ERR, "%s: no bounds checking",
					nconf->nc_netid);
			} else {
	...
	add_bndlist(nconf, bounds_checking);
	return (0);
error:
	(void) t_free((char *)taddr, T_BIND);
	(void) t_free((char *)baddr, T_BIND);

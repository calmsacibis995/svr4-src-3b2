...
#ident	"@(#)initpkg:./shutdown.sh	1.8.3.2"
...
if [ -n "${askconfirmation}" -a -x /usr/sbin/ckbupscd ]
then
	#	Check to see if backups are scheduled at this time
	BUPS=`/usr/sbin/ckbupscd`
	if [ "$BUPS" != "" ]
		then
		echo "$BUPS"
		echo "Do you wish to abort this shutdown to do these backups? [y, n] \c"
		read YORN
		if [ "$YORN" = "y" -o "$YORN" = "Y" ]
		then 
			exit 1

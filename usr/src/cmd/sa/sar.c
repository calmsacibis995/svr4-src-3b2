...
/*
	...
	usage: sar [-ubdycwaqvmpgrkxACDS] [-o file] t [n]    or
	       sar [-ubdycwaqvmpgrkxACDS][-s hh:mm][-e hh:mm][-i ss][-f file]
*/
#include <stdio.h>
#include <sys/param.h>
#include <ctype.h>
...
int	lines=0;
#if	u3b2 || u3b15
double 	magic = 4.294967296e9;	/* 2^32 (a long on 3b2 is 32 bits) */
int	hz;
#endif
#ifndef u370
...
time_t	ts, te;			/* time interval start and end */
...
/*      process options with arguments and pack options 
	...
	if(strchr(options, 'A') != NULL) {
		strcpy(options, "uUdqbwcayvmpgrkxCS");
		dflg++;
	}
	else if ( (dflg)

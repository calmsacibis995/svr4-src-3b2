	...
	for (drive = 0; drive < nsub; ++drive) {
		...
		for (slice = 0; slice < PARTCNT; ++slice) {
			if (!haveblk && mknod(name = devxblk(ctl, drive, slice),
			    S_IFBLK | ACCESS,
			    makedev(maj, (minor_t)drive * PARTCNT + slice)) < 0)
				warn(name, syserr());

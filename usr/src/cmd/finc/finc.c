...
long	atim,		/* abs(number) of days from -a option */
	mtim,
	ctim,		/* "         "           "  -c option */
	ntim,
...
char **argv;
{
	...
	while((c=getopt(argc,argv,"da:m:c:n:")) != EOF)
		...
		case 'a':
			as = *optarg;
			if ((atim = atoi(optarg)) == 0) 
				errorx("bad value %s for option a\n",optarg);
			++criteria;
			break;
		case 'm':
			ms = *optarg;
			if ((mtim = atoi(optarg)) == 0) 
				errorx("bad value %s for option m\n",optarg);
			++criteria;
			break;
		case 'c':
			cs = *optarg;
			if ((ctim = atoi(optarg)) == 0) 
...
datacopy()
{
	...
	if ((g_obuf=(char *)malloc(g_bsize*OBLKFACT)) == 0)
...
struct dinode *dip;
{
	...
	if (atim) 
		ok += (scomp((int)((today-dip->di_atime)/A_DAY),atim,as)) ? 1 : 0;

	if (mtim) 
		ok += (scomp((int)((today-dip->di_mtime)/A_DAY),mtim,ms)) ? 1 : 0;

	if (ctim) 
		ok += (scomp((int)((today-dip->di_ctime)/A_DAY),ctim,cs)) ? 1 : 0;

	if (ntim) 
		ok += (dip->di_mtime > ntim) ? 1 : 0;

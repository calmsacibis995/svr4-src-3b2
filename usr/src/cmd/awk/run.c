...
#ifdef	NOFILE
#define	MAXFILES	(NOFILE-5)	/* max number of open files, from sys/param.h */
#else
#define	MAXFILES	15
#endif
...
format(buf, bufsize, s, a) uchar *buf, *s; int bufsize; Node *a;
{
	...
	while (*s) {
		...
		*t = '\0';
		...
		a = a->nnext;
		switch (flag) {
		case 1:	sprintf((char *)p, fmt, getfval(x)); break;
		case 2:	sprintf((char *)p, fmt, (long) getfval(x)); break;
		case 3:	sprintf((char *)p, fmt, (int) getfval(x)); break;
		case 4:	sprintf((char *)p, fmt, getsval(x)); break;

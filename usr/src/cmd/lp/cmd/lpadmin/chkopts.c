...
#ident	"@(#)nlp:cmd/lpadmin/chkopts.c	1.16"
...
#define BADILIST(X) (lenlist(X) > 1 || *X && !STREQU(NAME_SIMPLE, *X))
		if (
			I && BADILIST(I)
	...
	*/
	...
	if (oldp) {
		...
		if (!s)
			...
			else if (!H || (lenlist(H) == 1 && STREQU(H[0], NAME_NONE)))
				H = getlist(DEFMODULES, LP_WS, LP_SEP);
#endif
		...
		if (!H) {
			...
			if (lenlist(H) == 1 && STREQU(H[0], NAME_NONE))
				H = getlist(DEFMODULES, LP_WS, LP_SEP);
#endif

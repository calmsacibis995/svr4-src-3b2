...
usage:
	printf("usage: reject [-r [reason]] dest ...\n");
	cleanup();	
	exit(argc == 1);
    }
...
char **argv;
{
    ...
    if(argc == 1 || STREQU(argv[1], "-?")) {
usage:
	printf("usage: disable [-c|-W] [-r [reason]] printer ...\n");
	cleanup();
	exit(argc == 1);
    }

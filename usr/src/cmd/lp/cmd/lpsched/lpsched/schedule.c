...
#ident	"@(#)nlp:cmd/lpsched/schedule.c	1.15.1.2"
...
#endif
{
	...
	for (prs = Request_List; prs; prs = prs->next) {
schedlog ("REX_NOTIFY %s? %s %soriginating, RS_NOTIFY %sset, RSS_SENDREMOTE %sset\n", pss->system->name, prs->secure->req_id, (ORIGINATING_AT(prs, pss)? "" : "not "), (prs->request->outcome & RS_NOTIFY? "" : "not "), (prs->status & RSS_SENDREMOTE? "" : "not "));
		if (

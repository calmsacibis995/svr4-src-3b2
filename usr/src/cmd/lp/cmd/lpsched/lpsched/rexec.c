...
#ident	"@(#)lp:cmd/lpsched/lpsched/rexec.c	1.1"
...
#endif
{
	...
	if (!ep->md && !(ep->flags & EXF_WAITCHILD)) {
		...
		ep->type = type; /* so as to pick up where we left off */
	...
	case REX_STATUS:
		...
		case S5_PROTO:
			...
			rex_send_job (pss, 0, "", msgbuf);
			putmessage (msgbuf, S_INQUIRE_REQUEST_RANK, "", "", "", "", "");
			rex_send_job (pss, 0, "", msgbuf);
			break;

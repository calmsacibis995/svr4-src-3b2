...
#ident	"@(#)nlp:cmd/lpsched/pickfilter.c	1.2.1.2"
	...
	*pp = 0;	/* null terminated list! */
	...
	if (ret == fl_none && !must_have_filter) {
		...
		if (
			...
			prs->printer_type = strdup(prs->request->input_type);
			 ...
			 */
			prs->printer_type = strdup(*(pc->printer_types));
...
#define	NZ(X)	((X)? *(X) : (char *)0)
		...
		if (chk) {
			 ...
			 */
#define DROP(P,R)	if (P) { P[-1] = P[0] = hole; R = 0; } else

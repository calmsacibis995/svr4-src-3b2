...
#ident	"@(#)nlp:cmd/lpsched/exec.c	1.13.1.2"
...
#endif
{
	...
	case EX_INTERF:
		 ...
		 * here, sorry!
		 *
		 * Use the user name from the request structure, as that's
		 * who the person claimed to be.
		 */
		cp = strchr(request->request->user, BANG_C);
		sprintf (
			argbuf,
			"%s/%s %s %s%s%s \"%s\" %d \"",
			Lp_A_Interfaces,

...
#ident	"@(#)nlp:cmd/lpsched/disp3.c	1.10.1.1"
# include	"dispatch.h"
...
#endif
{
	...
	if (pps->form) {
		...
		/*
		 * If unmounting the old form leaves it mounted nowhere,
		 * maybe an alert is due.
		 */
		if (
			!Opfs->mounted
		     && Opfs->requests
		     && Opfs->requests >= Opfs->trigger
		)
			alert (A_FORM, Opfs);
	}
	...
	if (pfs) {
		 ...
		 * mounted form. We only need to get one request printing,
		 * because it completing will cause the next request
		 * to be found.
		 */
		if (pfs->requests) {
			form_in_question = pfs;
			queue_attract (pps, qchk_form);
		}
...
#endif
{
	...
	if (pps->pwheel_name) {
		...
		/*
		 * If unmounting the old print wheel leaves it mounted
		 * nowhere, maybe an alert is due.
		 */
		if (
			Oppws
		     && !Oppws->mounted
		     && Oppws->requests
		     && Oppws->requests >= Oppws->trigger
		)
			alert (A_PWHEEL, Oppws);
	}
	...
	if (pwheel_name) {
		...
		if (ppws = search_pwtable(pwheel_name)) {
			...
			/*
			 * Attract the first request that needs this newly
			 * mounted print wheel. We only need to get one
			 * request printing, because it completing will
			 * cause the next request to be found.
			 */
			if (ppws->requests) {
				pwheel_in_question = pwheel_name;
				queue_attract (pps, qchk_pwheel);
			}

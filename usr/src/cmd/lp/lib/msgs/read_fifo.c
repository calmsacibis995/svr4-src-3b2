...
#ident	"@(#)nlp:lib/msgs/read_fifo.c	1.10"
/* LINTLIBRARY */
...
int	fifo;
{
	...
	*/
	...
	memmove(save, psave, psave_end - psave);

	psave_end = save + (psave_end - psave);
...
#define UNSYNCHED_READ(N) \
    if (psave_end - psave < N || psave >= psave_end) \
    { \
	switch (_buffer(fifo)) \
	{ \
	    case -1: \

...
int slot;
{
	...
	if(active)
	{
		if(sys3b(RDUBLK, slot_to_pid(slot), (char *)ubp, (USIZE*NBPC))==-1)
			return(-1);
		else
			return(0);
	}
...
char type;
{
	...
	switch(type) {
		case 'n' : kpcbp = (struct kpcb *)(((long)ubp->u_pcbp - 
			   ...
			   break;
		case 'u' : kpcbp = (struct kpcb*)&ubp->u_ipcb;
			   break;
...
int proc;
{
	...
	if((proc == -1) || (proc == getcurproc())){
		...
		if(((long)ubp->u_stack <= (unsigned)UBADDR)  ||
			((long)ubp->u_stack >= (unsigned)USTKADDR))
...
int proc;
{
	...
	if((proc == -1) || (proc == getcurproc())){
		...
		if(panicstr == 0)
			error("information to process stack for current process not available\n");
		if((long)ubp->u_stack < (unsigned)USTKADDR)
...
int proc,kfpset;
{
	...
	if((proc == -1) || (proc == getcurproc())){
		...
		readmem((long)procp,1,proc,(char *)&procbuf,sizeof procbuf,
			"process table");
		if(sys3b(RDUBLK, slot_to_pid(proc), 
		  (char *)stk_bptr, NBPP*USIZE)==-1)
		{
			prerrmes("Invalid process\n");
			return;
		}

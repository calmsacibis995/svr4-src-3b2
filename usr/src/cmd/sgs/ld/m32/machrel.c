...
#define	IS_STYPE(X) (byte_swap_type[(X)] == 1)
...
#define BUILD_RELOC(osect, rtype, ndx, off, rloc)	{\
	Rel	*relbits = (Rel *)(Elf_Void *)((char*) ((osect)->os_outrels->is_rawbits->d_buf)+*(rloc));\
	relbits->r_info = ELF_R_INFO((ndx),(rtype));\
...
#define	PUT4(V, P)	{ (P)[0] = ((V) >> 24); \
			  (P)[1] = ((V) >> 16); \
			  (P)[2] = ((V) >> 8); \
			  (P)[3] = ((char)(V)); }

#define	GET4_S(P)	((long)(((unsigned long)(P)[3] << 24) | \
			((unsigned long)(P)[2] << 16) | \
			((unsigned long)(P)[1] << 8) | \
			(unsigned long)(P)[0]))

#define	PUT4_S(V, P)	{ (P)[3] = ((V) >> 24); \
			  (P)[2] = ((V) >> 16); \
			  (P)[1] = ((V) >> 8); \
			  (P)[0] = ((char)(V)); }
...
#define INST_JMP	0x24
#define INST_PUSHW	0xa0

/* operand descriptors */

#define OPND_IMM	0x4f	/* word immediate &foo	    */
#define OPND_ABS	0x7f	/* absolute */
#define OPND_ABSDEF	0xef	/* absolute deferred */
#define OPND_PCREL	0x8f	/* word displacement n(%pc) */
#define OPND_PCRELDEF	0x9f	/* deferred word displacement *n(%pc) */
...
static CONST unsigned char pc_rel_type[R_M32_NUM] = { 	0,	/* R_M32_NONE		*/
							0,	/* R_M32_32		*/
							0,	/* R_M32_32_S		*/
							1,	/* R_M32_PC32_S		*/
							1,	/* R_M32_GOT32_S	*/
...
static CONST unsigned char byte_swap_type[R_M32_NUM] = { 0,	/* R_M32_NONE		*/
							0,	/* R_M32_32		*/
							1,	/* R_M32_32_S		*/
							1,	/* R_M32_PC32_S		*/
							1,	/* R_M32_GOT32_S	*/
	...
	Word	value;
{
	...
	switch (rtype) {
	case R_M32_NONE:
		DPRINTF(DBG_RELOC, (MSG_DEBUG, "reloc case R_M32_NONE"));
		break;
	case R_M32_32:
		DPRINTF(DBG_RELOC, (MSG_DEBUG,
				    "reloc case R_M32_32: value %#x, memloc %#x",
				    value, memloc));
		uvalue = GET4(memloc);
		uvalue += value;
		PUT4(uvalue, memloc);
	...
	case R_M32_PLT32_S:
		DPRINTF(DBG_RELOC, (MSG_DEBUG,
			    "reloc case R_M32_[PC]32_S: value %#x, memloc %#x",
			    value, memloc));
		uvalue = GET4_S(memloc);
		uvalue += value;
		PUT4_S(uvalue, memloc);
		break;
	default:
		lderror(MSG_FATAL,
 ...
 *
 *	The got_off@GOT entry gets filled with the address of the PUSHW,
	...
	Word	rel_off;
{
	...
	pent[0] = INST_JMP;
	pent[1] = OPND_PCRELDEF;
	pent += 2;
	PUT4_S((Word)((got_sect->is_newVAddr + got_off) - (plt_sect->is_newVAddr + plt_off)), pent);
	pent += 4;
	...
	plt_off = -(plt_off + 12);	/* JMP, PUSHW take 6 bytes each */
	pent[0] = INST_JMP;
	pent[1] = OPND_PCREL;
	pent += 2;
	PUT4_S((long)plt_off, pent);
	...
	Os_desc	*osp;
{
	...
	for ( ; reloc < rend; ++reloc) {
		...
		/* dynamic mode */
		if (ELF_ST_BIND(sym->ls_syment->st_info) == STB_LOCAL) {
			if (!IS_PC_RELATIVE(rtype)) {
				if (Gflag && !noload)
					osp->os_szoutrel += sizeof(Rel);
			}
			else if (rtype == R_M32_GOT32_S) {
				if (!sym->ls_GOTndx) {
					sym->ls_GOTndx = countGOT++;
					if (Gflag)
						grels += sizeof(Rel);
		...
		if ((sym->ls_deftag == REF_DEFN) || 
			...
			if (Gflag || bflag || 
				...
				|| (ELF_ST_TYPE(sym->ls_syment->st_info) == STT_NOTYPE)) {
				osp->os_szoutrel += sizeof(Rel);
			} else {
			 ...
			 */
				...
				if (ELF_ST_TYPE(sym->ls_syment->st_info) 
					== STT_FUNC) {
					if (!sym->ls_PLTndx) {
						sym->ls_PLTndx = countPLT++;
						sym->ls_PLTGOTndx = countGOT++;
				...
				else if ((ELF_ST_TYPE(sym->ls_syment->st_info) == STT_OBJECT) &&
...
fillin_gotplt()
{
	...
	if (dmode && got_sect) {
		Ldsym *sym;
		if((sym = sym_find(DYN_USYM, NOHASH)) == NULL)
			lderror(MSG_SYSTEM,"internal error:  `%s` not found in ld symbol table",DYN_USYM);
	 ...
	 * the first entry is:
	 *	PUSHW	got[1]@GOT(%pc) # the address of the link map entry
	 *	JMP	*got[2]@GOT(%pc)	# the address of rtbinder
	 */
	if (dmode && plt_sect) {
		unsigned char	*pent;
		unsigned long	plt_addr; /* distance between the plt entry and the GOT */
           	...
           	plt_addr = plt_sect->is_newVAddr + (PLT_XRTLD * PLTENTSZ);

                pent[0] = INST_PUSHW;
		pent[1] = OPND_PCREL;
		pent += 2;
		PUT4_S((Word)((got_sect->is_newVAddr + GOT_XLINKMAP * GOTENTSZ)
                        - plt_addr), pent);
		pent += 4;
		plt_addr += PLT_INST_SZ;

                pent[0] = INST_JMP;
		pent[1] = OPND_PCRELDEF;
		pent += 2;
		PUT4_S((Word)((got_sect->is_newVAddr + GOT_XRTLD * GOTENTSZ)
                        - plt_addr), pent);
	}
	...
	Os_desc	*osect;
{
	...
	for ( ; reloc < rend; ++reloc) {
		...
		if (ELF_ST_BIND(rldsym->ls_syment->st_info) == STB_LOCAL &&
		...
		if (rflag) {
			if( (ELF_ST_BIND(rldsym->ls_syment->st_info) == STB_LOCAL)
			&& (ELF_ST_TYPE(rldsym->ls_syment->st_info) == STT_SECTION) ) {
		...
		if (ELF_ST_BIND(rldsym->ls_syment->st_info) == STB_LOCAL) {
			if (ELF_ST_TYPE(rldsym->ls_syment->st_info) == STT_SECTION) {
				 ...
				 */
				ovalue = value;
				value = (Word)(got_sect->is_newVAddr) + (rldsym->ls_GOTndx * GOTENTSZ);
				if (!rldsym->ls_GOTdone) {
					rldsym->ls_GOTdone = TRUE;
					R1addr = (Word)((char *)(got_sect->is_rawbits->d_buf) + (rldsym->ls_GOTndx * GOTENTSZ));
					PUT4(ovalue, (char *)R1addr);
					if (Gflag) {
						BUILD_RELOC(got_sect->is_outsect_ptr, R_M32_RELATIVE, STN_UNDEF, value, &grels);
					}
				}
			}
			if (IS_PC_RELATIVE(rtype))
				...
				R1type = IS_STYPE(rtype) ? R_M32_RELATIVE_S : R_M32_RELATIVE;
		...
		if (rtype == R_M32_GOT32_S ) {
			...
			if (!rldsym->ls_GOTdone) {
				...
				if ((rldsym->ls_deftag < REF_RELOBJ) ||
					...
					(Gflag && !Bflag_symbolic)) {
						PUT4(0, (char *)R1addr);
						BUILD_RELOC(got_sect->is_outsect_ptr, R_M32_GLOB_DAT, rldsym->ls_outndx, value, &grels);
					}
				else {
					PUT4(ovalue, (char *)R1addr);
					if (Gflag && Bflag_symbolic) {
			...
			value -= refaddr; /* got references are pc-relative */
			do_reloc(isect, rtype, roff, value);
			continue;
		} 
		if (rtype == R_M32_PLT32_S) {
			...
			if ((Gflag && !Bflag_symbolic) ||
				...
				(rldsym->ls_syment->st_shndx == SHN_UNDEF)) {
				value = (Word)(plt_sect->is_newVAddr) + (rldsym->ls_PLTndx * PLTENTSZ);
				if (!rldsym->ls_PLTdone) {
					rldsym->ls_PLTdone = TRUE;
					plt_entry(rldsym, prels);
					BUILD_RELOC(plt_sect->is_outsect_ptr, R_M32_JMP_SLOT,
						rldsym->ls_outndx,
						(Word)(got_sect->is_newVAddr) + rldsym->ls_PLTGOTndx * GOTENTSZ, &prels);
				}
			}
		...
		if ((rldsym->ls_deftag >= REF_RELOBJ) && 
			...
			if (Gflag && !Bflag_symbolic && !noload) {
				...
				if (IS_PC_RELATIVE(rtype))
					...
					R1type = IS_STYPE(rtype) ?
						R_M32_RELATIVE_S : R_M32_RELATIVE;
					BUILD_RELOC(osect, R1type, STN_UNDEF, refaddr, &orels);
				}
				do_reloc(isect, rtype, roff, value);
			}
		...
		if ((rldsym->ls_deftag == REF_DEFN) || 
			...
			if (Gflag || bflag || 
				...
				(ELF_ST_TYPE(rldsym->ls_syment->st_info) == STT_NOTYPE)) {
				BUILD_RELOC(osect, rtype, rldsym->ls_outndx, refaddr, &orels);
			}
			else {
				if (ELF_ST_TYPE(rldsym->ls_syment->st_info)
					== STT_FUNC) {
					/* function */
					value = (Word)(plt_sect->is_newVAddr) + (rldsym->ls_PLTndx * PLTENTSZ);
					if (!rldsym->ls_PLTdone) {
				...
				else if ((ELF_ST_TYPE(rldsym->ls_syment->st_info) == STT_OBJECT) &&
				(rldsym->ls_syment->st_shndx != SHN_ABS)) {
					if (rldsym->ls_scnptr->is_shdr->
						sh_type != SHT_NOBITS) {
						if (!rldsym->ls_COPYdone) {
							rldsym->ls_COPYdone = TRUE;
							BUILD_RELOC(bss_sect->is_outsect_ptr, R_M32_COPY, rldsym->ls_outndx, value, &copyrels);
						}
					}
				}

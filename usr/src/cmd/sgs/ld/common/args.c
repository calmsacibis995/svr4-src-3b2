	...
	int	argc;		/* the usual passed from main */
{
	...
	if (dmode) {
		...
		if (!Gflag)		/* dynamically linked executable */
		{
			...
			{
                                lderror(MSG_WARNING,
				    "-h ignored when building a dynamic executable");
	...
	DPRINTF(DBG_ARGS, (MSG_DEBUG, "input argument to process_flags is: %#x",c));
		...
		case 'z':
			if (strncmp(optarg, "defs", 4) == SAME) {
				if (zseen)
					lderror(MSG_WARNING,"-zdefs/nodefs appears more than once: first taken");
				else
					zseen = TRUE;
				znflag = !(zdflag = TRUE);

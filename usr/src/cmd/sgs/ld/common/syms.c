			...
			if(ELF_ST_TYPE(locsym->ls_syment->st_info) == STT_SECTION ){
				...
				if (locsym->ls_scnptr)
					locsym->ls_syment->st_shndx =
						my_elf_ndxscn(locsym->ls_scnptr->
						is_outsect_ptr->os_scn);
			}
		}
	}
	return ndx;
...
update_syms()
{
	...
	for(h = 0; h< NBKTS; h++){
		...
		case SHN_ABS:
			if( strcmp(sym->ls_name,ETEXT_SYM) == SAME || strcmp(sym->ls_name,ETEXT_USYM) == SAME){
				...
				sym->ls_GOTndx = 0;
                        } else if( strcmp(sym->ls_name,END_SYM) == SAME || strcmp(sym->ls_name,END_USYM) == SAME){
				sym->ls_syment->st_value =
					dlast->sg_phdr.p_vaddr + dlast->sg_phdr.p_filesz;
	...
	if (aflag || dmode) {
		...
		if (dlast->sg_phdr.p_vaddr > tlast->sg_phdr.p_vaddr)
			eseg = dlast;

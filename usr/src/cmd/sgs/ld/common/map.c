...
ecrit_setup()
{
	...
	for (; nptr->ec_segment != EXTRA_SEG; nptr++, i++){
		if(dmode && !Gflag){
			/* dynsym is used; file symbol table will land in EXTRA_SEG */
			if(nptr->ec_type == SHT_SYMTAB) {
				nptr->ec_name = ".dynsym";
				nptr->ec_type = SHT_DYNSYM;
			} else if (nptr->ec_type == SHT_STRTAB && strcmp(nptr->ec_name,".strtab") == SAME )
				nptr->ec_name = ".dynstr";

			/* interp section must go into text */
			if(strcmp(nptr->ec_name,".interp") == SAME)

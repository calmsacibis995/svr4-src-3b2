...
#ident	"@(#)lprof:libprof/symintLoad.c	1.7"
...
PROF_FILE	*proffilePtr;
{
	 ...
	 * sanity checks.
	 */
	DEBUG(printf("profPtr = %x\n", profPtr));
	DEBUG(printf("profPtr->pf_symdat_p = %x\n", profPtr->pf_symdat_p));
	DEBUG(printf("profPtr->pf_nstsyms = %x\n", profPtr->pf_nstsyms));
	...
	DEBUG(printf("symdat_p->d_size = %x\n", symdat_p->d_size));

	prstsym_size = (symdat_p->d_size / profPtr->pf_nstsyms);
	DEBUG(printf("_symintLoad: prstsym_size = %d\n",prstsym_size));

#if isLPROF
	prsym_size = prstsym_size + sizeof(PROF_DEBUGE);
	DEBUG(printf("_symintLoad: prsym_size = %d\n",prsym_size));
	...
	DEBUG(printf("number of symbols constructed = %d\n", symcount));
#ifdef ddt
...
Elf32_Sym *cov_p;
{
	...
	symcount++;
	DEBUG(printf("debug name for ps_p = %s\n", DB_NAME(ps_p)));
...
int		*count_p;
{
	...
	DEBUG(printf("begin_p = 0x%lx,  end_p = 0x%lx\n", begin_p, end_p));
	...
	DEBUG(sn_dump("symbol table (pre sort)", list_p, count));

	qsort(list_p, count, sizeof(*list_p), sn_compare);

	DEBUG(sn_dump("symbol table (post sort)", list_p, count));
...
init_dblist()
{
	...
	DEBUG(printf("dat_p = 0x%lx, d_buf = 0x%x, d_size = %d\n",
	...
	DEBUG(printf("dblist_cnt = %d\n",dblist_cnt));
	DEBUG(printf("dblist = 0x%lx\n", dblist));
	...
	DEBUG(sn_dump("debug info (pre sort)", dblist, dblist_cnt));

	qsort(dblist, dblist_cnt, sizeof(*dblist), sn_compare);

	DEBUG(sn_dump("debug info (post sort)", dblist, dblist_cnt));
...
Elf32_Sym *prsym_p;
{
	...
	DEBUG(printf("Post search: sn_p->sn_name_p = %s\n",sn_p->sn_name_p));
...
DB_TAG		*tag_p;
{
	...
	DEBUG(printf("lo_delta = 0x%x\n", lo_delta));
	DEBUG(printf("hi_delta = 0x%x\n", hi_delta));

	line_p = curf_lp;
	pl_p = curf_lns_p - 1;

	DEBUG(printf("Building symbol: %s\n",SYMBOL_NAME(ps_p)));
	i = 0;
	while (i++ < curf_lncnt) {
		LEN4	delad;
	...
	/*
	*	Warning: There may be something wrong with this code;
	*	lali should have been found above, but sometimes (e.g.,
	*	for the last function in a file) isn't.  It may be that
	*	curf_lncnt is too small.
	*/
	if (ps_p->ps_dbg.pd_lali_p == NULL) {
		DEBUG_LOC("found last line (by default)");
		ps_p->ps_dbg.pd_lali_p = pl_p;
	}
	DEBUG(printf("first line (pd_line_p) = 0x%x\n",ps_p->ps_dbg.pd_line_p));
	DEBUG(printf("last line (pd_lali_p) = 0x%x\n",ps_p->ps_dbg.pd_lali_p));

...

/* This function will check to see if the opcode instruction is
...
char *text;
{
	...
	if (text == NULL) {
		fprintf(stderr,"ERROR - CAinstr(): %s\n", err_msg[SR107]);
		exit(FATAL);
	}
	...
	switch(*text) {
		...
		case 's':
			...
			for (i = 0; i < tblsize; i++) {
					...
					/* checks the opcode against instructions in the table */
				result = strcmp(text,cmd_tbl[i]);
				if (result == SUCCESS) {
					return(PASS);
				}
				if (result < 0) {
				    /* table is in alphabetical order, and we have
				       gone too far */
				    return(FAIL);
				}
			}

...
#ident	"@(#)acomp:common/decl.c	55.3"
/* decl.c */
...
#define	DCL_PTR		4		/* in "pointer" non-terminal */
...
static struct decl {
    ...
    I16 dcl_inparens;		/* current context is parenthesized */
...
*/
{
    int retval =  dcl_state.dcl_context != DCL_PTR && !dcl_state.dcl_inparens;
...
*/
{
    ...
    /* Also leave current state of parenthesization alone. */
    return;
}
...
void
dcl_s_ptrtsl()
/* Prepare for pointer type specifier list.  Just after seeing
** '*' in declarator.  Push new context.
*/
{
    dcl_push(DCL_PTR);
    return;
...
*/
{
    ...
    dcl_state.dcl_inparens = 1;		/* note that we are currently paren'ed */

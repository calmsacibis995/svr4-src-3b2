...
#ident	"@(#)acomp:common/optim.c	52.104"
/* optim.c */
...
*/
{
	...
	** smaller than int.  The trees look like:
	**
	**	=				op=
	**    /   \			       /   \
	**  NAME   CONV		==>	     CONV   X
	...
	*/
    ...
    if (asgop(p->op) && (l = p->left)->op == CONV && !TY_ISFPTYPE(l->type)) {
	switch( p->op ){
	case ASG MUL:
	case ASG PLUS:
	case ASG MINUS:
...
*/
{
    ...
    */
    ...
    if (TY_ISPTR(p->type) && !OP_ISNNCON(p->right)) {
	l = p->left;

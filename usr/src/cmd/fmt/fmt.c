...
#ifndef lint
static	char sccsid[] = "@(#)fmt.c 1.9 88/02/08 SMI"; /* from UCB 2.1 07/01/81 */
#endif
 ...
 *
 * Syntax: fmt [-c] [-s] [ -w width | -width ] [ name ... ]
 */
...
single:
	...
	while (argc)  {
		...
		if (cp[1] == 'c' && cp[2] == '\0') 
			crown_state = c_reset;
		else if (cp[1] == 's' && cp[2] == '\0') 
			nojoin = 1;
		else if (cp[1] == 'w' && cp[2] == '\0') {
			argc--; cp = *++argv;
			width = wconv(cp);
		}
			...
			fprintf(stderr, "usage: fmt [-c] [-s] [-w width | -width] [inputfile...]\n");
			exit(1);
		}
		argc--; argv++;
	}
	...
	char str[];
{
	...
	if (top == NOSTR) {
		(void)fprintf(stderr, "fmt:  Ran out of memory\n");
		exit(1);
	}
	copy(str, top);
	return(top);
...
char *c;
{
	...
	if (*c || n <= 0 || n >= BUFSIZ -2) {
		fprintf(stderr, "usage: fmt [-c] [-s] [-w width | -width] [inputfile...]\n");
		fprintf(stderr,"       Non-numeric character found in width specification\n");
		exit(1);
	}
	return(n);
}

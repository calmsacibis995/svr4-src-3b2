...
#ident	"@(#)oamintf:syssetup/dateset.sh	1.2"
################################################################################
#
#	Module Name: dateset
#
...
case "${7}" in
'Greenwich' | 'GMT0' | 'GMT0GDT')	tz="GMT0" ;;
 'Atlantic' | 'AST4' | 'AST4AMT') 	tz="AST4" ;;
  'Eastern' | 'EST5' | 'EST5EDT') 	tz="EST5" ;;
  'Central' | 'CST6' | 'CST6CDT') 	tz="CST6" ;;
 'Mountain' | 'MST7' | 'MST7MDT') 	tz="MST7" ;;
  'Pacific' | 'PST8' | 'PST8PDT') 	tz="PST8" ;;
    'Yukon' | 'YST8' | 'YST8YDT') 	tz="YST8" ;;
   'Alaska' | 'AST10' | 'AST10ADT') 	tz="AST10" ;;
   'Bering' | 'BST11' | 'BST11BDT') 	tz="BST11" ;;
   'Hawaii' | 'HST10' | 'HST10HDT') 	tz="HST10" ;;
       		             '*')	tz="EST5" ;;
esac
tc=`echo $tz | cut -c1-3`
if test "$8" = "yes"

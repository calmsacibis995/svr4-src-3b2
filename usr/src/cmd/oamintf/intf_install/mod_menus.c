...
#ident	"@(#)oamintf:intf_install/mod_menus.c	1.6"
...
char **argv;
{
	...
	while(fgets(instr, sizeof(instr), miffile)) {
		...
		/* get action */
		/* doesn't matter if action is NULL */
		action = menutok(NULL);
...
char *name;		/* name of new menu item to write into menu file */
{
	...
	char menu_nam[20];	/* menu name - truncated to MAXSTR */
	...
	len = strlen(new_dir);
	(void) strncat(new_dir, name, MAXDIR);
	*(new_dir + len + MAXDIR) = NULL;
...
{
	int len;		/* string lenght of action */

	(void) strcpy(itemptr->mdescr, descr);
	itemptr->pholder = P_NONE;
	if(strncmp(action, LOC_IDENT, strlen(LOC_IDENT)) != 0) {
		/* orig name is NULL, then no rename present */
		if(*(itemptr->orig_name) == NULL)
			(void) strncpy(itemptr->maction, itemptr->mname, MAXDIR);
		else if(action != NULL) /* task, use orig name */
			(void) strncpy(itemptr->maction, itemptr->orig_name,
				MAXDIR);
		else if(action == NULL)	/* menu, use itemptr->mname */
			(void) strncpy(itemptr->maction, itemptr->mname,MAXDIR);
		*(itemptr->maction + MAXDIR) = NULL;
...
char	*cur_dir;
{
	...
	do {
		...
		if(strncmp(m_item->item, itemptr->mname, MAXITEMLEN) == 0) {
		...
		} else if(strcmp(m_item->item, itemptr->mname) > 0) {
			if(itemptr->next == NULL) {
				/* set done so it will 
		...
		if((strncmp(m_item->item, itemptr->mname, strlen(m_item->item))
			< 0) || done) {
			/*
			...
			*(newitem->mname + MAXITEMLEN) = NULL;
			(void) strcpy(newitem->orig_name, NULL);
			cp_pkginst = 1;

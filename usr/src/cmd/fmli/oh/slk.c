 ...
 *    "yes", "y", or ""  (leading whitespace ignored.)  Prompt only occurs 
...
bool do_prompt;				/* abs k18 */
{
	...
	if (no_pad_char)	/* can't create delay by sending pads */
	{
	    ...
	    if (Mandatory_delay)
	    {
		delay_time.it_value.tv_usec = Mandatory_delay * 1000L;
		delay_time.it_value.tv_sec = 0L; 		

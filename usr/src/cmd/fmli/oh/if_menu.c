 ...
 *	All Rights Reserved
 */
#ident	"@(#)vm.oh:src/oh/if_menu.c	1.39"
...
register struct actrec *a;
{
    ...
    register int i, but;
    char buf[10];
    char *str, *label, *intr,  *onintr, *get_def();
    menuinfo *mi;
    int	lcv;
    ...
    ar_ctl(a, CTSETINTR, get_sing_def(CURmenu(), MN_INTR));
    ar_ctl(a, CTSETONINTR, get_sing_def(CURmenu(), MN_ONINTR));

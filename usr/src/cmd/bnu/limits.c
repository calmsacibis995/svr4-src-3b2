...
struct limits *limitval;
{
	...
	if ((strcmp(flds[U_SERVICE],CNULL)==0) ||
	 			(strcmp(flds[U_MAX],CNULL)==0))
		return FAIL;
	if ((strcmp(flds[U_SERVICE],service) != 0) || 
			(strcmp(flds[U_SYSTEM],CNULL)) != 0)
		return FAIL;
...
struct limits *limitval;
{
	...
	if (strcmp(flds[U_SERVICE],"uucico") != 0)
		return FAIL; 
	if (strcmp(flds[U_SYSTEM],CNULL)==0 || (sysmatch(flds[U_SYSTEM]) != 0))
		return FAIL;
	if (strcmp(flds[U_MAX],CNULL) == 0)
		limitval->sitemax = 1; /* default value */
	else if (sscanf(flds[U_MAX],"%d",&limitval->sitemax)==0)
			limitval->sitemax = -1; /* type conflict*/

	if (strcmp(flds[U_MODE],CNULL) == 0)

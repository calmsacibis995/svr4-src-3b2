...
#define USAGE1	"[-q] or [-m] or [-kJOB [-n]] or [-rJOB [-n]] or [-p]"
#define USAGE2	"[-a] [-sSYSTEM [-j]] [-uUSER] [-SSTATE]"
#define USAGE3  "-tSYSTEM [-d number] [-c]"
#define LOCK "LCK.."
#define STST_MAX	132
#define MAXDATE		12
#define MINTIME		60
...
char **envp;
{
	...
	while ((i = getopt(argc, argv, "acjk:mnpr:qs:u:x:t:d:S:")) != EOF) {
		...
		case 'S':
			State = 1;
			(void) strcpy(arglist, optarg);
			if (strlen(arglist) > (size_t)MAXSTATE)
				{
				errortn();
				  exit(1);
				}
...
char   *machine, *dir, *file;
{
	...
	if (s.st_size == 0 && User[0] == '\0') { /* dummy D. for polling */
		...
		if (xfp == NULL) { /* program error */
		    ...
		    DEBUG(4, "errno=%d -- skip it!\n", errno);
		    sprintf(format_tmp,"%s  %s  %s  ", type, dir, user);
		    (void) strcat(outbuf, format_tmp);
		    (void) strcat(outbuf,"????\n");
		    (void) strcat(outbuf, format_tmp);
		}
...
char *file, *dir;
{
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
	    ...
	    if (first) {
			...
			fprintf(stderr,
			    "Not owner, uucp or root - can't %s job %s\n",
	...
	if (ret != 0) {
	    /* program error?? */
		fprintf(stderr, "Error1: Can't %s, File ( %s), errno (%d)\n",
...
if (fp == NULL)
   {
	fprintf(stderr,"Can't open account log\n");
		return;
   }
...
long timerange;
{
	...
	if (fp == NULL)
 	  {
		(void) fprintf(stderr, "Can't open performance log\n");
			return(0);
	   }
	...
	while (fgets(abuf, BUFSIZ, fp) != NULL)
	  {
		...
		rtnint = strncmp(startime, time, MAXDATE);
		rtnint1 = strncmp(upperlimit, time,   MAXDATE);

		DEBUG(9, "  rtnint = %d\n", rtnint);
		DEBUG(9, "  rtnint1 = %d\n", rtnint1);
		if (rtnint <= 0 && rtnint1 >= 0)
		{
			totaljob++;	

...
extern char *Mytype;	/* flag to force requested line type to be used 
			 ...
			 * the command line. */
static char *P_USAGE= "USAGE:%s [-c device] [-s speed] [-l line] [-b 7|8] [-h] [-n] [-t] [-d] [-o|-e] telno | systemname\n";
...
/*are set in fixline() in culine.c before remote connection is made	   */
	...
	switch(i) {
		...
		if ( line_8bit == -1 ) {
		    (void) fprintf(stderr,
			"%s: b option value must be '7' or '8'\n", argv[0]);
		    exit(1);
		}
		break;
	    case '?':
    ...
    if(strlen(Cucall.telno) >= (size_t)(MAXPH - 1)) {
	VERBOSE(P_TELLENGTH,"");
	exit(0);
    }

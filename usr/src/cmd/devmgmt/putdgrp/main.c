...
#ident	"@(#)devmgmt:putdgrp/main.c	1.6.1.1"
...
#define	E_NODGRP	"Device-group does not exist in the table: %s"
	...
	char   *argv[];			/* Argument list */
{
	...
	if (d_seen) {
	    ...
	    case EACCES:
		p = _dgrptabpath();
		if (access(p, R_OK)) (void) sprintf(msg, E_NOMKTAB, p);
		else (void) sprintf(msg, E_NODGRPTAB, p);
		stdmsg(MM_NRECOV, lbl, MM_ERROR, msg);
		exitcd = EX_DGROUP; 
		break;

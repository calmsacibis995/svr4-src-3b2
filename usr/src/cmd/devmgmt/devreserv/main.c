...
#ident	"@(#)devmgmt:devreserv/main.c	1.7"
 ...
 *	lists presented to it, reserving them on the key (<pid>).  If no
 *	device-lists are provided, the command lists those devices reserved
 *	on the given key (<pid>).  If no key (<pid>) is provided, the 
 *	command lists all devices currently reserved.
 *
 *  Options:  None
 *
 *  Arguments:
 *	pid		Key to lock the devices on
	...
	char	       *argv[];		/* Arg vector */
{
	...
	if (argcount == 0) {
		...
		/* Write the list of reserved devices with the key (pid)
	 ...
	 *  devreserv pid
	 *
	 *  	Generate a list of the devices allocated on the specific 
	 *	key "pid"
	 */
	...
	if (argcount == 1) {
	    ...
	    pid = (pid_t) strtol(*argp, &p, 10);
	    if (*p || (pid <= 0) || (pid > MAXPID)) {

		/* <pid> argument invalid */
		...
		if (rsvd = reservdev()) {
		    ...
		    exitcode = EX_OK;
		    for (plk = rsvd ; *plk ; plk++) {
			if ((*plk)->key == pid) (void) puts((*plk)->devname);
		    }
	...
	if (reqlist = buildreqlist(argp)) {
	    ...
	    if (alloclist = devreserv(pid, reqlist)) {
